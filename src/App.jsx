    ...
    <div className={`min-h-screen transition-colors duration-300 ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      ...
      <div className="max-w-7xl mx-auto px-4 py-4">
        <div className="grid grid-cols-3 md:grid-cols-6 gap-2 mb-4">
          <Kpi label="PIB T3" value={`+${d.indicateurs_cles.croissance_pib}%`} color="green" darkMode={darkMode} />
          <Kpi label="Climat affaires" value={d.indicateurs_cles.climat_affaires} color="blue" darkMode={darkMode} />
          <Kpi label="Chômage" value={`${d.indicateurs_cles.taux_chomage_actuel}%`} color="red" darkMode={darkMode} />
          <Kpi label="Inflation" value={`${d.indicateurs_cles.inflation_annuelle}%`} color="green" darkMode={darkMode} />
          <Kpi label="SMIC net" value={`${d.indicateurs_cles.smic_net}€`} color="blue" darkMode={darkMode} />
          <Kpi label="Défaillances" value={`${(d.indicateurs_cles.defaillances_12m/1000).toFixed(1)}k`} color="orange" darkMode={darkMode} />
        </div>

        <div className={`flex flex-wrap gap-2 mb-4 border-b pb-3 overflow-x-auto ${darkMode ? 'border-gray-700' : ''}`}>
          {[['conjoncture','📈 Conjoncture'],['pouvoir_achat','💰 Pouvoir d\'achat'],['salaires','💵 Salaires'],['emploi','👥 Emploi'],['conditions_vie','🏠 Conditions vie'],['inflation','📊 Inflation'],['conventions','📋 Conventions'],['comparaison_ue','🇪🇺 Europe'],['simulateur','🧮 Simulateur NAO'],['aide','📖 Aide']].map(([id,label]) => (
...
function Kpi({label, value, color, darkMode}) {
  const colors = {
