  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      ...
      <div className="max-w-7xl mx-auto px-2 sm:px-4 py-2 sm:py-4">
        ...
        <div className={`flex gap-1.5 sm:gap-2 mb-3 sm:mb-4 border-b pb-2 sm:pb-3 overflow-x-auto scrollbar-hide ${darkMode ? 'border-gray-700' : ''}`} style={{WebkitOverflowScrolling: 'touch'}}>
          {[['conjoncture','📈 Conjoncture'],['previsions','🔮 Prévisions'],['evolutions','📉 Évolutions'],['pouvoir_achat','💰 Pouvoir d\'achat'],['salaires','💵 Salaires'],['emploi','👥 Emploi'],['travail','⚙️ Travail'],['territoires','🗺️ Territoires'],['conditions_vie','🏠 Conditions'],['inflation','📊 Inflation'],['conventions','📋 Conventions'],['comparaison_ue','🇪🇺 Europe'],['simulateur','🧮 Simulateur'],['aide','📖 Aide']].map(([id,label]) => (
            <button key={id} onClick={() => setTab(id)} className={`px-2 sm:px-3 py-1 sm:py-1.5 rounded text-xs sm:text-sm font-medium whitespace-nowrap transition-colors flex-shrink-0 ${tab === id ? 'bg-blue-600 text-white' : darkMode ? 'bg-gray-800 border-gray-700 text-gray-300 hover:bg-gray-700' : 'bg-white border text-gray-600 hover:bg-gray-50'}`}>{label}</button>
          ))}
        </div>
