...
// ==================== CONTENU PANNEAU PRESSE ====================
  ...
  const index = React.useMemo(() => {
    ...
    const add = (label, valeur, onglet, keywords = [], sparkline = null, sous = '') => {
      items.push({ label, valeur, onglet, keywords: [label.toLowerCase(), ...keywords], sparkline, sous });
    };

    // ── Indicateurs clés ──
    add('Taux de chômage', `${ic.taux_chomage_actuel}%`, 'emploi',
      ['chomage', 'chômage', 'bit', 'unemployment'],
      d.sparklines?.chomage, 'BIT — T4 2025');
    add('Chômage des jeunes', `${ic.taux_chomage_jeunes}%`, 'emploi',
      ['jeunes', 'jeunesse', '15-24', 'youth']);
    add('Taux emploi seniors', `${ic.taux_emploi_seniors}%`, 'emploi',
      ['seniors', '55-64', 'emploi seniors']);
    add('Inflation annuelle', `${ic.inflation_annuelle}%`, 'inflation',
      ['inflation', 'ipc', 'prix', 'hausse des prix'],
      d.sparklines?.inflation, 'Glissement annuel IPC');
    add('SMIC brut mensuel', `${ic.smic_brut}€`, 'simulateur',
      ['smic', 'salaire minimum', 'smig'],
      d.sparklines?.smic);
    add('SMIC net mensuel', `${ic.smic_net}€`, 'simulateur',
      ['smic net', 'salaire minimum net'],
      d.sparklines?.smic);
    add('Salaire médian', `${ic.salaire_median}€`, 'salaires',
      ['médian', 'median', 'salaire', 'rémunération']);
    add('Salaire moyen', `${ic.salaire_moyen}€`, 'salaires',
      ['moyen', 'salaire moyen', 'rémunération moyenne']);
    add('Écart salarial H/F', `${ic.ecart_hf_eqtp}%`, 'salaires',
      ['inégalité', 'femmes', 'hommes', 'égalité', 'hf', 'ecart']);
    add('Difficultés recrutement', `${ic.difficultes_recrutement}%`, 'emploi',
      ['recrutement', 'tension', 'recruter', 'difficultés']);
    add('IRL (glissement)', `+${ic.irl_glissement}%`, 'conditions_vie',
      ['irl', 'loyer', 'indice loyer', 'révision loyer']);
    add('Prix du gazole', `${ic.prix_gazole}€/L`, 'conditions_vie',
      ['gazole', 'diesel', 'carburant', 'essence']);
    add('Taux effort locataires', `${ic.taux_effort_locataires}%`, 'conditions_vie',
      ['effort', 'logement', 'loyer', 'taux effort']);
    add('Croissance PIB', `+${ic.croissance_pib}%`, 'conjoncture',
      ['pib', 'croissance', 'gdp', 'économie']);
    add('Climat des affaires', `${ic.climat_affaires}`, 'conjoncture',
      ['climat', 'confiance', 'insee', 'conjoncture']);
    add('Défaillances entreprises', `${(ic.defaillances_12m/1000).toFixed(1)}k`, 'conjoncture',
      ['défaillances', 'faillites', 'liquidation', 'redressement']);

    // ── Partage VA ──
    const pva = d.partage_va || {};
    add('Part salaires dans la VA', `${pva.part_salaires_snf}%`, 'conjoncture',
      ['valeur ajoutée', 'va', 'partage', 'salaires snf']);
    add('Taux de marge EBE', `${pva.part_ebe_snf}%`, 'conjoncture',
      ['marge', 'ebe', 'excédent brut', 'profits', 'bénéfices']);

    // ── PIB ──
    const pib = d.pib || {};
    add('Croissance trimestrielle PIB', `+${pib.croissance_trim_actuel}%`, 'conjoncture',
      ['pib', 'trimestriel', 'croissance t3', 'gdp']);
    add('Croissance annuelle PIB', `+${pib.croissance_annuelle}%`, 'conjoncture',
      ['pib', 'annuel', 'croissance 2024']);

    // ── Salaires secteurs ──
    (d.salaires_secteur || []).forEach(s => {
      add(`Salaire médian — ${s.secteur}`, `${s.salaire.toLocaleString('fr-FR')}€`, 'salaires',
        ['salaire', s.secteur.toLowerCase(), 'secteur', 'rémunération'],
        null, `Évol. ${s.evolution >= 0 ? '+' : ''}${s.evolution}%/an`);
    });

    // ── Comparaison UE ──
    (d.comparaison_ue?.smic_europe || []).forEach(p => {
      add(`SMIC — ${p.pays}`, `${p.smic.toLocaleString('fr-FR')}€`, 'comparaison_ue',
        ['europe', 'ue', p.pays.toLowerCase(), 'smic europe', 'salaire minimum europe']);
    });

    // ── IRL ──
    const irl = d.irl || {};
    add('IRL (valeur actuelle)', `${irl.valeur_actuelle}`, 'conditions_vie',
      ['irl', 'indice référence loyers', 'loyer']);

    // ── Carburants ──
    const carb = d.carburants || {};
    if (carb.gazole) add('Gazole', `${carb.gazole.prix}€/L`, 'conditions_vie',
      ['gazole', 'diesel', 'carburant']);
    if (carb.sp95) add('SP95', `${carb.sp95.prix}€/L`, 'conditions_vie',
      ['sp95', 'essence', 'carburant']);

    // ── Conventions collectives ──
    const cc = d.conventions_collectives || {};
    add('Branches non conformes SMIC', `${cc.statistiques_branches?.branches_non_conformes} / ${cc.statistiques_branches?.total_branches}`, 'conventions',
      ['convention', 'branche', 'smic', 'conformité', 'minima']);
    (cc.branches || []).forEach(b => {
      add(`Convention — ${b.nom}`, b.statut === 'conforme' ? '✅ Conforme' : '❌ Non conforme', 'conventions',
        ['convention', 'ccn', b.nom.toLowerCase(), `idcc ${b.idcc}`, 'branche'],
        null, `IDCC ${b.idcc} · Min ${b.grille[0]?.minimum_mensuel}€`);
    });

    // ── Emploi vacants ──
    const ev = d.emplois_vacants || {};
    if (ev.taux_vacance?.length > 0) {
      const last = ev.taux_vacance[ev.taux_vacance.length - 1];
      add('Taux emplois vacants', `${last.taux}%`, 'emploi',
        ['vacants', 'postes', 'offres', 'emplois vacants']);
    }

    // ── PPV ──
    const ppv = d.ppv || {};
    add('Prime de partage de la valeur (PPV)', `${ppv.montant_moyen}€ moy.`, 'salaires',
      ['ppv', 'prime macron', 'partage valeur', 'prime exceptionnelle']);
  ...
  const handleKey = (e) => {
    if (e.key === 'ArrowDown') { e.preventDefault(); setSelected(s => Math.min(s + 1, results.length - 1)); }
    if (e.key === 'ArrowUp')   { e.preventDefault(); setSelected(s => Math.max(s - 1, 0)); }
    if (e.key === 'Enter' && results[selected]) onNavigate(results[selected].onglet);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[10vh] px-4"
      ...
      <div
        className={`relative w-full max-w-xl rounded-2xl shadow-2xl overflow-hidden ${darkMode ? 'bg-gray-900' : 'bg-white'}`}
        onClick={e => e.stopPropagation()}
        ...
        <div className={`flex items-center gap-3 px-4 py-3.5 border-b ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
          ...
          <input
            ...
            onKeyDown={handleKey}
            placeholder="Chercher un indicateur, un chiffre, un onglet..."
          ...
          />
          <kbd className={`shrink-0 text-xs px-1.5 py-0.5 rounded border ${darkMode ? 'border-gray-600 text-gray-500' : 'border-gray-300 text-gray-400'}`}>
            Esc
          </kbd>
        </div>
        ...
        <div className="max-h-[60vh] overflow-y-auto py-2">
          {results.length === 0 ? (
            ...
            <button key={i} onClick={() => onNavigate(r.onglet)}
              onMouseEnter={() => setSelected(i)}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className={`text-sm font-semibold truncate ${darkMode ? 'text-gray-100' : 'text-gray-800'}`}>
                    {r.label}
                  </span>
                  <span className={`text-sm font-black shrink-0 ${
                    i === selected ? (darkMode ? 'text-blue-400' : 'text-blue-600') : (darkMode ? 'text-white' : 'text-gray-900')
                  }`}>
                    {r.valeur}
                  </span>
                </div>
                <div className="flex items-center gap-2 mt-0.5">
                  <span className={`text-[11px] px-1.5 py-0.5 rounded-lg ${darkMode ? 'bg-gray-700 text-gray-400' : 'bg-gray-100 text-gray-500'}`}>
                    {ONGLET_LABELS[r.onglet] || r.onglet}
                  </span>
                  {r.sous && (
                    <span className={`text-[11px] ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>{r.sous}</span>
                  )}
                </div>
              </div>
              ...
              {r.sparkline && (
                <div className="shrink-0">
                  <MiniSparkline data={r.sparkline} color={i === selected ? '#3b82f6' : (darkMode ? '#6b7280' : '#9ca3af')} />
                </div>
              )}

              {/* Flèche */}
              <span className={`shrink-0 text-xs ${i === selected ? (darkMode ? 'text-blue-400' : 'text-blue-500') : (darkMode ? 'text-gray-600' : 'text-gray-300')}`}>
                ↵
              </span>
            </button>
          ))}
        ...
        {/* Footer */}
        <div className={`px-4 py-2 border-t flex items-center gap-4 text-[11px] ${darkMode ? 'border-gray-700 text-gray-600' : 'border-gray-100 text-gray-400'}`}>
          <span>↑↓ naviguer</span>
          <span>↵ ouvrir l'onglet</span>
          <span>Esc fermer</span>
          <span className="ml-auto">{index.length} indicateurs indexés</span>
        </div>
      </div>
    </div>
  );
