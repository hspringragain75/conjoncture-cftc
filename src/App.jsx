    ...
    <div className="space-y-4">
      <div className={`flex flex-wrap gap-1 p-2 rounded-lg shadow-lg ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
        {[['pib','Croissance PIB'],['partage_va','⚖️ Partage VA'],['climat','Climat affaires'],['defaillances','Défaillances'],['investissement','Investissement']].map(([id,label])=>(
          <button key={id} onClick={()=>setSubTab(id)} className={`px-3 py-1.5 rounded-xl text-xs font-medium ${subTab===id?'bg-indigo-600 text-white': darkMode ? 'text-gray-300 hover:bg-gray-700' : 'text-gray-600 hover:bg-gray-100'}`}>{label}</button>
        ))}
      </div>
...
function ComparaisonUETab({d, darkMode}) {
  ...
  return (
    <div className="space-y-4">
      <div className={`flex flex-wrap gap-1 p-2 rounded-2xl ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow-lg`}>
        {[['smic','💰 SMIC'],['chomage','👥 Chômage'],['partage_va','⚖️ Part salaires VA']].map(([id,label])=>(
          <button key={id} onClick={()=>setSubTab(id)} className={`px-3 py-1.5 rounded-xl text-xs font-medium transition-all ${subTab===id?'bg-blue-600 text-white shadow': darkMode ? 'text-gray-300 hover:bg-gray-700' : 'text-gray-600 hover:bg-gray-100'}`}>{label}</button>
        ))}
      </div>
    ...
    <div className="space-y-4">
      ...
      <div className={`flex flex-wrap gap-1 p-2 rounded-lg shadow-lg ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
        {[['general','📊 Général'],['budget','🧺 Budget UNAF'],['heures','⏱️ Heures de travail']].map(([id,label])=>(
          <button key={id} onClick={()=>setSubTab(id)} className={`px-3 py-1.5 rounded-xl text-sm font-medium ${subTab===id?'bg-orange-600 text-white': darkMode ? 'text-gray-300 hover:bg-gray-700' : 'text-gray-600 hover:bg-gray-100'}`}>{label}</button>
        ))}
      </div>
          ...
          <Card title="📈 Évolution du budget 2020-2025" darkMode={darkMode}>
            <ResponsiveContainer width="100%" height={200}>
              <ComposedChart data={historiqueData}>
                ...
                <Bar radius={[6, 6, 0, 0]} dataKey="total" fill={C.primary} radius={[4, 4, 0, 0]}>
          ...
          <Card title="🥧 Répartition par poste" darkMode={darkMode}>
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={repartitionData} layout="vertical">
                ...
                <Bar radius={[6, 6, 0, 0]} dataKey="montant" radius={[0, 4, 4, 0]}>
              ...
              <div>
                ...
                <ResponsiveContainer width="100%" height={180}>
                  <BarChart data={evolutionProduit}>
                    ...
                    <Bar radius={[6, 6, 0, 0]} dataKey="heures" fill={C.primary} radius={[4, 4, 0, 0]}>
    ...
    <div className="space-y-4">
      <div className={`flex flex-wrap gap-1 p-2 rounded-2xl ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow-lg`}>
        {[['chomage','Chômage'],['seniors','Seniors'],['contrats','Contrats'],['secteurs','Secteurs'],['recrutement','Recrutement'],['dynamique','Dynamique']].map(([id,label])=>(
          <button key={id} onClick={()=>setSubTab(id)} className={`px-3 py-1.5 rounded-xl text-xs font-medium transition-all ${subTab===id?'bg-purple-600 text-white shadow': darkMode ? 'text-gray-300 hover:bg-gray-700' : 'text-gray-600 hover:bg-gray-100'}`}>{label}</button>
        ))}
      </div>
    ...
    <div className="space-y-4">
      <div className={`flex flex-wrap gap-1 p-2 rounded-2xl ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow-lg`}>
        {[['loyers','Loyers (IRL)'],['immobilier','Prix immobilier'],['carburants','Carburants'],['effort','Taux d\'effort']].map(([id,label])=>(
          <button key={id} onClick={()=>setSubTab(id)} className={`px-3 py-1.5 rounded-xl text-xs font-medium transition-all ${subTab===id?'bg-cyan-600 text-white shadow': darkMode ? 'text-gray-300 hover:bg-gray-700' : 'text-gray-600 hover:bg-gray-100'}`}>{label}</button>
        ))}
      </div>
      ...
      <div className={`flex flex-wrap gap-2 p-2 rounded-lg ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow`}>
        {[
          ...
          <button 
            ...
            className={`px-3 py-1.5 rounded-xl text-xs font-medium transition-all ${graphType === id ? 'bg-indigo-600 text-white' : darkMode ? 'text-gray-300 hover:bg-gray-700' : 'text-gray-600 hover:bg-gray-100'}`}
          >
            {label}
          </button>
        ))}
