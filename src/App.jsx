    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 py-4">
        ...
        <div className="flex flex-wrap gap-2 mb-4 border-b pb-3 overflow-x-auto">
          {[['conjoncture','📈 Conjoncture'],['pouvoir_achat','💰 Pouvoir d\'achat'],['salaires','💵 Salaires'],['emploi','👥 Emploi'],['conditions_vie','🏠 Conditions vie'],['inflation','📊 Inflation'],['comparaison_ue','🇪🇺 Europe'],['simulateur','🧮 Simulateur NAO']].map(([id,label]) => (
...
function SimulateurNAOTab({d}) {
  ...
  // Taux de cotisations salariales selon statut (2025)
  const tauxCotisations = {
    'non_cadre': 0.23,      // ~23% : sécu + retraite AGIRC-ARRCO + CSG/CRDS
    'cadre': 0.25,          // ~25% : idem + APEC + prévoyance cadre
    'fonctionnaire': 0.17   // ~17% : pension civile 11.1% + RAFP 5% + CSG ~1%
  };
  ...
  // Prime d'activité 2025 (barèmes officiels CAF avril 2025)
  const calculPrimeActivite = (netMensuel, sit, enf) => {
    // Montants forfaitaires 2025
    const forfaitBase = 633.21;
    let forfait = forfaitBase;
    
    // Majoration selon composition foyer
    if (sit === 'couple') {
      forfait = forfaitBase * 1.5; // 949.82€
    }
    // Majoration enfants : +30% par enfant (212.17€)
    forfait += enf * forfaitBase * 0.3;
    
    // 61% des revenus d'activité
    const partRevenus = netMensuel * 0.61;
    
    // Bonification individuelle (si revenus > 700.92€)
    let bonification = 0;
    if (netMensuel >= 700.92) {
      if (netMensuel >= 1416) {
        bonification = 184.27; // Max
      } else {
        // Progressif entre 700.92€ et 1416€
        bonification = ((netMensuel - 700.92) / (1416 - 700.92)) * 184.27;
      }
    }
    
    // Calcul de la prime
    const baseCalcul = forfait + partRevenus + bonification;
    let prime = baseCalcul - netMensuel;
    
    // Forfait logement déduit si APL (approximation)
    const forfaitLogement = sit === 'seul' ? 76.04 : 152.08;
    prime -= forfaitLogement;
    
    // Plafonds et seuils
    if (netMensuel < 595) return 0; // Seuil minimum (étudiants/apprentis: 1104€)
    if (sit === 'seul' && enf === 0 && netMensuel > 1900) return 0;
    if (sit === 'couple' && enf === 0 && netMensuel > 2500) return 0;
    if (prime < 15) return 0; // Seuil versement
    
    return Math.max(0, Math.round(prime));
  };
  
  const primeActuelle = calculPrimeActivite(salaireNet, situation, enfants);
  const primeNouvelle = calculPrimeActivite(nouveauNet, situation, enfants);
  const pertePrime = Math.max(0, primeActuelle - primeNouvelle);
  
  // Gain réel après impact prestations
  const gainReelMensuel = gainNetMensuel - pertePrime;
  const gainReelAnnuel = gainReelMensuel * 12;
  
  // Exonérations employeur (info)
  const smicBrut = 1823.03;
  const ratioSmic = salaireBrut / smicBrut;
  const beneficieExonerations = ratioSmic <= 1.6;
  
  // Coût employeur (approximation)
  const tauxPatronal = beneficieExonerations ? 0.30 : 0.45;
  const coutEmployeurActuel = salaireBrut * (1 + tauxPatronal);
  const coutEmployeurNouveau = nouveauBrut * (1 + (nouveauBrut/smicBrut <= 1.6 ? 0.30 : 0.45));
  const coutSupplementaireEmployeur = coutEmployeurNouveau - coutEmployeurActuel;

  return (
    <div className="space-y-4">
      <div className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white p-4 rounded-xl">
        <h2 className="text-lg font-bold">🧮 Simulateur NAO - Impact d'une augmentation</h2>
        <p className="text-sm opacity-80">Calcul précis avec prestations socio-fiscales (barèmes 2025)</p>
      </div>
      ...
      <div className="grid md:grid-cols-4 gap-4">
        <Card title="📝 Situation actuelle">
          <div className="space-y-3">
            <div>
              ...
              <select value={statut} onChange={e => setStatut(e.target.value)} className="w-full border rounded p-2 text-sm mt-1">
                <option value="non_cadre">🏢 Salarié non-cadre</option>
                <option value="cadre">👔 Salarié cadre</option>
                <option value="fonctionnaire">🏛️ Fonctionnaire</option>
              </select>
            </div>
            <div>
              <label className="text-xs text-gray-600 font-medium">Salaire brut mensuel</label>
              <input type="range" min="1200" max="5000" step="50" value={salaireBrut} onChange={e => setSalaireBrut(Number(e.target.value))} className="w-full mt-1" />
              <div className="text-center font-bold text-blue-600 text-lg">{salaireBrut.toLocaleString()}€ brut</div>
              <div className="text-center text-sm text-gray-500">{Math.round(salaireNet)}€ net ({(tauxNet*100).toFixed(0)}%)</div>
            </div>
            <div>
              <label className="text-xs text-gray-600 font-medium">Situation familiale</label>
              <select value={situation} onChange={e => setSituation(e.target.value)} className="w-full border rounded p-2 text-sm mt-1">
                <option value="seul">Seul(e)</option>
                <option value="couple">En couple</option>
              </select>
            </div>
            <div>
              <label className="text-xs text-gray-600 font-medium">Enfants à charge</label>
              <select value={enfants} onChange={e => setEnfants(Number(e.target.value))} className="w-full border rounded p-2 text-sm mt-1">
        ...
        <Card title="📈 Négociation">
          <div className="space-y-3">
            <div>
              <label className="text-xs text-gray-600 font-medium">Augmentation demandée</label>
              <input type="range" min="0" max="10" step="0.5" value={augmentation} onChange={e => setAugmentation(Number(e.target.value))} className="w-full mt-1" />
              <div className="text-center font-bold text-green-600 text-2xl">+{augmentation}%</div>
            </div>
            <div>
              <label className="text-xs text-gray-600 font-medium">Inflation de référence</label>
              <input type="range" min="0" max="8" step="0.1" value={inflation} onChange={e => setInflation(Number(e.target.value))} className="w-full mt-1" />
              <div className="text-center font-medium text-orange-600">{inflation}%</div>
              <p className="text-xs text-gray-400 text-center">Actuel: {d.indicateurs_cles.inflation_annuelle}%</p>
            </div>
            <div className={`p-3 rounded text-center ${pouvoirAchatPreserve ? 'bg-green-50' : 'bg-red-50'}`}>
              <p className="text-xs text-gray-600">Augmentation réelle</p>
              <p className={`text-xl font-bold ${augReelle >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                {augReelle >= 0 ? '+' : ''}{augReelle.toFixed(1)}%
              </p>
              <p className="text-xs">{pouvoirAchatPreserve ? '✅ Pouvoir d\'achat préservé' : '❌ Perte de pouvoir d\'achat'}</p>
            </div>
          </div>
        </Card>

        <Card title="💰 Impact sur le salaire">
          <div className="space-y-2">
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div className="p-2 bg-gray-50 rounded text-center">
                <p className="text-xs text-gray-500">Brut actuel</p>
                <p className="font-bold">{salaireBrut.toLocaleString()}€</p>
              </div>
              <div className="p-2 bg-green-50 rounded text-center">
                <p className="text-xs text-gray-500">Nouveau brut</p>
                <p className="font-bold text-green-600">{Math.round(nouveauBrut).toLocaleString()}€</p>
              </div>
              <div className="p-2 bg-gray-50 rounded text-center">
                <p className="text-xs text-gray-500">Net actuel</p>
                <p className="font-bold">{Math.round(salaireNet)}€</p>
              </div>
              <div className="p-2 bg-green-50 rounded text-center">
                <p className="text-xs text-gray-500">Nouveau net</p>
                <p className="font-bold text-green-600">{Math.round(nouveauNet)}€</p>
              </div>
            </div>
            <div className="p-3 bg-blue-50 rounded">
              <div className="flex justify-between text-sm">
                <span>Gain net mensuel</span>
                <span className="font-bold text-blue-600">+{Math.round(gainNetMensuel)}€</span>
              </div>
              <div className="flex justify-between text-xs text-gray-600">
                <span>Gain net annuel</span>
                <span>+{Math.round(gainNetAnnuel).toLocaleString()}€</span>
              </div>
            </div>
            <div className="text-xs p-2 bg-gray-100 rounded">
              <p className="font-medium">Taux cotisations {statut === 'fonctionnaire' ? 'agent' : 'salariales'}</p>
              <p>{(tauxCotisations[statut]*100).toFixed(0)}% ({statut === 'non_cadre' ? 'non-cadre' : statut === 'cadre' ? 'cadre' : 'fonctionnaire'})</p>
            </div>
          </div>
        </Card>

        <Card title="🏛️ Prestations socio-fiscales">
          <div className="space-y-2">
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div className="p-2 bg-purple-50 rounded text-center">
                <p className="text-xs text-gray-500">Prime activité avant</p>
                <p className="font-bold text-purple-600">{primeActuelle}€/mois</p>
              </div>
              <div className="p-2 bg-purple-50 rounded text-center">
                <p className="text-xs text-gray-500">Prime après</p>
                <p className="font-bold text-purple-600">{primeNouvelle}€/mois</p>
              </div>
            </div>
            {pertePrime > 0 && (
              <div className="p-2 bg-red-50 rounded text-sm text-red-700 text-center">
                ⚠️ Perte prime activité : <b>-{pertePrime}€/mois</b>
              </div>
            )}
            <div className="p-4 bg-green-100 rounded text-center">
              <p className="text-xs text-gray-600">GAIN RÉEL MENSUEL</p>
              <p className="text-3xl font-bold text-green-600">+{Math.round(gainReelMensuel)}€</p>
              <p className="text-xs text-gray-500">soit +{Math.round(gainReelAnnuel).toLocaleString()}€/an</p>
            </div>
            {pertePrime > 0 && (
              <p className="text-xs text-orange-600 text-center">
                ⚡ Effet de seuil : {Math.round((pertePrime / gainNetMensuel) * 100)}% du gain absorbé
              </p>
            )}
          </div>
        </Card>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <Card title="🏭 Coût pour l'employeur">
          <div className="space-y-2">
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div className="p-2 bg-gray-50 rounded">
                <p className="text-xs text-gray-500">Coût actuel</p>
                <p className="font-bold">{Math.round(coutEmployeurActuel).toLocaleString()}€</p>
              </div>
              <div className="p-2 bg-orange-50 rounded">
                <p className="text-xs text-gray-500">Nouveau coût</p>
                <p className="font-bold text-orange-600">{Math.round(coutEmployeurNouveau).toLocaleString()}€</p>
              </div>
            </div>
            <div className="p-2 bg-yellow-50 rounded text-sm">
              <p>Surcoût employeur : <b className="text-orange-600">+{Math.round(coutSupplementaireEmployeur)}€/mois</b></p>
              <p className="text-xs text-gray-500">soit +{Math.round(coutSupplementaireEmployeur * 12).toLocaleString()}€/an</p>
            </div>
            {beneficieExonerations && (
              <div className="p-2 bg-green-50 rounded text-xs">
                <p className="font-medium text-green-700">✅ Réduction Fillon applicable</p>
                <p className="text-gray-600">Salaire ≤ 1.6 SMIC : exonérations patronales</p>
              </div>
            )}
            {!beneficieExonerations && (
              <div className="p-2 bg-gray-50 rounded text-xs">
                <p className="text-gray-600">Salaire &gt; 1.6 SMIC : pas d'exonération Fillon</p>
              </div>
            )}
          </div>
        </Card>

        <Card title="💡 Arguments pour la négociation">
          <div className="space-y-2 text-sm">
            <div className="p-2 bg-red-50 rounded">
              <p className="font-medium text-red-800">📈 Inflation cumulée 2022-2024</p>
              <p className="text-red-700">+12% sur 3 ans → rattrapage nécessaire</p>
            </div>
            <div className="p-2 bg-blue-50 rounded">
              <p className="font-medium text-blue-800">⚖️ Partage de la valeur ajoutée</p>
              <p className="text-blue-700">Taux de marge 32.5% → marges disponibles</p>
            </div>
            <div className="p-2 bg-green-50 rounded">
              <p className="font-medium text-green-800">🇪🇺 Comparaison européenne</p>
              <p className="text-green-700">SMIC France -20% vs Allemagne</p>
            </div>
            <div className="p-2 bg-purple-50 rounded">
              <p className="font-medium text-purple-800">🎯 Pour cette négociation</p>
              <p className="text-purple-700">
                +{augmentation}% = +{Math.round(gainReelMensuel)}€ net réel/mois
                {pertePrime > 0 && ` (après perte prime)`}
              </p>
            </div>
          </div>
        </Card>
      ...
      <p className="text-xs text-gray-400 text-center">
        ⚠️ Simulation indicative basée sur les barèmes 2025. Prime d'activité : consultez caf.fr pour un calcul exact.
        Taux cotisations moyens (hors mutuelle/prévoyance entreprise).
      </p>
    </div>
  );
}
