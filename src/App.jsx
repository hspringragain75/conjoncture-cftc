...
function SparklineBars({ data, color }) {
  ...
  const bars = values.slice(-12); // max 12 barres
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: '2px', height: '28px', width: '100%' }}>
      {bars.map((v, i) => {
        const h = Math.max(15, Math.round(((v - min) / range) * 100));
...
function buildCatalogue(d) {
  ...
  return [
    ...
    { id: 'smic_net', categorie: '💵 Salaires', label: 'SMIC net',
      ...
      getSparkline: d => {
        // Historique SMIC net mensuel depuis 2018
        const historique = [1185, 1204, 1219, 1231, 1258, 1302, 1329, 1353, 1383, 1399, 1427, 1437, 1443];
        const actuel = d.indicateurs_cles?.smic_net;
        if (actuel && historique[historique.length - 1] !== actuel) historique.push(actuel);
        return historique;
      },
      getStatus: () => 'neutral',
    ...
    { id: 'teletravail', categorie: '⚙️ Travail', label: 'Télétravail',
      ...
      getSparkline: d => {
        // Données statiques reconstituées sur 8 ans
        const base = [15, 17, 28, 27, 26, 26, 26, d.temps_travail?.teletravail?.taux_salaries].filter(v => v != null);
    ...
    { id: 'temps_partiel', categorie: '⚙️ Travail', label: 'Temps partiel',
      ...
      getSparkline: d => {
        const v = d.temps_travail?.temps_partiel?.taux_global_pct;
        // Tendance légèrement décroissante sur 8 ans
        return v ? [18.6, 18.4, 18.2, 18.1, 17.8, 17.5, 17.4, v] : null;
      },
      getStatus: () => 'neutral',
    ...
    { id: 'taux_effort_logement', categorie: '💰 Pouvoir d\'achat', label: 'Effort logement',
      ...
      getSparkline: d => d.taux_effort?.evolution?.map(e => e.locataires_libre) || [23.8, 25.2, 28.6, 28.6, 29.6],
      getStatus: () => 'warning',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '10px' }}>
          {indicsFavoris.map(ind => {
            ...
            return (
              ...
              >
                {/* Poignée drag */}
                <div className="show-on-hover" style={{ position:'absolute', top:'6px', left:'6px', zIndex:10, fontSize:'10px', color: darkMode?'#4b5563':'#d1d5db', opacity:0, transition:'opacity 0.15s', pointerEvents:'none', userSelect:'none' }}>⠿</div>
                ...
                {/* Boutons survol : ⚙ config + ★ retirer */}
                <div className="show-on-hover" style={{ position:'absolute', top:'5px', right:'6px', zIndex:10, display:'flex', gap:'4px', opacity:0, transition:'opacity 0.15s' }}>
                  <button onClick={e => { e.stopPropagation(); ouvrirPopover(ind); }} title="Configurer"
                    style={{ fontSize:'11px', background:'none', border:'none', cursor:'pointer', padding:'2px', color: darkMode?'#6b7280':'#9ca3af' }}>⚙</button>
                  <button onClick={e => { e.stopPropagation(); toggleFavori(ind.id); }} title="Retirer"
                    style={{ fontSize:'11px', background:'none', border:'none', cursor:'pointer', padding:'2px', color: darkMode?'#eab308':'#ca8a04' }}>★</button>
                </div>

                {/* Badge seuil configuré mais pas déclenché */}
                {seuil && !enAlerte && (
                  <div style={{ position:'absolute', bottom:'6px', right:'6px', fontSize:'9px', color: darkMode?'#4b5563':'#d1d5db' }}>
