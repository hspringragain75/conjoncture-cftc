...
function PouvoirAchatTab({d, darkMode}) {
  ...
  // Récupérer les données du panier depuis data.json (ou fallback sur données par défaut)
  const panierData = d.panier_familial || { lastUpdate: "Janvier 2026", produits: [] };
  const heuresData = d.heures_travail || { 
    smic_horaire_net: { 2026: 9.65, 2025: 9.40, 2024: 9.22, 2022: 8.58, 2020: 8.03, 2018: 7.83, 2015: 7.58, 2012: 7.26, 2010: 6.96 },
    produits: []
  };

  const anneeActuelle = 2026;
  const anneePrecedente = anneeActuelle - 5;
  ...
  const calculerPanier = (annee, type) => {
    ...
    return panierData.produits.reduce((total, p) => {
      const prix = p.prix?.[annee] || p.prix?.[anneeActuelle] || 0;
  ...
  const panierAnneeRef = calculerPanier(anneePrecedente, familleType);
  const evolutionPct = panierAnneeRef > 0 ? ((calculerPanier(anneeActuelle, familleType) / panierAnneeRef - 1) * 100).toFixed(0) : "0";

  const smicHoraireNet = heuresData.smic_horaire_net || {};
      ...
      {subTab === 'panier' && <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <div className={`p-4 rounded-xl text-center ${darkMode ? 'bg-blue-900/30 border border-blue-700' : 'bg-blue-50 border border-blue-200'}`}>
            <p className={`text-xs ${darkMode ? 'text-blue-300' : 'text-blue-600'}`}>Évolution 5 ans</p>
