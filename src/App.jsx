...
function ConjonctureTab({d, subTab, setSubTab, darkMode}) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* MARCHES FINANCIERS */}
      {subTab === 'marches' && d.marches_financiers && <div className="space-y-4">
          ...
          <div className={`p-4 rounded-2xl border ${darkMode ? 'bg-purple-900/20 border-purple-800/50' : 'bg-purple-50 border-purple-200'}`}>
            <div className="flex items-center justify-between mb-1">
              ...
              <span className={`text-[10px] px-2 py-0.5 rounded-lg ${darkMode ? 'bg-purple-800/50 text-purple-300' : 'bg-purple-100 text-purple-600'}`}>
                {d.marches_financiers.oat_10ans.source}
              </span>
            </div>
            <div className="flex items-baseline gap-2">
              <span className={`text-2xl font-bold ${darkMode ? 'text-purple-400' : 'text-purple-700'}`}>
                {d.marches_financiers.oat_10ans.valeur}%
              </span>
              <span className={`text-xs ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>
                {d.marches_financiers.oat_10ans.date}
              </span>
            </div>
          </div>
          <div className={`p-4 rounded-2xl border ${darkMode ? 'bg-amber-900/20 border-amber-800/50' : 'bg-amber-50 border-amber-200'}`}>
            <div className="flex items-center justify-between mb-1">
              ...
              <span className={`text-[10px] px-2 py-0.5 rounded-lg ${darkMode ? 'bg-amber-800/50 text-amber-300' : 'bg-amber-100 text-amber-600'}`}>
                {d.marches_financiers.petrole_brent.source}
              </span>
            </div>
            <div className="flex items-baseline gap-2">
              <span className={`text-2xl font-bold ${darkMode ? 'text-amber-400' : 'text-amber-700'}`}>
                {d.marches_financiers.petrole_brent.valeur} $
              </span>
              <span className={`text-xs ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>
                USD/baril {d.marches_financiers.petrole_brent.date}
              </span>
            </div>
          </div>
          <div className={`p-4 rounded-2xl border ${darkMode ? 'bg-blue-900/20 border-blue-800/50' : 'bg-blue-50 border-blue-200'}`}>
            <div className="flex items-center justify-between mb-1">
              ...
              <span className={`text-[10px] px-2 py-0.5 rounded-lg ${
                d.marches_financiers.cac40.variation_jour_pct >= 0
                  ? (darkMode ? 'bg-green-800/50 text-green-300' : 'bg-green-100 text-green-600')
                  : (darkMode ? 'bg-red-800/50 text-red-300' : 'bg-red-100 text-red-600')
              }`}>
                {d.marches_financiers.cac40.variation_jour_pct >= 0 ? '+' : ''}{d.marches_financiers.cac40.variation_jour_pct}%
              </span>
            </div>
            <div className="flex items-baseline gap-2">
              <span className={`text-2xl font-bold ${darkMode ? 'text-blue-400' : 'text-blue-700'}`}>
                {d.marches_financiers.cac40.valeur?.toLocaleString('fr-FR')}
              </span>
              <span className={`text-xs ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>
                pts {d.marches_financiers.cac40.date}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
          <Card title="OAT 10 ans - Taux moyen annuel (%)" darkMode={darkMode}>
            <ResponsiveContainer width="100%" height={180}>
              <ComposedChart data={d.marches_financiers.oat_10ans.evolution_annuelle}>
                <CartesianGrid {...chartProps.cartesianGrid} />
                <XAxis dataKey="annee" {...chartProps.xAxis} />
                ...
                <Line strokeLinecap="round" strokeLinejoin="round" dataKey="valeur" stroke={C.purple} strokeWidth={2.5} dot={{ r: 4, fill: C.purple }} name="OAT 10 ans" />
              </ComposedChart>
            </ResponsiveContainer>
          </Card>

          <Card title="Prix du Brent (USD/baril)" darkMode={darkMode}>
            <ResponsiveContainer width="100%" height={180}>
              <ComposedChart data={d.marches_financiers.petrole_brent.historique}>
                <CartesianGrid {...chartProps.cartesianGrid} />
                <XAxis dataKey="date" {...chartProps.xAxis} fontSize={8} />
                <YAxis {...chartProps.yAxis} domain={['auto', 'auto']} />
                <Tooltip {...chartProps.tooltip} formatter={v => `${v} $/b`} />
                <Area dataKey="valeur" fill={C.quaternary} fillOpacity={0.15} stroke="none" />
                <Line strokeLinecap="round" strokeLinejoin="round" dataKey="valeur" stroke={C.quaternary} strokeWidth={2.5} dot={false} name="Brent" />
              </ComposedChart>
            </ResponsiveContainer>
          </Card>

          <Card title="CAC 40 (points)" darkMode={darkMode}>
            <ResponsiveContainer width="100%" height={180}>
              <ComposedChart data={d.marches_financiers.cac40.historique}>
                <CartesianGrid {...chartProps.cartesianGrid} />
                <XAxis dataKey="date" {...chartProps.xAxis} fontSize={8} />
                <YAxis {...chartProps.yAxis} domain={['auto', 'auto']} />
                <Tooltip {...chartProps.tooltip} formatter={v => `${typeof v === 'number' ? v.toLocaleString('fr-FR') : v} pts`} />
                <Area dataKey="valeur" fill={C.primary} fillOpacity={0.15} stroke="none" />
                <Line strokeLinecap="round" strokeLinejoin="round" dataKey="valeur" stroke={C.primary} strokeWidth={2.5} dot={false} name="CAC 40" />
              </ComposedChart>
            </ResponsiveContainer>
          </Card>
            ...
            <div className="space-y-3 p-1">
              <div className={`p-3 rounded-xl ${darkMode ? 'bg-purple-900/20' : 'bg-purple-50'}`}>
                ...
                <p className={`text-xs ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                  {d.marches_financiers.oat_10ans.note_lecture}
                </p>
              </div>
              <div className={`p-3 rounded-xl ${darkMode ? 'bg-amber-900/20' : 'bg-amber-50'}`}>
                ...
                <p className={`text-xs ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                  {d.marches_financiers.petrole_brent.note_lecture}
                </p>
              </div>
              <div className={`p-3 rounded-xl ${darkMode ? 'bg-blue-900/20' : 'bg-blue-50'}`}>
                ...
                <p className={`text-xs ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                  {d.marches_financiers.cac40.note_lecture}
                </p>
              </div>
            </div>
          </Card>
        ...
        <div className={`${darkMode ? 'bg-indigo-900/30' : 'bg-indigo-50'} border-l-4 border-indigo-400 p-4 rounded`}>
          <h3 className={`font-semibold ${darkMode ? 'text-indigo-300' : 'text-indigo-800'}`}>Pourquoi suivre les march&eacute;s financiers ?</h3>
          <ul className={`mt-2 text-sm ${darkMode ? 'text-indigo-200' : 'text-indigo-700'} space-y-1`}>
            <li>{"•"} <b>OAT 10 ans</b> : influence les taux de cr&eacute;dit immobilier et le co&ucirc;t de la dette publique</li>
            <li>{"•"} <b>P&eacute;trole Brent</b> : impacte directement les prix des carburants et de l&apos;&eacute;nergie (inflation)</li>
            <li>{"•"} <b>CAC 40</b> : refl&egrave;te la sant&eacute; des grandes entreprises, impacte int&eacute;ressement et participation</li>
          </ul>
        </div>
      </div>}
