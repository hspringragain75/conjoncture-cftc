...
function EvolutionsTab({d, darkMode}) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        {[['graphiques','📊 Graphiques'],['timeline','⏳ Timeline interactive']].map(([id,label]) => (
...
function GlobeTab({ darkMode }) {
  ...
  const rotationVelocity = React.useRef({ x: 0, y: 0 });

  const [indicateur, setIndicateur] = React.useState('chomage');
  const [pays, setPays] = React.useState(null);
  const [loaded, setLoaded] = React.useState(false);
  const countryMeshes = React.useRef([]);

  // ── Données mondiales ────────────────────────────────────────────────────
  const PAYS_DATA = {
    // Europe
    'France':        { chomage: 7.7,  inflation: 0.9,  pib: 1.1,  smic: 1802, gini: 31.5, lat: 46,  lon: 2   },
    'Allemagne':     { chomage: 3.4,  inflation: 2.2,  pib: -0.2, smic: 2054, gini: 31.7, lat: 51,  lon: 10  },
    'Espagne':       { chomage: 11.6, inflation: 2.8,  pib: 3.2,  smic: 1323, gini: 33.0, lat: 40,  lon: -4  },
    'Italie':        { chomage: 6.2,  inflation: 1.1,  pib: 0.7,  smic: null, gini: 34.8, lat: 43,  lon: 12  },
    'Pays-Bas':      { chomage: 3.7,  inflation: 2.7,  pib: 0.9,  smic: 2070, gini: 28.2, lat: 52,  lon: 5   },
    'Belgique':      { chomage: 5.5,  inflation: 3.1,  pib: 1.4,  smic: 2070, gini: 27.2, lat: 50,  lon: 4   },
    'Suède':         { chomage: 8.5,  inflation: 1.7,  pib: 0.5,  smic: null, gini: 27.6, lat: 62,  lon: 16  },
    'Pologne':       { chomage: 2.9,  inflation: 4.7,  pib: 2.9,  smic: 925,  gini: 30.2, lat: 52,  lon: 20  },
    'Portugal':      { chomage: 6.4,  inflation: 2.3,  pib: 2.1,  smic: 1020, gini: 33.5, lat: 39,  lon: -8  },
    'Grèce':         { chomage: 11.0, inflation: 3.0,  pib: 2.3,  smic: 910,  gini: 32.9, lat: 39,  lon: 22  },
    'Autriche':      { chomage: 5.3,  inflation: 2.9,  pib: -1.0, smic: null, gini: 30.8, lat: 47,  lon: 14  },
    'Suisse':        { chomage: 2.6,  inflation: 1.1,  pib: 1.3,  smic: null, gini: 33.5, lat: 47,  lon: 8   },
    'Norvège':       { chomage: 3.9,  inflation: 3.1,  pib: 0.8,  smic: null, gini: 26.1, lat: 62,  lon: 10  },
    'Danemark':      { chomage: 2.6,  inflation: 2.4,  pib: 1.5,  smic: null, gini: 28.3, lat: 56,  lon: 10  },
    'Finlande':      { chomage: 7.5,  inflation: 0.5,  pib: -1.0, smic: null, gini: 27.7, lat: 64,  lon: 26  },
    'Roumanie':      { chomage: 5.6,  inflation: 5.6,  pib: 2.0,  smic: 760,  gini: 34.8, lat: 46,  lon: 25  },
    'Hongrie':       { chomage: 4.1,  inflation: 3.7,  pib: 0.6,  smic: 701,  gini: 28.0, lat: 47,  lon: 19  },
    'République tchèque': { chomage: 2.6, inflation: 2.0, pib: 1.2, smic: 870, gini: 25.3, lat: 50, lon: 15 },
    // Amériques
    'États-Unis':    { chomage: 4.1,  inflation: 2.9,  pib: 2.8,  smic: 1160, gini: 41.5, lat: 38,  lon: -97 },
    'Canada':        { chomage: 6.8,  inflation: 1.8,  pib: 1.3,  smic: 1580, gini: 33.3, lat: 56,  lon: -96 },
    'Brésil':        { chomage: 6.2,  inflation: 4.8,  pib: 3.1,  smic: 325,  gini: 52.0, lat: -10, lon: -53 },
    'Mexique':       { chomage: 2.8,  inflation: 4.2,  pib: 1.5,  smic: 270,  gini: 45.4, lat: 23,  lon: -102 },
    'Argentine':     { chomage: 7.0,  inflation: 118.0,pib: -1.7, smic: 320,  gini: 42.0, lat: -34, lon: -64 },
    'Chili':         { chomage: 8.6,  inflation: 4.5,  pib: 2.3,  smic: 540,  gini: 44.9, lat: -35, lon: -71 },
    'Colombie':      { chomage: 9.5,  inflation: 5.2,  pib: 1.7,  smic: 310,  gini: 54.8, lat: 4,   lon: -74 },
    // Asie
    'Chine':         { chomage: 5.0,  inflation: 0.3,  pib: 5.0,  smic: 360,  gini: 38.2, lat: 35,  lon: 105 },
    'Japon':         { chomage: 2.5,  inflation: 2.8,  pib: 0.1,  smic: 1020, gini: 32.9, lat: 36,  lon: 138 },
    'Corée du Sud':  { chomage: 3.0,  inflation: 2.3,  pib: 2.1,  smic: 1480, gini: 31.4, lat: 37,  lon: 128 },
    'Inde':          { chomage: 7.8,  inflation: 4.9,  pib: 6.8,  smic: 120,  gini: 35.7, lat: 20,  lon: 78  },
    'Indonésie':     { chomage: 4.8,  inflation: 2.8,  pib: 5.1,  smic: 230,  gini: 38.2, lat: -5,  lon: 120 },
    'Turquie':       { chomage: 8.5,  inflation: 47.0, pib: 3.2,  smic: 630,  gini: 41.9, lat: 39,  lon: 35  },
    'Arabie Saoudite':{ chomage: 7.7, inflation: 1.5,  pib: 0.8,  smic: null, gini: null, lat: 24,  lon: 45  },
    'Vietnam':       { chomage: 2.1,  inflation: 3.6,  pib: 7.1,  smic: 200,  gini: 36.1, lat: 16,  lon: 108 },
    'Thaïlande':     { chomage: 1.1,  inflation: 0.4,  pib: 2.7,  smic: 310,  gini: 36.4, lat: 15,  lon: 101 },
    // Afrique
    'Afrique du Sud':{ chomage: 33.5, inflation: 4.4,  pib: 0.8,  smic: 310,  gini: 63.0, lat: -29, lon: 25  },
    'Nigeria':       { chomage: 5.0,  inflation: 28.0, pib: 3.2,  smic: 55,   gini: 35.1, lat: 10,  lon: 8   },
    'Maroc':         { chomage: 13.0, inflation: 0.9,  pib: 3.6,  smic: 310,  gini: 39.5, lat: 32,  lon: -6  },
    'Égypte':        { chomage: 6.7,  inflation: 26.0, pib: 4.2,  smic: 115,  gini: 31.5, lat: 27,  lon: 30  },
    'Éthiopie':      { chomage: 3.5,  inflation: 20.0, pib: 7.1,  smic: null, gini: 35.0, lat: 8,   lon: 38  },
    // Océanie
    'Australie':     { chomage: 4.0,  inflation: 2.4,  pib: 1.5,  smic: 2070, gini: 34.3, lat: -25, lon: 134 },
    'Nouvelle-Zélande':{ chomage: 4.7, inflation: 2.2, pib: 0.5,  smic: 1790, gini: 33.9, lat: -41, lon: 174 },
    // Russie & ex-URSS
    'Russie':        { chomage: 2.4,  inflation: 8.5,  pib: 3.6,  smic: 220,  gini: 36.0, lat: 62,  lon: 90  },
    'Ukraine':       { chomage: 18.0, inflation: 12.0, pib: 4.5,  smic: 210,  gini: 26.6, lat: 49,  lon: 32  },
  };

  const INDICATEURS = [
    { id: 'chomage',   label: 'Taux de chômage (%)',  unite: '%',  low_is_good: true,  palette: ['#22c55e','#86efac','#fde68a','#f97316','#dc2626'] },
    { id: 'inflation', label: 'Inflation (%)',         unite: '%',  low_is_good: true,  palette: ['#22c55e','#86efac','#fde68a','#f97316','#dc2626'] },
    { id: 'pib',       label: 'Croissance PIB (%)',    unite: '%',  low_is_good: false, palette: ['#dc2626','#f97316','#fde68a','#86efac','#22c55e'] },
    { id: 'smic',      label: 'Salaire minimum (€)',   unite: '€',  low_is_good: false, palette: ['#dc2626','#f97316','#fde68a','#86efac','#22c55e'] },
    { id: 'gini',      label: 'Indice Gini (inégalités)', unite: '', low_is_good: true, palette: ['#22c55e','#86efac','#fde68a','#f97316','#dc2626'] },
  ];

  const indDef = INDICATEURS.find(i => i.id === indicateur);

  // Couleur selon valeur
  const getColor = (val, indId) => {
    if (val === null || val === undefined) return '#6b7280';
    const vals = Object.values(PAYS_DATA).map(p => p[indId]).filter(v => v !== null && v !== undefined);
    ...
    const c0 = hex2rgb(p[i0]), c1 = hex2rgb(p[i1]);
  ...
  // lat/lon → position 3D alignée avec texture NASA Blue Marble
  const latLonTo3D = (lat, lon, r = 1.01) => {
  ...
  // ── Init Three.js ────────────────────────────────────────────────────────
  useEffect(() => {
    // Charger Three.js dynamiquement si pas déjà chargé
    const loadThree = () => {
    ...
    // Globe avec texture Terre
    const geo = new THREE.SphereGeometry(1, 64, 64);
    const loader = new THREE.TextureLoader();
    // Texture NASA Blue Marble (domaine public)
    const earthTexture = loader.load(
      'https://unpkg.com/three-globe/example/img/earth-blue-marble.jpg',
      () => { if (rendererRef.current) rendererRef.current.render(sceneRef.current, cameraRef.current); },
      undefined,
      // Fallback si texture indisponible : couleur unie
      () => { globe.material.color.set(0x1a4a7a); }
    );
  ...
  useEffect(() => {
    const THREE = window.THREE;
    if (!THREE || !globeRef.current || !loaded) return;

    // Nettoyer les anciens marqueurs (enfants du globe seulement)
    countryMeshes.current.forEach(m => globeRef.current.remove(m));
    countryMeshes.current = [];

    Object.entries(PAYS_DATA).forEach(([nom, data]) => {
      const val = data[indicateur];
      const color = getColor(val, indicateur);
      const [x, y, z] = latLonTo3D(data.lat, data.lon, 1.05);
      ...
      mesh.userData = { nom, data, val };

      // Ajouter comme enfant du globe → tourne avec lui automatiquement
      globeRef.current.add(mesh);
      countryMeshes.current.push(mesh);
    });
  }, [indicateur, loaded]);
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (!isDragging.current) {
        ...
        countryMeshes.current.forEach(m => {
          m.position.applyAxisAngle(new THREE.Vector3(0,1,0), 0.0015);
        });
        // Amortissement de la vélocité
  ...
  const handleMouseMove = (e) => {
    ...
    globeRef.current.rotateOnWorldAxis(axis, angle);
    countryMeshes.current.forEach(m => {
  ...
  const handleMouseUp = (e) => {
    ...
    // Raycasting pour détecter le clic sur un pays
    const THREE = window.THREE;
    const canvas = canvasRef.current;
    ...
    const hits = raycaster.intersectObjects(countryMeshes.current);
    if (hits.length > 0) setPays(hits[0].object.userData);
  };

  // ── Légende gradient ──────────────────────────────────────────────────────
  const LegendGradient = () => {
    const vals = Object.values(PAYS_DATA).map(p => p[indicateur]).filter(v => v !== null && v !== undefined);
    const min = Math.min(...vals), max = Math.max(...vals);
    const pal = indDef.palette;
    return (
      <div className="flex flex.col gap-1">
        <div className="flex items-center gap-2">
          <span className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
            {indDef.low_is_good ? '✅ Bas' : '❌ Bas'}
          </span>
          <div className="flex-1 h-3 rounded-full overflow-hidden" style={{
            background: `linear-gradient(to right, ${pal.join(',')})`
          }} />
          <span className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
            {indDef.low_is_good ? '❌ Élevé' : '✅ Élevé'}
          </span>
        </div>
        <div className="flex justify-between">
          <span className={`text-[10px] ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>{min.toFixed(1)}{indDef.unite}</span>
          <span className={`text-[10px] ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>{max.toFixed(1)}{indDef.unite}</span>
        </div>
      </div>
    );
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={`p-5 rounded-2xl ${darkMode ? 'bg-gradient-to-r from-blue-900 to-indigo-900' : 'bg-gradient-to-r from-blue-600 to-indigo-600'} text-white`}>
        ...
        <p className="text-sm opacity-80 mt-1">Explorez les indicateurs économiques pays par pays · Cliquez sur un point pour les détails</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {/* Globe canvas */}
        <div className={`lg:col-span-2 rounded-2xl overflow-hidden relative ${darkMode ? 'bg-gray-950' : 'bg-gray-900'}`}
          style={{ height: '480px' }}>
          ...
          {!loaded && (
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-white text-sm opacity-60">Chargement du globe...</div>
            </div>
          )}

          {/* Hint */}
          <div className="absolute bottom-3 left-0 right-0 flex justify-center">
            <span className="text-white/40 text-xs">🖱️ Glisser pour tourner · Cliquer sur un point pour les détails</span>
          </div>
        </div>
        ...
        <div className="space-y-3">
          {/* Légende */}
          <div className={`rounded-2xl p-4 ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow`}>
            <p className={`text-xs font-bold mb-3 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
              {indDef.label}
            </p>
            <LegendGradient />
          </div>
          ...
          {pays ? (
            <div className={`rounded-2xl p-4 ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow`}>
              <h3 className={`font-black text-lg mb-3 ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                {pays.nom}
              </h3>
              <div className="space-y-2">
                {INDICATEURS.map(ind => {
                  ...
                  return (
                    <div key={ind.id} className={`flex justify-between items-center py-1.5 border-b ${darkMode ? 'border-gray-700' : 'border-gray-100'}`}>
            ...
            <div className={`rounded-2xl p-6 text-center ${darkMode ? 'bg-gray-800 text-gray-500' : 'bg-white text-gray-400'} shadow`}>
              ...
              <p className="text-sm">Cliquez sur un point du globe pour voir les détails d'un pays</p>
            </div>
          )}
          ...
          <div className={`rounded-2xl p-4 ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow`}>
            <p className={`text-xs font-bold mb-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
              🏆 Top 5 — {indDef.label.split(' (')[0]}
            </p>
            {Object.entries(PAYS_DATA)
              .filter(([, d]) => d[indicateur] !== null && d[indicateur] !== undefined)
              .sort(([,a],[,b]) => indDef.low_is_good ? a[indicateur]-b[indicateur] : b[indicateur]-a[indicateur])
              .slice(0, 5)
              .map(([nom, data], i) => (
                <div key={nom} className={`flex items-center justify-between py-1 ${i < 4 ? (darkMode ? 'border-b border-gray-700' : 'border-b border-gray-100') : ''}`}>
                  <div className="flex items-center gap-2">
      ...
      <p className={`text-xs text-center ${darkMode ? 'text-gray-600' : 'text-gray-400'}`}>
        📚 Sources : FMI, OCDE, Eurostat, Banque mondiale — Données 2024-2025 · {Object.keys(PAYS_DATA).length} pays
      </p>
    </div>
  );
}
