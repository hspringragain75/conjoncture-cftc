...
function BubbleKpi({ label, value, status, darkMode, tooltip, sparklineData, invertTrend = false }) {
  ...
  return (
    ...
    >
      {/* Label */}
      <span style={{ fontSize: '11px', color: darkMode ? '#9ca3af' : '#6b7280', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
        {label}
      </span>

      {/* Valeur + badge tendance */}
      <div style={{ display: 'flex', alignItems: 'baseline', gap: '6px', flexWrap: 'wrap' }}>
        <span style={{ fontSize: '20px', fontWeight: 600, color: darkMode ? '#f9fafb' : '#111827', lineHeight: 1, letterSpacing: '-0.02em' }}>
          {value}
        </span>
        <span style={{
          fontSize: '10px', fontWeight: 600,
          padding: '1px 5px', borderRadius: '99px',
          background: trendBg, color: trendText,
          flexShrink: 0,
        }}>
          {trendLabel}
        </span>
...
function FavorisTab({ d, darkMode, favoris, toggleFavori, isFavori }) {
  ...
  // Ordre drag & drop — initialisé depuis favoris, persisté séparément
  ...
  // Ordre effectif : favoris triés selon l'ordre mémorisé, nouveaux favoris en fin
  ...
  const indicsFavoris = ordreEffectif
    .map(id => catalogue.find(ind => ind.id === id))
    .filter(Boolean);
  ...
  // Résumé auto
  const nbHausse  = indicsFavoris.filter(ind => {
    const sp = ind.getSparkline?.(d);
    if (!sp || sp.length < 2) return false;
    const vals = sp.filter(v => v != null);
    return vals.length >= 2 && vals[vals.length-1] > vals[vals.length-2];
  }).length;
  const nbBaisse  = indicsFavoris.filter(ind => {
    const sp = ind.getSparkline?.(d);
    if (!sp || sp.length < 2) return false;
    const vals = sp.filter(v => v != null);
    return vals.length >= 2 && vals[vals.length-1] < vals[vals.length-2];
  }).length;
  const nbAlertes = indicsFavoris.filter(ind => ind.getStatus(d) === 'bad').length;
    ...
    <div className="space-y-3">
      ...
      <div className={`flex items-center justify-between px-4 py-3 rounded-2xl ${darkMode ? 'bg-gray-800' : 'bg-white shadow-sm'}`}>
        ...
        <div className="flex items-center gap-2">
          {indicsFavoris.length > 1 && (
            <span className={`text-[10px] hidden sm:block ${darkMode ? 'text-gray-600' : 'text-gray-400'}`}>
      ...
      {/* Grille cockpit — pleine largeur, toutes les cartes au même niveau */}
      {indicsFavoris.length > 0 && (
        <div style={{
          display: 'grid',
          ...
          {indicsFavoris.map(ind => (
            ...
            >
              ...
              <div style={{
                ...
                fontSize: '10px', color: darkMode ? '#4b5563' : '#d1d5db',
              ...
              <button
                ...
                style={{
                  position: 'absolute', top: '5px', right: '6px', zIndex: 10,
                  fontSize: '11px', background: 'none', border: 'none',
                  cursor: 'pointer', padding: '2px',
                  color: darkMode ? '#eab308' : '#ca8a04',
                  opacity: 0, transition: 'opacity 0.15s',
                }}
                className="group-hover:opacity-100"
              >★</button>
