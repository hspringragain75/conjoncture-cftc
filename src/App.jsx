...
function BubbleSubTabs({ tabs, activeTab, setActiveTab, darkMode }) {
  return (
    <div className={`flex flex-wrap gap-1.5 p-2 rounded-2xl ${darkMode ? 'bg-gray-800' : 'bg-white shadow-sm'}`}>
      {tabs.map(([id, label]) => (
        <button 
          key={id} 
          onClick={() => setActiveTab(id)} 
...
const useChartProps = (darkMode) => {
  ...
  return {
    ...
    xAxis: { tick: { fill: colors.axis, fontSize: 10 }, axisLine: { stroke: colors.grid }, tickLine: false },
...
function ConjonctureTab({d, subTab, setSubTab, darkMode}) {
  ...
  return (
    <div className="space-y-4">
      <div className={`flex flex-wrap gap-1 p-2 rounded-lg shadow-lg ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
        {[['pib','Croissance PIB'],['partage_va','⚖️ Partage VA'],['climat','Climat affaires'],['defaillances','Défaillances'],['investissement','Investissement']].map(([id,label])=>(
          <button key={id} onClick={()=>setSubTab(id)} className={`px-2 py-1 rounded-lg text-xs font-medium ${subTab===id?'bg-indigo-600 text-white': darkMode ? 'text-gray-300 hover:bg-gray-700' : 'text-gray-600 hover:bg-gray-100'}`}>{label}</button>
        ))}
      </div>
...
function ComparaisonUETab({d, darkMode}) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-1 bg-white p-2 rounded-lg shadow-lg">
        {[['smic','💰 SMIC'],['chomage','👥 Chômage'],['partage_va','⚖️ Part salaires VA']].map(([id,label])=>(
          <button key={id} onClick={()=>setSubTab(id)} className={`px-2 py-1 rounded-lg text-xs font-medium ${subTab===id?'bg-blue-600 text-white':'text-gray-600 hover:bg-gray-100'}`}>{label}</button>
        ))}
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
          <div className="bg-white rounded-lg p-3">
            ...
            <div className="grid grid-cols-2 gap-2 mt-2">
              <div className="p-2 bg-gray-50 rounded-lg text-center"><p className="text-xs text-gray-500">Actuel</p><p className="font-bold text-green-600">{redActuelle.mens}€/m</p><p className="text-xs">{redActuelle.an.toLocaleString()}€/an</p></div>
              <div className="p-2 bg-gray-50 rounded-lg text-center"><p className="text-xs text-gray-500">Après</p><p className={`font-bold ${redNouvelle.ok ? 'text-green-600' : 'text-red-600'}`}>{redNouvelle.mens}€/m</p><p className="text-xs">{redNouvelle.an.toLocaleString()}€/an</p></div>
            </div>
    ...
    <div className="space-y-4">
      ...
      <div className={`flex flex-wrap gap-1 p-2 rounded-lg shadow-lg ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
        {[['general','📊 Général'],['budget','🧺 Budget UNAF'],['heures','⏱️ Heures de travail']].map(([id,label])=>(
          <button key={id} onClick={()=>setSubTab(id)} className={`px-3 py-1.5 rounded-lg text-sm font-medium ${subTab===id?'bg-orange-600 text-white': darkMode ? 'text-gray-300 hover:bg-gray-700' : 'text-gray-600 hover:bg-gray-100'}`}>{label}</button>
        ))}
      </div>
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-1 bg-white p-2 rounded-lg shadow-lg">
        {[['chomage','Chômage'],['seniors','Seniors'],['contrats','Contrats'],['secteurs','Secteurs'],['recrutement','Recrutement'],['dynamique','Dynamique']].map(([id,label])=>(
          <button key={id} onClick={()=>setSubTab(id)} className={`px-2 py-1 rounded-lg text-xs font-medium ${subTab===id?'bg-purple-600 text-white':'text-gray-600 hover:bg-gray-100'}`}>{label}</button>
        ))}
      </div>
          ...
          <Card title="📉 Taux de chômage 50+ (%)" darkMode={darkMode}>
            <ResponsiveContainer width="100%" height={200}>
              <ComposedChart data={d.chomage_seniors}>
                ...
                <Line dataKey="taux" stroke={C.secondary} strokeWidth={3} dot={{ fill: C.secondary, strokeWidth: 2 }} />
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-1 bg-white p-2 rounded-lg shadow-lg">
        {[['loyers','Loyers (IRL)'],['immobilier','Prix immobilier'],['carburants','Carburants'],['effort','Taux d\'effort']].map(([id,label])=>(
          <button key={id} onClick={()=>setSubTab(id)} className={`px-2 py-1 rounded-lg text-xs font-medium ${subTab===id?'bg-cyan-600 text-white':'text-gray-600 hover:bg-gray-100'}`}>{label}</button>
        ))}
      </div>
    ...
    <div className="space-y-4">
      ...
      <div className="grid md:grid-cols-2 gap-3">
        {branches.map((branche, idx) => {
          ...
          return (
            <div key={idx} className={`bg-white rounded-xl border-2 ${branche.statut === 'conforme' ? 'border-green-200' : 'border-red-300'} overflow-hidden`}>
              <div 
              ...
              >
                <div className="flex justify-between items-start">
                  ...
                  <span className={`px-2 py-1 rounded-lg text-xs ${branche.statut === 'conforme' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
...
function EvolutionsTab({d, darkMode}) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={`flex flex-wrap gap-2 p-2 rounded-lg ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow`}>
        {[
          ...
          <button 
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${graphType === id ? 'bg-indigo-600 text-white' : darkMode ? 'text-gray-300 hover:bg-gray-700' : 'text-gray-600 hover:bg-gray-100'}`}
          >
            {label}
          </button>
        ))}
...
function TerritoiresTab({d, darkMode}) {
  ...
  const [metric, setMetric] = useState('chomage'); // chomage, salaire, tensions
  ...
  const getColor = (region, metricType, isDark) => {
    ...
    if (metricType === 'chomage') {
      // Plus c'est bas, mieux c'est (vert)
      ...
      // Plus c'est haut, mieux c'est (vert)
  ...
  // Positions approximatives des régions sur la carte (en pourcentage)
  const regionPositions = {
    'IDF': { x: 52, y: 28, size: 1.0 },
    'HDF': { x: 52, y: 12, size: 1.1 },
    'GES': { x: 72, y: 25, size: 1.2 },
    'NOR': { x: 32, y: 18, size: 1.0 },
    'BRE': { x: 12, y: 28, size: 1.0 },
    'PDL': { x: 22, y: 40, size: 1.0 },
    'CVL': { x: 42, y: 42, size: 1.1 },
    'BFC': { x: 65, y: 45, size: 1.1 },
    'NAQ': { x: 28, y: 62, size: 1.3 },
    'ARA': { x: 65, y: 62, size: 1.2 },
    'OCC': { x: 42, y: 78, size: 1.2 },
    'PAC': { x: 72, y: 80, size: 1.1 },
    'COR': { x: 88, y: 88, size: 0.7 },
  };
  
  // Trouver les extrêmes pour la légende
          ...
          <div className="flex-1 p-4">
            <div className="relative w-full" style={{ paddingBottom: '100%', maxWidth: '500px', margin: '0 auto' }}>
              <div className="absolute inset-0">
                {/* Fond décoratif */}
                <div className={`absolute inset-4 rounded-full opacity-5 ${darkMode ? 'bg-blue-400' : 'bg-blue-600'}`}></div>
                
                {/* Régions comme bulles */}
                {regions.map((region) => {
                  const pos = regionPositions[region.code] || { x: 50, y: 50, size: 1 };
                  const isSelected = selectedRegion?.code === region.code;
                  const isHovered = hoveredRegion?.code === region.code;
                  const color = getColor(region, metric, darkMode);
                  const baseSize = 45 * pos.size;
                  const size = isSelected ? baseSize * 1.15 : isHovered ? baseSize * 1.08 : baseSize;
                    ...
                    >
                      ...
                      <div
                        ...
                        style={{
                          ...
                          boxShadow: isSelected 
                            ? `0 0 0 4px ${darkMode ? 'rgba(255,255,255,0.3)' : 'rgba(0,0,0,0.2)'}, 0 8px 25px rgba(0,0,0,0.3)`
                            : isHovered 
                      ...
                      >
                        <div className="text-center px-1">
                          <div className="text-white font-bold text-xs leading-tight drop-shadow">
                            {region.code}
                          </div>
                          <div className="text-white/90 font-semibold text-[10px] drop-shadow">
                            {getMetricValue(region, metric)}
                          </div>
                        </div>
                      </div>
                      
                      {/* Label au survol/sélection */}
                      {(isSelected || isHovered) && (
                        <div 
                          className={`absolute left-1/2 -translate-x-1/2 whitespace-nowrap px-2 py-1 rounded text-xs font-semibold shadow-lg transition-all duration-200 ${
            ...
            {/* Légende */}
            <div className={`flex justify-center gap-4 mt-4 text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
              {metric === 'chomage' && (
                <>
                  <span className="flex items-center gap-1">
                    <span className="w-3 h-3 rounded-full bg-green-500"></span> &lt;6.5%
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="w-3 h-3 rounded-full bg-yellow-500"></span> 6.5-7.5%
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="w-3 h-3 rounded-full bg-orange-500"></span> 7.5-8.5%
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="w-3 h-3 rounded-full bg-red-500"></span> &gt;8.5%
                  </span>
                </>
              )}
              {metric === 'salaire' && (
                <>
                  <span className="flex items-center gap-1">
                    <span className="w-3 h-3 rounded-full bg-green-500"></span> ≥2200€
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="w-3 h-3 rounded-full bg-blue-500"></span> 2050-2200€
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="w-3 h-3 rounded-full bg-yellow-500"></span> 1980-2050€
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="w-3 h-3 rounded-full bg-orange-500"></span> &lt;1980€
                  </span>
                </>
              )}
              {metric === 'tensions' && (
                <>
                  <span className="flex items-center gap-1">
                    <span className="w-3 h-3 rounded-full bg-blue-500"></span> &lt;60%
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="w-3 h-3 rounded-full bg-yellow-500"></span> 60-68%
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="w-3 h-3 rounded-full bg-orange-500"></span> &gt;68%
                  </span>
                </>
              )}
            </div>
      ...
      <div className="flex gap-2 flex-wrap">
        {[['glossaire', '📚 Glossaire'], ['indicateurs', '📊 Indicateurs'], ['sources', '🔗 Sources'], ['methodologie', '🔬 Méthodologie']].map(([id, label]) => (
          <button 
            ...
            className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${openSection === id ? 'bg-purple-600 text-white' : darkMode ? 'bg-gray-800 text-gray-300 hover:bg-gray-700' : 'bg-white border text-gray-600 hover:bg-gray-50'}`}
          >
            {label}
          </button>
        ))}
