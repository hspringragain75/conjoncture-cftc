          ...
          <div className={`fixed top-0 right-0 h-full w-full sm:w-[420px] z-50 flex flex-col shadow-2xl ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
              {!d?.revue_presse ? (
                ...
                const rp = d.revue_presse;
                const [filtre, setFiltre] = React.useState('une'); // 'une' | 'tous'
                const [sourceFiltre, setSourceFiltre] = React.useState('Toutes');
                  ...
                  <>
                    ...
                    <div className="flex gap-1.5 flex-wrap">
                      {[['une','⭐ À la une'],['tous','📋 Tous']].map(([id, label]) => (
                        <button key={id} onClick={() => setFiltre(id)}
                          className={`px-3 py-1 rounded-xl text-xs font-medium transition-all ${filtre === id ? 'bg-blue-600 text-white' : darkMode ? 'bg-gray-800 text-gray-400 hover:bg-gray-700' : 'bg-white border border-gray-200 text-gray-500 hover:bg-gray-50'}`}>
                          {label}
                        </button>
                      ))}
                    ...
                    {articlesFiltres.length === 0 ? (
                      ...
                      <a key={i} href={a.lien} target="_blank" rel="noopener noreferrer"
                        className={`flex gap-3 p-3.5 rounded-2xl border transition-all hover:shadow-md group ${darkMode ? 'bg-gray-800 border-gray-700 hover:border-gray-500' : 'bg-white border-gray-200 hover:border-gray-300'}`}>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-1.5 mt-1.5">
                            <div className="flex gap-0.5">
                              {[...Array(5)].map((_, j) => (
                                <div key={j} className={`w-1.5 h-1.5 rounded-full ${j < Math.ceil(a.pertinence / 2) ? 'bg-blue-500' : darkMode ? 'bg-gray-600' : 'bg-gray-200'}`} />
                              ))}
                            </div>
