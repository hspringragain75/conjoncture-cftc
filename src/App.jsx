...
// ==================== COMPOSANT SPARKLINE ====================
// Sparkline pleine largeur avec area fill — utilisée dans BubbleKpi
function Sparkline({ data, color, darkMode }) {
  ...
  const py = (v) => H - pad - ((v - min) / range) * (H - 2 * pad - 6);

  const linePoints = values.map((v, i) => `${px(i)},${py(v)}`).join(' ');
  // Area : descend jusqu'au bas pour le fill
  const areaPoints = [
    `${px(0)},${H}`,
    ...values.map((v, i) => `${px(i)},${py(v)}`),
    `${px(values.length - 1)},${H}`
  ].join(' ');
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" style={{display:'block'}}>
      ...
      <polyline fill="none" stroke={color} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" points={linePoints} />
      <circle cx={lastX} cy={lastY} r="3" fill={color} stroke={darkMode ? '#1f2937' : '#fff'} strokeWidth="1.5" />
    </svg>
  );
}
...
function BubbleKpi({ label, value, status, darkMode, tooltip, sparklineData, invertTrend = false }) {
  ...
  const statusColors = {
    good:    { bg: darkMode ? 'bg-green-900/30'  : 'bg-green-50',  border: darkMode ? 'border-green-800/50'  : 'border-green-200',  text: darkMode ? 'text-green-400'  : 'text-green-600',  sparkline: '#22c55e' },
    neutral: { bg: darkMode ? 'bg-blue-900/30'   : 'bg-blue-50',   border: darkMode ? 'border-blue-800/50'   : 'border-blue-200',   text: darkMode ? 'text-blue-400'   : 'text-blue-600',   sparkline: '#3b82f6' },
    warning: { bg: darkMode ? 'bg-orange-900/30' : 'bg-orange-50', border: darkMode ? 'border-orange-800/50' : 'border-orange-200', text: darkMode ? 'text-orange-400' : 'text-orange-600', sparkline: '#f97316' },
    bad:     { bg: darkMode ? 'bg-red-900/30'    : 'bg-red-50',    border: darkMode ? 'border-red-800/50'    : 'border-red-200',    text: darkMode ? 'text-red-400'    : 'text-red-600',    sparkline: '#ef4444' },
  };
  const colors = statusColors[status] || statusColors.neutral;

  const trendIcon = trend === 'up' ? '▲' : trend === 'down' ? '▼' : '—';
  const trendColor = trend === 'up' ? 'text-green-500' : trend === 'down' ? 'text-red-500' : darkMode ? 'text-gray-600' : 'text-gray-300';

  return (
    <div
      className={`relative rounded-2xl border overflow-hidden transition-all duration-300 hover:scale-[1.02] hover:shadow-lg cursor-help ${colors.bg} ${colors.border}`}
    ...
    >
      ...
      <div className="px-3 pt-3 pb-1">
        <div className="flex items-center justify-between mb-1">
          ...
          <span className={`text-[10px] font-semibold ${trendColor}`}>{trendIcon}</span>
        </div>
        <span className={`text-lg sm:text-xl font-bold leading-tight ${colors.text}`}>
          {value}
        </span>
      </div>

      {/* Sparkline pleine largeur en bas */}
      {sparklineData && sparklineData.length >= 2 ? (
        <div className="mt-1" style={{height: '44px'}}>
...
function useFavoris() {
  const [favoris, setFavoris] = useState(() => {
    try {
      const saved = localStorage.getItem('cftc_favoris');
...
// ==================== CATALOGUE DES INDICATEURS FAVORISABLES ====================
// Chaque entrée : { id, label, categorie, getValue(d), getSparkline(d), status(d), tooltip, unite }
function buildCatalogue(d) {
  ...
  return [
    ...
    { id: 'pib_trim', categorie: '📈 Conjoncture', label: 'PIB trimestriel',
      ...
      getSparkline: d => d.sparklines?.pib,
    ...
    { id: 'climat_affaires', categorie: '📈 Conjoncture', label: 'Climat affaires',
      getValue: d => d.indicateurs_cles?.climat_affaires ?? '—',
      getSparkline: d => d.sparklines?.climat,
      getStatus: () => 'neutral',
      tooltip: 'Indice INSEE (100 = moyenne long terme)' },
    { id: 'confiance_menages', categorie: '📈 Conjoncture', label: 'Confiance ménages',
      getValue: d => d.conjoncture?.climat_affaires?.confiance_menages ?? '—',
      getSparkline: d => d.conjoncture?.climat_affaires?.evolution?.map(e => e.menages),
      getStatus: d => (d.conjoncture?.climat_affaires?.confiance_menages||0) >= 100 ? 'good' : 'warning',
    ...
    { id: 'defaillances', categorie: '📈 Conjoncture', label: 'Défaillances',
      ...
      getSparkline: d => d.sparklines?.defaillances,
      getStatus: () => 'bad',
    ...
    { id: 'chomage', categorie: '👥 Emploi', label: 'Chômage',
      ...
      getSparkline: d => d.sparklines?.chomage,
    ...
    { id: 'chomage_jeunes', categorie: '👥 Emploi', label: 'Chômage jeunes',
      ...
      getSparkline: d => d.chomage?.map(c => c.jeunes),
      getStatus: () => 'bad',
    ...
    { id: 'emploi_seniors', categorie: '👥 Emploi', label: 'Emploi seniors',
      ...
      getSparkline: d => d.emploi_seniors?.map(e => e.taux),
      getStatus: () => 'neutral',
    ...
    { id: 'smic_net', categorie: '💵 Salaires', label: 'SMIC net',
      ...
      getSparkline: d => d.sparklines?.smic,
      getStatus: () => 'neutral',
    ...
    { id: 'inflation', categorie: '💵 Salaires', label: 'Inflation',
      ...
      getSparkline: d => d.sparklines?.inflation,
    ...
    { id: 'ecart_hf', categorie: '💵 Salaires', label: 'Écart H/F',
      ...
      getSparkline: d => d.ecart_hf?.evolution?.map(e => e.ecart),
      getStatus: () => 'bad',
    ...
    { id: 'salaire_median', categorie: '💵 Salaires', label: 'Salaire médian',
      ...
      getSparkline: d => d.salaire_median?.evolution?.map(e => e.montant),
      getStatus: () => 'neutral',
    ...
    { id: 'irl', categorie: '🏠 Conditions de vie', label: 'IRL (loyers)',
      ...
      getSparkline: d => d.irl?.evolution?.map(e => e.glissement),
    ...
    { id: 'gazole', categorie: '🏠 Conditions de vie', label: 'Gazole',
      ...
      getSparkline: d => d.carburants?.evolution?.map(e => e.gazole),
    ...
    { id: 'sp95', categorie: '🏠 Conditions de vie', label: 'SP95',
      ...
      getSparkline: d => d.carburants?.evolution?.map(e => e.sp95),
    ...
    { id: 'oat_10ans', categorie: '💹 Marchés financiers', label: 'OAT 10 ans',
      ...
      getSparkline: d => d.marches_financiers?.oat_10ans?.historique?.slice(-20)?.map(p => p.valeur),
      getStatus: () => 'neutral',
    ...
    { id: 'taux_immo', categorie: '💰 Pouvoir d\'achat', label: 'Taux immo',
      ...
      getSparkline: d => d.marches_financiers?.taux_immobilier?.historique?.slice(-12)?.map(p => p.valeur),
...
// ==================== ONGLET FAVORIS ====================
function FavorisTab({ d, darkMode, favoris, toggleFavori, isFavori }) {
  ...
  const indicsFavoris = catalogue.filter(ind => favoris.includes(ind.id));

  // Catalogue filtré pour le mode config
  const catalogueFiltré = filterCat === 'Tous'
    ? catalogue
    : catalogue.filter(ind => ind.categorie === filterCat);
        ...
        <div className={`flex items-center justify-between p-4 rounded-2xl ${darkMode ? 'bg-gray-800' : 'bg-white shadow-sm'}`}>
          <div>
            <h2 className={`font-bold text-base ${darkMode ? 'text-white' : 'text-gray-800'}`}>
              ✏️ Configurer mes favoris
            </h2>
            <p className={`text-xs mt-0.5 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
              {favoris.length} indicateur{favoris.length > 1 ? 's' : ''} sélectionné{favoris.length > 1 ? 's' : ''}
            </p>
          </div>
          <button
            onClick={() => setModeConfig(false)}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
          {catalogueFiltré.map(ind => {
            ...
            return (
              <button
                key={ind.id}
                onClick={() => toggleFavori(ind.id)}
                className={`flex items-center gap-3 p-3 rounded-2xl border text-left transition-all ${
                  actif
                    ? darkMode
                      ? 'bg-blue-900/40 border-blue-700 shadow-lg'
              ...
              >
                <span className={`text-lg ${actif ? 'text-yellow-400' : darkMode ? 'text-gray-600' : 'text-gray-300'}`}>
                  {actif ? '★' : '☆'}
                </span>
                ...
                <span className={`text-sm font-bold tabular-nums ${darkMode ? 'text-blue-400' : 'text-blue-600'}`}>
                  {val}
                </span>
              </button>
            );
    ...
    <div className="space-y-4">
        ...
        <div>
          ...
          <p className={`text-xs mt-0.5 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
            {indicsFavoris.length > 0
              ? `${indicsFavoris.length} indicateur${indicsFavoris.length > 1 ? 's' : ''} suivi${indicsFavoris.length > 1 ? 's' : ''}`
              : 'Aucun indicateur sélectionné'}
          </p>
        </div>
        <button
          onClick={() => setModeConfig(true)}
      ...
      {indicsFavoris.length === 0 && (
        <div className={`flex flex-col items-center justify-center py-16 rounded-2xl ${darkMode ? 'bg-gray-800' : 'bg-white shadow-sm'}`}>
          ...
          <h3 className={`font-semibold text-base mb-2 ${darkMode ? 'text-gray-200' : 'text-gray-700'}`}>
            Votre tableau de bord est vide
          </h3>
          <p className={`text-sm mb-6 text-center max-w-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
            Sélectionnez les indicateurs que vous souhaitez suivre au quotidien.
          </p>
          <button
            onClick={() => setModeConfig(true)}
            className="px-5 py-2.5 bg-blue-600 text-white rounded-xl text-sm font-medium hover:bg-blue-700 transition"
          >
            ✏️ Choisir mes indicateurs
          </button>
        </div>
      )}
      ...
      {indicsFavoris.length > 0 && (
        <>
          ...
          {categories.map(cat => {
            const indsDecat = indicsFavoris.filter(ind => ind.categorie === cat);
            if (indsDecat.length === 0) return null;
            return (
              <div key={cat}>
                <h3 className={`text-xs font-semibold mb-2 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                  {cat}
                </h3>
                  ...
                  {indsDecat.map(ind => (
                    <div key={ind.id} className="relative group">
                      ...
                      <button
                        ...
                        className={`absolute top-1.5 right-1.5 opacity-0 group-hover:opacity-100 transition-opacity text-xs rounded-lg p-0.5 ${
                          darkMode ? 'text-yellow-400 hover:text-red-400' : 'text-yellow-500 hover:text-red-400'
                        }`}
                      >
                        ★
                      </button>
          ...
          <p className={`text-center text-[10px] mt-2 ${darkMode ? 'text-gray-600' : 'text-gray-400'}`}>
            Survolez une carte pour la retirer • Cliquez sur ✏️ Configurer pour modifier la sélection
          </p>
        </>
      )}
    </div>
      ...
      <main className="max-w-7xl mx-auto px-4 pb-8">
        ...
        <div className="mt-4 sm:mt-6">
          {(() => {
            ...
            return <>
              {tab === 'favoris' && <FavorisTab d={d} darkMode={darkMode} favoris={favoris} toggleFavori={toggleFavori} isFavori={isFavori} />}
  ...
  return (
    <div className="space-y-4">
      ...
      {subTab === 'carburants' && <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
        <Card title="⛽ Prix carburants (€/L)" darkMode={darkMode} favoriId="gazole" isFavori={fp.isFavori?.("gazole")} toggleFavori={fp.toggleFavori}>
          <ResponsiveContainer width="100%" height={180}>
        ...
        <Card title={`💰 Prix ${d.carburants.derniere_donnee ? d.carburants.derniere_donnee : 'actuels'}`} darkMode={darkMode}>
          <div className="p-4 space-y-3">
            <div className={`flex justify-between items-center p-3 ${darkMode ? 'bg-orange-900/30' : 'bg-orange-50'} rounded`}>
              <span>Gazole</span>
