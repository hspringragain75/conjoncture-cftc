...
export default function App() {
  ...
  // Fonction pour aller vers l'onglet concerné
    ...
    <div className={`min-h-screen transition-colors duration-300 ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      ...
      {showAlertes && alertesNonLues.length > 0 && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowAlertes(false)}>
          <div 
            className={`w-full max-w-lg max-h-[80vh] overflow-y-auto rounded-xl shadow-2xl ${darkMode ? 'bg-gray-800' : 'bg-white'}`}
          ...
          >
            <div className={`sticky top-0 p-4 border-b ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              <div className="flex justify-between items-center">
                <h2 className={`text-xl font-bold ${darkMode ? 'text-white' : 'text-gray-800'}`}>
                  🔔 Quoi de neuf ? <span className="text-sm font-normal text-blue-500">({alertesNonLues.length} nouveau{alertesNonLues.length > 1 ? 'x' : ''})</span>
                </h2>
                <button 
                  onClick={() => setShowAlertes(false)}
                  className={`p-1 rounded hover:bg-gray-200 ${darkMode ? 'hover:bg-gray-700 text-gray-400' : ''}`}
                >
                  ✕
                </button>
              </div>
            ...
            <div className="p-4 space-y-3">
              {alertesNonLues.map(alerte => (
                <div 
                  key={alerte.id}
                  className={`p-4 rounded-lg border-l-4 cursor-pointer transition-transform hover:scale-[1.02] ${
                    alerte.type === 'success' ? 'border-green-500 bg-green-50' :
                ...
                >
                  <div className="flex justify-between items-start mb-1">
                    ...
                    <span className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>{alerte.date}</span>
                  </div>
            ...
            <div className={`sticky bottom-0 p-4 border-t ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
              <button 
      ...
      <header className={`py-3 md:py-4 px-3 md:px-4 ${darkMode ? 'bg-gray-800' : 'bg-gradient-to-r from-blue-800 to-blue-600'} text-white`}>
        <div className="flex justify-between items-center gap-2">
          <div className="min-w-0 flex-1">
            <h1 className="text-base sm:text-lg md:text-xl font-bold truncate">📊 Tableau de Bord Économique CFTC</h1>
            <div className="flex flex-wrap gap-x-2 md:gap-x-4 text-xs sm:text-sm mt-1">
              <span className={darkMode ? 'text-gray-400' : 'text-blue-200'}>Màj : {d.last_updated}</span>
              <span className={`hidden sm:inline ${darkMode ? 'text-gray-300' : 'text-blue-100'}`}>📧 <a href={`mailto:${d.contact}`} className="underline hover:text-white">{d.contact}</a></span>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button 
              ...
              className={`p-2 rounded-lg transition-colors flex-shrink-0 relative ${darkMode ? 'bg-gray-700 hover:bg-gray-600' : 'bg-blue-700 hover:bg-blue-800'}`}
            ...
            <button 
              ...
              className={`p-2 rounded-lg transition-colors flex-shrink-0 ${darkMode ? 'bg-gray-700 hover:bg-gray-600' : 'bg-blue-700 hover:bg-blue-800'}`}
      ...
      <div className="max-w-7xl mx-auto px-2 sm:px-4 py-2 sm:py-4">
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-1.5 sm:gap-2 mb-3 sm:mb-4">
        ...
        {/* Navigation par onglets avec flèches sur desktop */}
        <NavTabs 
          tabs={[['conjoncture','📈 Conjoncture'],['previsions','🔮 Prévisions'],['evolutions','📉 Évolutions'],['pouvoir_achat','💰 Pouvoir d\'achat'],['salaires','💵 Salaires'],['emploi','👥 Emploi'],['travail','⚙️ Travail'],['territoires','🗺️ Territoires'],['conditions_vie','🏠 Conditions'],['inflation','📊 Inflation'],['conventions','📋 Conventions'],['comparaison_ue','🇪🇺 Europe'],['simulateur','🧮 Simulateur'],['aide','📖 Aide']]}
          activeTab={tab}
          setActiveTab={setTab}
          darkMode={darkMode}
        />
...
function Kpi({label, value, color, darkMode, tooltip, sparklineData}) {
  ...
  return (
    <div 
      className={`p-1.5 sm:p-2 rounded border relative cursor-help ${colors[color]}`}
      onMouseEnter={() => setShowTooltip(true)}
      onMouseLeave={() => setShowTooltip(false)}
      onClick={() => setShowTooltip(!showTooltip)}
    >
      <p className="text-[10px] sm:text-xs opacity-75 flex items-center gap-1 truncate">{label} {tooltip && <span className="text-[8px] sm:text-[10px]">ⓘ</span>}</p>
      <div className="flex items-center justify-between">
        <p className="text-sm sm:text-lg font-bold">{value}</p>
        <span className="hidden sm:block">{sparklineData && <Sparkline data={sparklineData} color={color} darkMode={darkMode} />}</span>
      </div>
      {showTooltip && tooltip && (
        <div className={`absolute z-50 bottom-full left-1/2 -translate-x-1/2 mb-2 p-2 rounded shadow-lg text-xs w-44 sm:w-52 ${darkMode ? 'bg-gray-700 text-gray-200' : 'bg-gray-800 text-white'}`}>
          {tooltip}
          {sparklineData && (
...
function Card({title, children, darkMode}) {
  return <div className={`rounded-xl shadow p-3 sm:p-4 border ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}><h3 className={`text-xs sm:text-sm font-semibold mb-2 sm:mb-3 ${darkMode ? 'text-gray-200' : 'text-gray-800'}`}>{title}</h3>{children}</div>;
}

// Composant de navigation avec flèches sur desktop
function NavTabs({ tabs, activeTab, setActiveTab, darkMode }) {
  ...
  return (
    <div className={`relative mb-3 sm:mb-4 border-b pb-2 sm:pb-3 ${darkMode ? 'border-gray-700' : ''}`}>
      {/* Flèche gauche - desktop uniquement */}
      {showLeftArrow && (
        <button
          onClick={() => scroll('left')}
          className={`hidden md:flex absolute left-0 top-1/2 -translate-y-1/2 z-10 w-8 h-8 items-center justify-center rounded-full shadow-lg transition-all hover:scale-110 ${
      ...
      <div
        ref={scrollRef}
        className={`flex gap-1.5 sm:gap-2 overflow-x-auto scrollbar-hide ${showLeftArrow ? 'md:ml-10' : ''} ${showRightArrow ? 'md:mr-10' : ''}`}
      ...
      >
        {tabs.map(([id, label]) => (
          <button
            ...
            className={`px-2 sm:px-3 py-1 sm:py-1.5 rounded text-xs sm:text-sm font-medium whitespace-nowrap transition-colors flex-shrink-0 ${
              activeTab === id
      ...
      {showRightArrow && (
        <button
          ...
          className={`hidden md:flex absolute right-0 top-1/2 -translate-y-1/2 z-10 w-8 h-8 items-center justify-center rounded-full shadow-lg transition-all hover:scale-110 ${
            darkMode ? 'bg-gray-700 text-white hover:bg-gray-600' : 'bg-white text-gray-700 hover:bg-gray-100 border'
          }`}
          style={{ marginTop: '-4px' }}
        >
      ...
      {/* Dégradés pour indiquer qu'il y a plus de contenu */}
      {showLeftArrow && (
        <div className={`hidden md:block absolute left-8 top-0 bottom-0 w-8 pointer-events-none ${
          darkMode ? 'bg-gradient-to-r from-gray-900 to-transparent' : 'bg-gradient-to-r from-gray-50 to-transparent'
        }`} style={{ marginBottom: '8px' }} />
      )}
      {showRightArrow && (
        <div className={`hidden md:block absolute right-8 top-0 bottom-0 w-8 pointer-events-none ${
          darkMode ? 'bg-gradient-to-l from-gray-900 to-transparent' : 'bg-gradient-to-l from-gray-50 to-transparent'
        }`} style={{ marginBottom: '8px' }} />
      )}
    </div>
  );
      ...
      {subTab === 'seniors' && <div className="space-y-4">
          ...
          <Card title="👴 Taux d'emploi 55-64 ans (%)" darkMode={darkMode}>
            <ResponsiveContainer width="100%" height={200}>
              <ComposedChart data={d.emploi_seniors}>
                ...
                <Line dataKey="taux" stroke={C.tertiary} strokeWidth={3} dot={{ fill: C.tertiary, strokeWidth: 2 }} />
                <ReferenceLine y={56.2} stroke={C.quaternary} strokeDasharray="5 5" label={{ value: 'Moy. UE', position: 'right', fontSize: 10, fill: C.quaternary }} />
              </ComposedChart>
            </ResponsiveContainer>
          </Card>
          
          <Card title="📊 Situation des seniors" darkMode={darkMode}>
            <div className="space-y-3 p-2">
              <div className={`flex justify-between items-center p-3 ${darkMode ? 'bg-green-900/30' : 'bg-green-50'} rounded`}>
                <span className={darkMode ? 'text-gray-300' : ''}>Taux d'emploi 55-64 ans</span>
                <span className="text-2xl font-bold text-green-600">{d.indicateurs_cles.taux_emploi_seniors}%</span>
              </div>
              <div className={`flex justify-between items-center p-3 ${darkMode ? 'bg-blue-900/30' : 'bg-blue-50'} rounded`}>
                <span className={darkMode ? 'text-gray-300' : ''}>Taux d'emploi 60-64 ans</span>
                <span className="text-xl font-bold text-blue-600">{d.emploi_seniors_detail?.taux_60_64 || 38.2}%</span>
              </div>
              <div className={`flex justify-between items-center p-3 ${darkMode ? 'bg-orange-900/30' : 'bg-orange-50'} rounded`}>
                <span className={darkMode ? 'text-gray-300' : ''}>Taux de chômage 55+</span>
                <span className="text-xl font-bold text-orange-600">{d.emploi_seniors_detail?.chomage_seniors || 5.8}%</span>
              </div>
              <div className={`flex justify-between items-center p-3 ${darkMode ? 'bg-purple-900/30' : 'bg-purple-50'} rounded`}>
                <span className={darkMode ? 'text-gray-300' : ''}>Évolution sur 1 an</span>
                <span className={`text-xl font-bold ${(d.emploi_seniors_detail?.evolution_1an || 1.2) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                  {(d.emploi_seniors_detail?.evolution_1an || 1.2) >= 0 ? '+' : ''}{d.emploi_seniors_detail?.evolution_1an || 1.2} pts
                </span>
              </div>
            </div>
          </Card>
          ...
          <Card title="🇪🇺 Comparaison européenne - Taux d'emploi 55-64 ans" darkMode={darkMode}>
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={[
                ...
                {pays: '🇫🇷 France', taux: d.indicateurs_cles.taux_emploi_seniors || 58.4},
                {pays: '🇪🇺 Moy. UE', taux: 62.4},
                ...
                <Tooltip {...chartProps.tooltip} formatter={v => `${v}%`} />
                <Bar dataKey="taux" fill={C.primary}>
                  {[0,1,2,3,4,5,6].map(i => (
                    <Cell key={i} fill={i === 3 ? C.tertiary : i === 4 ? C.quaternary : C.primary} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
            ...
            <div className="space-y-2 p-2">
              <div className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                ...
                <p className="text-xs mt-2 opacity-75">
                  📈 +{((d.indicateurs_cles.taux_emploi_seniors || 58.4) - 39.7).toFixed(1)} points en 15 ans
                </p>
              </div>
            </div>
          </Card>
        ...
        <div className={`p-4 rounded-lg border-l-4 border-green-500 ${darkMode ? 'bg-green-900/20' : 'bg-green-50'}`}>
          ...
          <ul className={`text-sm space-y-1 ${darkMode ? 'text-green-200' : 'text-green-700'}`}>
            <li>• Le <b>taux d'emploi des 55-64 ans</b> mesure la part des personnes en emploi parmi cette tranche d'âge</li>
            <li>• La France a progressé de +19 pts depuis 2010 grâce aux réformes des retraites successives</li>
            <li>• Mais reste en-dessous de la moyenne européenne (62.4%) et loin de l'Allemagne (72%)</li>
            <li>• Le taux chute fortement après 60 ans ({d.emploi_seniors_detail?.taux_60_64 || 38.2}%) : effet de l'âge légal de départ</li>
            <li>• Le chômage des seniors ({d.emploi_seniors_detail?.chomage_seniors || 5.8}%) est plus faible mais la durée de chômage est plus longue</li>
          </ul>
        </div>
      </div>}
