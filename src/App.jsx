...
function ConjonctureTab({d, subTab, setSubTab, darkMode}) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* MARCHES FINANCIERS - Analyse avancee quotidienne */}
      {subTab === 'marches' && d.marches_financiers && (() => {
        ...
        const MinMaxBar = ({minMax, unite, color}) => {
        ...
        // Thin every Nth point for chart perf (~250 data points shown max)
        ...
        {/* KPI Cards with variations */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className={`p-4 rounded-2xl border ${darkMode ? 'bg-purple-900/20 border-purple-800/50' : 'bg-purple-50 border-purple-200'}`}>
            <div className="flex items-center justify-between mb-1">
              <span className={`text-xs font-medium ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>OAT 10 ans</span>
              <span className={`text-[10px] px-2 py-0.5 rounded-lg ${darkMode ? 'bg-purple-800/50 text-purple-300' : 'bg-purple-100 text-purple-600'}`}>
                {mf.oat_10ans.source} ({mf.oat_10ans.nb_points || '?'} pts)
              </span>
            </div>
            <div className="flex items-baseline gap-2">
              <span className={`text-2xl font-bold ${darkMode ? 'text-purple-400' : 'text-purple-700'}`}>{mf.oat_10ans.valeur}%</span>
              <span className={`text-xs ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>{mf.oat_10ans.date}</span>
            </div>
            <VariationsRow variations={mf.oat_10ans.variations} label="Var." />
            <MinMaxBar minMax={mf.oat_10ans.min_max} unite="%" />
          </div>
          <div className={`p-4 rounded-2xl border ${darkMode ? 'bg-amber-900/20 border-amber-800/50' : 'bg-amber-50 border-amber-200'}`}>
            <div className="flex items-center justify-between mb-1">
              <span className={`text-xs font-medium ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Brent</span>
              <span className={`text-[10px] px-2 py-0.5 rounded-lg ${darkMode ? 'bg-amber-800/50 text-amber-300' : 'bg-amber-100 text-amber-600'}`}>
                {mf.petrole_brent.source} ({mf.petrole_brent.nb_points || '?'} pts)
              </span>
            </div>
            <div className="flex items-baseline gap-2">
              <span className={`text-2xl font-bold ${darkMode ? 'text-amber-400' : 'text-amber-700'}`}>{mf.petrole_brent.valeur} $</span>
              <span className={`text-xs ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>USD/baril {mf.petrole_brent.date}</span>
            </div>
            <VariationsRow variations={mf.petrole_brent.variations} label="Var." />
            <MinMaxBar minMax={mf.petrole_brent.min_max} unite=" $" />
          </div>
          <div className={`p-4 rounded-2xl border ${darkMode ? 'bg-blue-900/20 border-blue-800/50' : 'bg-blue-50 border-blue-200'}`}>
            <div className="flex items-center justify-between mb-1">
              <span className={`text-xs font-medium ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>CAC 40</span>
              <span className={`text-[10px] px-2 py-0.5 rounded-lg ${darkMode ? 'bg-blue-800/50 text-blue-300' : 'bg-blue-100 text-blue-600'}`}>
                {mf.cac40.source} ({mf.cac40.nb_points || '?'} pts)
              </span>
            </div>
            <div className="flex items-baseline gap-2">
              <span className={`text-2xl font-bold ${darkMode ? 'text-blue-400' : 'text-blue-700'}`}>{mf.cac40.valeur?.toLocaleString('fr-FR')}</span>
              <span className={`text-xs ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>pts {mf.cac40.date}</span>
            </div>
            <VariationsRow variations={mf.cac40.variations} label="Var." />
            <MinMaxBar minMax={mf.cac40.min_max} unite=" pts" />
          </div>
        </div>

        {/* Charts with MA50/MA200 */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
          <Card title="OAT 10 ans - Quotidien + MA50/MA200" darkMode={darkMode}>
            <ResponsiveContainer width="100%" height={220}>
              <ComposedChart data={thin(mf.oat_10ans.historique)}>
                <CartesianGrid {...chartProps.cartesianGrid} />
                <XAxis dataKey="date" {...chartProps.xAxis} tickFormatter={v => v?.slice(5)} interval={Math.floor((mf.oat_10ans.historique?.length||50)/6)} />
                <YAxis {...chartProps.yAxis} domain={['auto', 'auto']} />
                <Tooltip {...chartProps.tooltip} labelFormatter={v => `Date: ${v}`} formatter={(v,n) => [v != null ? `${v}${n==='valeur'?'%':''}` : '-', n==='valeur'?'OAT':n==='ma50'?'MA50':n==='ma200'?'MA200':n]} />
                <Area dataKey="valeur" fill={C.purple} fillOpacity={0.08} stroke="none" />
                <Line dataKey="valeur" stroke={C.purple} strokeWidth={1.5} dot={false} name="valeur" />
                <Line dataKey="ma50" stroke="#f59e0b" strokeWidth={1.5} dot={false} strokeDasharray="4 2" name="ma50" />
                <Line dataKey="ma200" stroke="#ef4444" strokeWidth={1.5} dot={false} strokeDasharray="8 3" name="ma200" />
              </ComposedChart>
            </ResponsiveContainer>
          </Card>

          <Card title="Brent - Quotidien + MA50/MA200" darkMode={darkMode}>
            <ResponsiveContainer width="100%" height={220}>
              <ComposedChart data={thin(mf.petrole_brent.historique)}>
                <CartesianGrid {...chartProps.cartesianGrid} />
                <XAxis dataKey="date" {...chartProps.xAxis} tickFormatter={v => v?.slice(5)} interval={Math.floor((mf.petrole_brent.historique?.length||50)/6)} />
                <YAxis {...chartProps.yAxis} domain={['auto', 'auto']} />
                <Tooltip {...chartProps.tooltip} labelFormatter={v => `Date: ${v}`} formatter={(v,n) => [v != null ? `${v}${n==='valeur'?' $/b':''}` : '-', n==='valeur'?'Brent':n==='ma50'?'MA50':n==='ma200'?'MA200':n]} />
                <Area dataKey="valeur" fill={C.quaternary} fillOpacity={0.08} stroke="none" />
                <Line dataKey="valeur" stroke={C.quaternary} strokeWidth={1.5} dot={false} name="valeur" />
                <Line dataKey="ma50" stroke="#f59e0b" strokeWidth={1.5} dot={false} strokeDasharray="4 2" name="ma50" />
                <Line dataKey="ma200" stroke="#ef4444" strokeWidth={1.5} dot={false} strokeDasharray="8 3" name="ma200" />
              </ComposedChart>
            </ResponsiveContainer>
          ...
          <Card title="CAC 40 - Quotidien + MA50/MA200" darkMode={darkMode}>
            <ResponsiveContainer width="100%" height={220}>
              <ComposedChart data={thin(mf.cac40.historique)}>
                <CartesianGrid {...chartProps.cartesianGrid} />
                <XAxis dataKey="date" {...chartProps.xAxis} tickFormatter={v => v?.slice(5)} interval={Math.floor((mf.cac40.historique?.length||50)/6)} />
                <YAxis {...chartProps.yAxis} domain={['auto', 'auto']} />
                <Tooltip {...chartProps.tooltip} labelFormatter={v => `Date: ${v}`} formatter={(v,n) => [v != null ? `${typeof v==='number'?v.toLocaleString('fr-FR'):v}${n==='valeur'?' pts':''}` : '-', n==='valeur'?'CAC 40':n==='ma50'?'MA50':n==='ma200'?'MA200':n]} />
                <Area dataKey="valeur" fill={C.primary} fillOpacity={0.08} stroke="none" />
                <Line dataKey="valeur" stroke={C.primary} strokeWidth={1.5} dot={false} name="valeur" />
                <Line dataKey="ma50" stroke="#f59e0b" strokeWidth={1.5} dot={false} strokeDasharray="4 2" name="ma50" />
                <Line dataKey="ma200" stroke="#ef4444" strokeWidth={1.5} dot={false} strokeDasharray="8 3" name="ma200" />
              </ComposedChart>
            </ResponsiveContainer>
          ...
          <Card title="Analyse & Correlations inflation" darkMode={darkMode}>
            <div className="space-y-3 p-1">
              {mf.correlations && <div className={`p-3 rounded-xl ${darkMode ? 'bg-gray-800' : 'bg-gray-50'}`}>
                <p className={`text-xs font-semibold mb-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Corr&eacute;lation march&eacute;s / inflation (Pearson mensuel)</p>
              ...
              <div className={`p-3 rounded-xl ${darkMode ? 'bg-purple-900/20' : 'bg-purple-50'}`}>
                <p className={`text-xs font-semibold mb-1 ${darkMode ? 'text-purple-300' : 'text-purple-700'}`}>OAT 10 ans</p>
                <p className={`text-xs ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>{mf.oat_10ans.note_lecture}</p>
              </div>
              <div className={`p-3 rounded-xl ${darkMode ? 'bg-amber-900/20' : 'bg-amber-50'}`}>
                <p className={`text-xs font-semibold mb-1 ${darkMode ? 'text-amber-300' : 'text-amber-700'}`}>Brent</p>
                <p className={`text-xs ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>{mf.petrole_brent.note_lecture}</p>
              </div>
              <div className={`p-3 rounded-xl ${darkMode ? 'bg-blue-900/20' : 'bg-blue-50'}`}>
                <p className={`text-xs font-semibold mb-1 ${darkMode ? 'text-blue-300' : 'text-blue-700'}`}>CAC 40</p>
                <p className={`text-xs ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>{mf.cac40.note_lecture}</p>
              </div>
            </div>
          </Card>
        </div>
        ...
        {mf.oat_10ans.evolution_annuelle && <Card title="OAT 10 ans - Taux moyen annuel (contexte long terme)" darkMode={darkMode}>
          <ResponsiveContainer width="100%" height={160}>
            <ComposedChart data={mf.oat_10ans.evolution_annuelle}>
              ...
              <Line dataKey="valeur" stroke={C.purple} strokeWidth={2.5} dot={{ r: 4, fill: C.purple }} name="OAT 10 ans" />
            </ComposedChart>
          </ResponsiveContainer>
        </Card>}
        ...
        <div className={`${darkMode ? 'bg-indigo-900/30' : 'bg-indigo-50'} border-l-4 border-indigo-400 p-4 rounded`}>
          ...
          <div className={`mt-2 text-xs ${darkMode ? 'text-indigo-200' : 'text-indigo-700'} space-y-1`}>
            ...
            <p><b>Signaux :</b> quand la MA50 croise la MA200 vers le haut = signal haussier ("golden cross"), vers le bas = signal baissier ("death cross")</p>
            <p><b>Variations :</b> 1J = 1 jour, 1S = 1 semaine, 1M = 1 mois, YTD = depuis le 1er janvier</p>
            {mf.analyse?.periode && <p><b>P&eacute;riode :</b> {mf.analyse.periode} | Sources : {mf.sources}</p>}
          </div>
        </div>
      </div>;
      })()}
