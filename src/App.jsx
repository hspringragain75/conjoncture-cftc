...
const DATA = {
  ...
  "indicateurs_cles": { "taux_chomage_actuel": 7.7, "taux_chomage_jeunes": 19.2, "taux_emploi_seniors": 62.0, "inflation_annuelle": 0.9, "smic_net": 1443.11, "difficultes_recrutement": 61, "irl_glissement": 0.87, "prix_gazole": 1.58, "taux_effort_locataires": 29.6 },
  "inflation_salaires": [
  ...
  // NOUVELLES DONNÉES - CONDITIONS DE VIE
  "irl": {
  ...
  "taux_effort": {
    ...
    "commentaire": "STATIQUE - màj annuelle",
    "par_statut": [
...
const C = { primary: '#1e40af', secondary: '#dc2626', tertiary: '#059669', quaternary: '#d97706', pink: '#db2777', purple: '#7c3aed', cyan: '#0891b2' };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 py-4">
        <div className="grid grid-cols-3 md:grid-cols-6 gap-2 mb-4">
          ...
          <Kpi label="IRL (loyers)" value={`+${d.indicateurs_cles.irl_glissement}%`} color="green" />
          <Kpi label="Gazole" value={`${d.indicateurs_cles.prix_gazole}€/L`} color="orange" />
          <Kpi label="Effort logement" value={`${d.indicateurs_cles.taux_effort_locataires}%`} color="red" />
        </div>

        <div className="flex flex-wrap gap-2 mb-4 border-b pb-3">
          {[['pouvoir_achat','💰 Pouvoir d\'achat'],['salaires','💵 Salaires'],['emploi','👥 Emploi'],['conditions_vie','🏠 Conditions de vie'],['inflation','📈 Inflation']].map(([id,label]) => (
...
// NOUVEL ONGLET CONDITIONS DE VIE
  ...
  return (
    <div className="space-y-4">
        ...
        <Card title="📈 Indice de Référence des Loyers (IRL)">
          <ResponsiveContainer width="100%" height={220}>
            <ComposedChart data={d.irl.evolution}>
              ...
              <Bar yAxisId="right" dataKey="glissement" name="Glissement annuel %" fill={C.quaternary} />
              <Line yAxisId="left" dataKey="indice" name="Indice IRL" stroke={C.primary} strokeWidth={2} />
            </ComposedChart>
          </ResponsiveContainer>
        </Card>
        <Card title="📊 Situation actuelle">
          <div className="p-4 space-y-3">
            <div className="flex justify-between items-center p-3 bg-blue-50 rounded">
              <span>IRL actuel</span>
              <span className="text-2xl font-bold text-blue-600">{d.irl.valeur_actuelle}</span>
            </div>
            <div className="flex justify-between items-center p-3 bg-green-50 rounded">
              <span>Glissement annuel</span>
              <span className="text-2xl font-bold text-green-600">+{d.irl.glissement_annuel}%</span>
            </div>
            <p className="text-xs text-gray-500">Trimestre : {d.irl.trimestre}</p>
            <div className="bg-yellow-50 p-3 rounded text-xs">
              <p className="font-semibold">💡 Impact</p>
              <p>Pour un loyer de 800€, l'augmentation max = <b>+{(800 * d.irl.glissement_annuel / 100).toFixed(0)}€/mois</b></p>
            </div>
          </div>
        </Card>
      </div>}
      ...
      {subTab === 'immobilier' && <div className="grid md:grid-cols-2 gap-4">
        <Card title="🏠 Évolution prix logements anciens">
          <ResponsiveContainer width="100%" height={220}>
            <ComposedChart data={d.prix_immobilier.evolution}>
              ...
              <Line dataKey="indice" name="Indice (base 100=2015)" stroke={C.primary} strokeWidth={2} />
            </ComposedChart>
          </ResponsiveContainer>
          <p className="text-xs text-center text-gray-500 mt-1">Transactions : {d.prix_immobilier.transactions_annuelles.toLocaleString()} sur 12 mois</p>
        </Card>
        <Card title="💰 Prix au m² par zone">
          <ResponsiveContainer width="100%" height={220}>
          ...
          <div className="flex flex-wrap gap-2 mt-2 justify-center">
            {d.prix_immobilier.par_zone.map((z,i)=>(
              <span key={i} className={`text-xs px-2 py-1 rounded ${z.variation<0?'bg-red-50 text-red-600':'bg-green-50 text-green-600'}`}>
                {z.zone}: {z.variation>0?'+':''}{z.variation}%
              </span>
            ))}
          </div>
        </Card>
      ...
      {subTab === 'carburants' && <div className="grid md:grid-cols-2 gap-4">
        <Card title="⛽ Évolution prix carburants (€/L)">
          <ResponsiveContainer width="100%" height={220}>
        ...
        <Card title="💰 Prix actuels">
          <div className="p-4 space-y-3">
            <div className="flex justify-between items-center p-3 bg-orange-50 rounded">
              <span className="font-medium">Gazole</span>
              <div className="text-right">
                <span className="text-2xl font-bold text-orange-600">{d.carburants.gazole.prix}€/L</span>
                <span className={`text-xs ml-2 ${d.carburants.gazole.variation_an<0?'text-green-600':'text-red-600'}`}>
                  {d.carburants.gazole.variation_an>0?'+':''}{d.carburants.gazole.variation_an}%/an
                </span>
              </div>
            </div>
            <div className="flex justify-between items-center p-3 bg-green-50 rounded">
              <span className="font-medium">SP95</span>
              <div className="text-right">
                <span className="text-2xl font-bold text-green-600">{d.carburants.sp95.prix}€/L</span>
                <span className={`text-xs ml-2 ${d.carburants.sp95.variation_an<0?'text-green-600':'text-red-600'}`}>
                  {d.carburants.sp95.variation_an>0?'+':''}{d.carburants.sp95.variation_an}%/an
                </span>
              </div>
            </div>
            <div className="flex justify-between items-center p-3 bg-gray-50 rounded">
              <span className="font-medium">SP98</span>
              <div className="text-right">
                <span className="text-2xl font-bold">{d.carburants.sp98.prix}€/L</span>
                <span className={`text-xs ml-2 ${d.carburants.sp98.variation_an<0?'text-green-600':'text-red-600'}`}>
                  {d.carburants.sp98.variation_an>0?'+':''}{d.carburants.sp98.variation_an}%/an
                </span>
              </div>
            </div>
            <div className="bg-blue-50 p-3 rounded text-xs">
              <p className="font-semibold">💡 Budget type</p>
              <p>Pour 1000 km/mois (7L/100km) : ~{(1000*0.07*d.carburants.gazole.prix).toFixed(0)}€/mois</p>
            </div>
          </div>
        </Card>
      </div>}
      ...
      {subTab === 'effort' && <div className="space-y-4">
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <Card title="📊 Par niveau de revenu">
            <ResponsiveContainer width="100%" height={200}>
        ...
        <Card title="📈 Évolution du taux d'effort">
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={d.taux_effort.evolution}>
              <CartesianGrid strokeDasharray="3 3" />
              <XAxis dataKey="annee" fontSize={11} />
              <YAxis domain={[10,35]} fontSize={11} />
              <Tooltip formatter={v=>`${v}%`} />
              <Legend wrapperStyle={{fontSize:10}} />
              <Line dataKey="ensemble" name="Ensemble ménages" stroke={C.primary} strokeWidth={2} />
              <Line dataKey="locataires_libre" name="Locataires libres" stroke={C.secondary} strokeWidth={2} />
            </LineChart>
          </ResponsiveContainer>
          <p className="text-xs text-gray-500 text-center mt-1">Source: {d.taux_effort.source}</p>
        </Card>
        <div className="bg-red-50 border-l-4 border-red-400 p-4 rounded">
          <h3 className="font-semibold text-red-800">⚠️ Alerte conditions de vie</h3>
          <ul className="mt-2 text-sm text-red-700 space-y-1">
            <li>• 25% des ménages les plus modestes : <b>31% à 44%</b> de revenus pour le logement</li>
            <li>• Locataires secteur libre : taux d'effort passé de 24% (2001) à <b>30%</b> (2023)</li>
            <li>• 1 locataire sur 5 consacre plus de 40% de ses revenus au logement</li>
          </ul>
        </div>
      </div>}
    </div>
