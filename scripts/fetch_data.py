...
SERIES_IDS = {
    ...
    # === EMPLOI PAR SECTEUR ===
    "emploi_industrie":          "010604645",  # Emploi salarié industrie (Cptes nationaux trimestriels base 2020)
    "emploi_construction":        "010604646",  # Emploi salarié construction
    "emploi_tertiaire_marchand":  "010604647",  # Emploi salarié tertiaire marchand
    "emploi_tertiaire_nonmarc":   "010604648",  # Emploi salarié tertiaire non marchand
    "emploi_total":               "010604644",  # Emploi salarié total
...
def build_emploi_secteur_data():
    ...
    # Récupération des 4 grands secteurs — Comptes nationaux trimestriels base 2020
    ...
    if emploi_industrie and emploi_constr and emploi_terc_marc:
        ...
        def _last(s):    return s[-1] if s else 0
        def _evol_t(s):  return round((s[-1]-s[-2])/s[-2]*100,1) if s and len(s)>=2 and s[-2] else 0
        def _evol_a(s):  return round((s[-1]-s[-5])/s[-5]*100,1) if s and len(s)>=5 and s[-5] else 0
        ...
        for i in range(nb):
            q = i % 4 + 1
            y = 2023 + i // 4
            tm  = emploi_terc_marc[i]  if i < len(emploi_terc_marc)  else 0
            tnm = emploi_terc_nmarc[i] if emploi_terc_nmarc and i < len(emploi_terc_nmarc) else 0
            trimestres_hist.append({
                "trimestre":    f"T{q} {y}",
                "industrie":    round(emploi_industrie[i] / 1000) if emploi_industrie[i] else None,
                "construction": round(emploi_constr[i]    / 1000) if emploi_constr[i]    else None,
                "tertiaire":    round((tm + tnm)           / 1000) if (tm or tnm)         else None,
                "interim":      None,
            })

        tnm_last = _last(emploi_terc_nmarc) if emploi_terc_nmarc else default_secteurs["secteurs"][1]["emploi"] * 1000
        return {
            "derniere_mise_a_jour": trimestres_hist[-1]["trimestre"] if trimestres_hist else "N/A",
            "secteurs": [
...
def build_marches_financiers_data():
    ...
    # Nickel : NI=F directement (LNI%3DF et NI%3DF sont morts)
    nickel_data, nickel_source = fetch_lme_metal("NI=F", (15500, 800, 40), "Nickel")
