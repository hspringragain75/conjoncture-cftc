...
def build_emplois_vacants_data():
    ...
    # Essai de plusieurs noms de datasets DARES (l'API renomme parfois les datasets)
    VACANTS_IDS = [
        "dares_emploivacants_brut_emploisvacants",
        "dares-emplois-vacants-brut-emploisvacants",
        "dares_emploi_vacant",
        "dares-emploi-vacant",
    ]
    OCCUPES_IDS = [
        "dares_emploivacants_brut_emploisoccupes",
        "dares-emplois-vacants-brut-emploisoccupes",
        "dares_emploi_occupe",
    ]
    vacants_raw = None
    for ds_id in VACANTS_IDS:
        vacants_raw = fetch_dares_dataset(ds_id, limit=100)
        if vacants_raw:
            break
    occupes_raw = None
    for ds_id in OCCUPES_IDS:
        occupes_raw = fetch_dares_dataset(ds_id, limit=100)
        if occupes_raw:
            break
...
def fetch_insee_series(series_id, start_period="2015"):
    ...
    Retry avec backoff exponentiel : 3 tentatives (10s → 20s → 30s).
    Délai anti-rate-limit entre tentatives pour éviter "connection reset".
    """
    ...
    timeouts = [10, 20, 30]
    delays   = [3, 6]      # Pauses entre tentatives (anti rate-limit INSEE)
    ...
    for attempt, timeout in enumerate(timeouts, 1):
        try:
            ...
            print(f"  ⚠️ HTTP {e.code} série {series_id} (tentative {attempt}/{len(timeouts)})")
            if e.code in (400, 404, 410):
                break  # Série inexistante — inutile de réessayer
        except urllib.error.URLError as e:
            reason = str(e.reason) if hasattr(e, "reason") else str(e)
            print(f"  ⚠️ Réseau série {series_id} (tentative {attempt}/{len(timeouts)}): {reason}")
        except Exception as e:
            print(f"  ⚠️ Erreur série {series_id} (tentative {attempt}/{len(timeouts)}): {e}")

        if attempt < len(timeouts):
            _time.sleep(delays[attempt - 1])

    print(f"  ⚠️ Série {series_id} indisponible après {len(timeouts)} tentatives")
    return None
...
def build_emploi_secteur_data():
    ...
    # Tentative sur série principale, puis série alternative si échec
    emploi_industrie = get_quarterly_values(SERIES_IDS["emploi_industrie"], 2024)
    if not emploi_industrie:
        # Série alternative INSEE : emploi salarié total (010565595)
        emploi_industrie = get_quarterly_values("010565595", 2024)

    if emploi_industrie and len(emploi_industrie) > 0:
        print(f"  ✓ Données emploi secteur récupérées ({len(emploi_industrie)} trimestres)")
    else:
...
def build_carburants_data():
    ...
    # Le dataset flux annuel a changé de nom fin 2025
    # Essai en cascade des noms connus
    def find_annuel_dataset():
        candidates = [
            "prix-des-carburants-en-france-flux-instantane-v2",  # Même dataset, historique inclus
            "prix-des-carburants-en-france-flux-annuel-v2",      # Ancien nom
            "prix-des-carburants-en-france-flux-annuel",         # Sans -v2
            "prix-des-carburants-en-france",                     # Générique
        ]
        for ds in candidates:
            try:
                test_url = f"{BASE_URL}/{ds}/records?limit=1"
                api_get(test_url)
                return ds
            except Exception:
                continue
        return None
    ANNUEL_DS = find_annuel_dataset() or "prix-des-carburants-en-france-flux-instantane-v2"
...
def build_salaires_secteur_data():
    ...
    # SMB : essai série principale puis série alternative (restructuration INSEE 2024)
    smb_industrie    = get_quarterly_values(SERIES_IDS["smb_industrie"], 2023)                     or get_quarterly_values("001698371", 2023)
    smb_construction = get_quarterly_values(SERIES_IDS["smb_construction"], 2023)                     or get_quarterly_values("001698373", 2023)
    smb_tertiaire    = get_quarterly_values(SERIES_IDS["smb_tertiaire"], 2023)                     or get_quarterly_values("001698375", 2023)
...
def build_marches_financiers_data():
    ...
    # Source 1 : BCE data-api (nouvelle URL stable)
    try:
    ...
    if not oat_data:
        try:
            print("    -> OAT 10 ans : Yahoo Finance ^TNX (proxy US 10Y)...")
            tnx_data = fetch_yahoo_daily("%5ETNX", period="1y", interval="1d")
            if tnx_data:
                # ^TNX cote en dixièmes de % (ex: 42.5 = 4.25%), OAT FR ≈ TNX - 1.2%
                oat_data = [{"date": p["date"], "valeur": round(p["valeur"] / 10 - 1.2, 3)} for p in tnx_data if p["valeur"] / 10 - 1.2 > 0]
    ...
    # Source 3 : FRED série mensuelle OAT FR (nomenclature correcte)
    if not oat_data:
        try:
    ...
    # Nickel : NI=F (Yahoo, LME Nickel Futures) — LNI%3DF est mort
    nickel_data, nickel_source = fetch_lme_metal("NI%3DF", (15500, 800, 40), "Nickel")
