...
def build_emplois_vacants_data():
    ...
    # Récupérer les 100 enregistrements les plus récents (sort=-date, sans filtre where)
    # Le filtrage par date et nomenclature est fait côté Python dans parse_dares_records
    ...
    def parse_dares_records(records, value_field=None):
        ...
        Champs réels : date (YYYY-Tn), nomenclature (A21), code_naf,
                       nombre_d_emplois_vacants, taux_d_emplois_vacants_en
        Stratégie : utiliser nomenclature A21, sommer nombre_d_emplois_vacants
        par trimestre (données Brutes — pas de CVS dans ce dataset).
        Pour emplois_occupes : utiliser nombre_d_emplois_occupes.
        """
        by_trimestre = {}
        ...
        for record in records:
            # Filtrer côté Python : nomenclature préférée + données à partir de 2015
            nomen = (record.get("nomenclature") or "").strip()
            if nomen_prefere and nomen and nomen != nomen_prefere:
                continue
            # Les données récentes sont garanties par l'offset

            # Date format 'YYYY-Tn' → 'Tn YYYY'
            ...
            # Valeur : chercher dans tous les champs possibles
            valeur = None
            ...
            # Sommer par trimestre (tous secteurs NAF)
            if trimestre not in by_trimestre:
                by_trimestre[trimestre] = {"trimestre": trimestre, "valeur": 0.0, "secteur": "Ensemble"}
