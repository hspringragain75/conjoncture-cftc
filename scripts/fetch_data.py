...
def build_emplois_vacants_data():
    ...
    vacants_raw = fetch_dares_dataset("dares_emploivacants_brut_emploisvacants", limit=500)
    occupes_raw = fetch_dares_dataset("dares_emploivacants_brut_emploisoccupes", limit=500)
    ...
    def parse_dares_records(records, value_field=None):
        ...
        Champs réels du dataset :
          date            : format 'YYYY-Tn' (ex: '2024-T3')
          champ           : 'Plus de 10 salariés' (périmètre)
          type_de_donnees : 'Brutes' ou 'CVS'
          code_naf / libelle_naf : secteur NAF
          nombre_d_emplois_vacants / nombre_d_emplois_occupes
        Stratégie : garder CVS uniquement, agréger tous secteurs par trimestre.
        """
        by_trimestre = {}

        # Identifier le type disponible (CVS préféré, sinon Brutes)
        types_dispo = set(r.get("type_de_donnees","").upper() for r in records if r.get("type_de_donnees"))
        type_prefere = "CVS" if "CVS" in types_dispo else ("BRUTES" if "BRUTES" in types_dispo else "")
        # Identifier la nomenclature la plus agrégée disponible (A17 > A21 > A38)
        nomenclatures = set(r.get("nomenclature","") for r in records if r.get("nomenclature"))
        nomen_prefere = next((n for n in ["A17","A21","A38","A10","A5"] if n in nomenclatures), "")
        if type_prefere:
            print(f"    → Filtre: type={type_prefere}, nomenclature={nomen_prefere or 'toutes'}")

        for record in records:
            # Filtrer sur le type préféré
            type_donnees = (record.get("type_de_donnees") or "").strip().upper()
            if type_prefere and type_donnees != type_prefere:
                continue
            # Filtrer sur la nomenclature préférée si disponible
            ...
            # Valeur — champs réels du dataset
            valeur = None
            champs_valeur = [
                value_field,
                "nombre_d_emplois_vacants",
                "nombre_d_emplois_occupes",
                "nombre_emplois_vacants",
                "nombre_emplois_occupes",
                "nombre", "valeur",
            ]
            for field in champs_valeur:
            ...
            # Agréger tous les secteurs NAF par trimestre
        ...
        if result:
            print(f"  ✓ {len(result)} trimestres emplois vacants/occupés parsés")
        return result
