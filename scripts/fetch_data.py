...
def build_emplois_vacants_data():
    ...
    def fetch_dares_recent(dataset_id, total=3300, window=300):
        ...
        for offset in range(max(0, total - window), total, 100):
            ...
            if records:
                ...
                print(f"  ⚠️ DARES offset={offset} → vide ou erreur")
        if all_records:
            dates = sorted(set(r.get("date","") for r in all_records if r.get("date")))
            print(f"  🔍 {len(all_records)} enregistrements, dates: {dates[:3]}...{dates[-2:]}")
    ...
    def parse_dares_records(records, value_field=None):
        """
        Parse les enregistrements DARES emplois vacants/occupés.
        Stratégie robuste : ne garder que les trimestres où TOUS les secteurs
        A21 sont présents (15 secteurs), pour éviter les totaux partiels instables.
        On compte le nombre de secteurs par trimestre et on n'agrège que les complets.
        """
        by_trimestre = {}   # {trimestre: {"valeur": float, "nb_secteurs": int}}

        # Nomenclature préférée A21
        ...
        for record in records:
            ...
            valeur = None
            for field in [value_field, "nombre_d_emplois_vacants",
                          "nombre_d_emplois_occupes", "nombre", "valeur"]:
            ...
            if trimestre not in by_trimestre:
                by_trimestre[trimestre] = {"trimestre": trimestre, "valeur": 0.0,
                                           "secteur": "Ensemble", "nb_secteurs": 0}
            by_trimestre[trimestre]["valeur"]      += valeur
            by_trimestre[trimestre]["nb_secteurs"] += 1

        # Ne garder que les trimestres avec au moins 10 secteurs (trimestres complets)
        # pour éviter les totaux partiels qui créent des pics artificiels
        nb_secteurs_attendus = max((v["nb_secteurs"] for v in by_trimestre.values()), default=1)
        seuil = max(10, int(nb_secteurs_attendus * 0.8))
        by_trimestre = {k: v for k, v in by_trimestre.items() if v["nb_secteurs"] >= seuil}

        def _tri_chron(item):
            # "T3 2024" → (2024, 3) pour un tri chronologique correct
            try:
                t, y = item["trimestre"].split()
                return (int(y), int(t[1]))
            except (ValueError, IndexError):
                return (0, 0)
        result = sorted(by_trimestre.values(), key=_tri_chron)
        if result:
            print(f"  ✓ {len(result)} trimestres parsés (nomen={nomen_prefere})")
        return result
