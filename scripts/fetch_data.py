...
def build_carburants_data():
    ...
    1. Flux annuel data.economie.gouv.fr  → historique jour par jour depuis le 1er janvier
    2. Flux instantané data.economie.gouv.fr → prix moyen national du jour (toutes les 10 min)
    3. Séries INSEE mensuelles (SDMX)    → fallback si les APIs gouvernementales sont indisponibles
    4. Valeurs statiques                  → dernier recours

    Structure retournée :
    {
        "gazole":  {"prix": float, "variation_an": float, "variation_sem": float},
        "sp95":    {"prix": float, "variation_an": float, "variation_sem": float},
        "sp98":    {"prix": float, "variation_an": float, "variation_sem": float},
        "e10":     {"prix": float, "variation_an": float, "variation_sem": float},
        "evolution": [{"date": "YYYY-MM-DD", "gazole": float, "sp95": float, "e10": float}, ...],
        "derniere_donnee": "JJ/MM/YYYY HH:MM",
        "source": str,
        "granularite": "quotidien" | "mensuel"
    }
    """
    ...
    def safe_prix(val):
        """Normalise un prix retourné en millièmes ou en euros."""
        if val is None:
            return None
        try:
            v = float(val)
            if v > 100:
                v = v / 1000   # millièmes → euros
    ...
    # ──────────────────────────────────────────────────────────────
    # 1) HISTORIQUE QUOTIDIEN — flux annuel (J-1)
    #    On agrège par date toutes les stations pour obtenir le prix
    #    moyen national chaque jour depuis le 1er janvier.
    # ──────────────────────────────────────────────────────────────
    evolution_quotidien = []
    source_historique = None

    try:
        print("  → Flux annuel (historique quotidien)...")
        annee = datetime.now().year
        # On récupère les 400 derniers jours (inclut l'année précédente pour le calcul de variation)
        url_annuel = (
        ...
        if evolution_quotidien:
            source_historique = f"data.economie.gouv.fr (flux annuel — {len(evolution_quotidien)} jours)"
            print(f"  ✓ {len(evolution_quotidien)} jours récupérés ({evolution_quotidien[0]['date']} → {evolution_quotidien[-1]['date']})")

    except Exception as e:
        print(f"  ⚠️ Flux annuel indisponible: {e}")

    # ──────────────────────────────────────────────────────────────
    # 2) PRIX DU JOUR — flux instantané (toutes les 10 min)
    #    Remplace ou complète le dernier point de l'historique.
    # ──────────────────────────────────────────────────────────────
    prix_jour = {}
    try:
        ...
        if data_instant.get('results'):
            ...
            if sp95_inst:
                prix_jour = {
                    "sp95":        sp95_inst,
                    "gazole":      gazole_inst,
                    "sp98":        sp98_inst or (round(sp95_inst + 0.10, 3) if sp95_inst else None),
                    "e10":         e10_inst,
                    "derniere_maj": maj,
                }
                aujourd_hui = datetime.now().strftime("%Y-%m-%d")
                # Injecter ou mettre à jour le point du jour dans l'historique
                if evolution_quotidien and evolution_quotidien[-1]['date'] == aujourd_hui:
                    evolution_quotidien[-1].update({k: v for k, v in prix_jour.items() if k in ('sp95','gazole','sp98','e10')})
                else:
                    evolution_quotidien.append({"date": aujourd_hui, **{k: prix_jour.get(k) for k in ('sp95','gazole','sp98','e10')}})
    ...
    # ──────────────────────────────────────────────────────────────
    # 3) FALLBACK — séries INSEE mensuelles
    # ──────────────────────────────────────────────────────────────
    if not evolution_quotidien:
            ...
            if gazole_insee and sp95_insee:
                ...
                for g in gazole_insee:
                    ...
                    if period in sp95_dict:
                        # Convertir "2024-03" → "2024-03-01" pour uniformiser
                        evolution_quotidien.append({
                            "date":   f"{period}-01",
                            "gazole": round(g['value'], 3),
                            "sp95":   round(sp95_dict[period], 3),
                            "sp98":   round(sp95_dict[period] + 0.10, 3),
                            "e10":    None,
                        })
                if evolution_quotidien:
                    source_historique = "INSEE SDMX (mensuel — fallback)"
                    print(f"  ✓ {len(evolution_quotidien)} mois INSEE récupérés")
        except Exception as e:
            print(f"  ⚠️ INSEE indisponible: {e}")

    # ──────────────────────────────────────────────────────────────
    # 4) FALLBACK STATIQUE ultime
    # ──────────────────────────────────────────────────────────────
    if not evolution_quotidien:
        print("  ⚠️ Utilisation des données statiques")
        evolution_quotidien = [
            {"date": "2023-01-01", "gazole": 1.85, "sp95": 1.82, "sp98": 1.92, "e10": 1.79},
            {"date": "2023-04-01", "gazole": 1.78, "sp95": 1.88, "sp98": 1.98, "e10": 1.84},
            {"date": "2023-07-01", "gazole": 1.72, "sp95": 1.85, "sp98": 1.95, "e10": 1.81},
            {"date": "2023-10-01", "gazole": 1.88, "sp95": 1.92, "sp98": 2.02, "e10": 1.88},
            {"date": "2024-01-01", "gazole": 1.72, "sp95": 1.78, "sp98": 1.88, "e10": 1.74},
            {"date": "2024-04-01", "gazole": 1.75, "sp95": 1.82, "sp98": 1.92, "e10": 1.78},
            {"date": "2024-07-01", "gazole": 1.68, "sp95": 1.75, "sp98": 1.85, "e10": 1.71},
            {"date": "2024-10-01", "gazole": 1.62, "sp95": 1.72, "sp98": 1.82, "e10": 1.68},
            {"date": "2025-01-01", "gazole": 1.65, "sp95": 1.78, "sp98": 1.88, "e10": 1.74},
            {"date": "2025-04-01", "gazole": 1.60, "sp95": 1.74, "sp98": 1.84, "e10": 1.70},
            {"date": "2025-07-01", "gazole": 1.58, "sp95": 1.70, "sp98": 1.80, "e10": 1.66},
            {"date": "2025-10-01", "gazole": 1.58, "sp95": 1.72, "sp98": 1.82, "e10": 1.68},
            {"date": "2026-01-01", "gazole": 1.62, "sp95": 1.71, "sp98": 1.81, "e10": 1.67},
            {"date": "2026-03-01", "gazole": 1.65, "sp95": 1.72, "sp98": 1.82, "e10": 1.68},
        ]
        source_historique = "Valeurs statiques (fallback)"

    # ──────────────────────────────────────────────────────────────
    # CALCULS FINAUX
    # ──────────────────────────────────────────────────────────────
    # Trier et dédupliquer par date
    seen = set()
    evolution_quotidien = [e for e in sorted(evolution_quotidien, key=lambda x: x['date'])
                           if e['date'] not in seen and not seen.add(e['date'])]

    dernier = evolution_quotidien[-1]

    # Prix actuels (flux instantané en priorité, sinon dernier point historique)
    prix_sp95   = prix_jour.get('sp95')   or dernier.get('sp95')   or 1.72
    prix_gazole = prix_jour.get('gazole') or dernier.get('gazole') or 1.65
    prix_sp98   = prix_jour.get('sp98')   or dernier.get('sp98')   or round(prix_sp95 + 0.10, 3)
    prix_e10    = prix_jour.get('e10')    or dernier.get('e10')    or round(prix_sp95 - 0.03, 3)

    def calc_var(serie_key, prix_actuel, jours_ref):
        """Calcule la variation sur jours_ref jours."""
        pts = [e for e in evolution_quotidien if e.get(serie_key) is not None]
        if len(pts) < 2:
            return 0.0
        idx_ref = max(0, len(pts) - jours_ref)
        ref = pts[idx_ref].get(serie_key)
        if not ref:
            return 0.0
        return normalize_zero(round(((prix_actuel / ref) - 1) * 100, 1))

    var_an_sp95    = calc_var('sp95',   prix_sp95,   365)
    var_sem_sp95   = calc_var('sp95',   prix_sp95,   7)
    var_an_gazole  = calc_var('gazole', prix_gazole, 365)
    var_sem_gazole = calc_var('gazole', prix_gazole, 7)

    # Date de dernière donnée
    derniere_donnee = prix_jour.get('derniere_maj') or dernier['date']

    # Pour le graphique : garder tous les points si quotidien < 90 jours,
    # sinon échantillonner pour ne pas surcharger le JSON (~1 point/semaine sur l'historique long)
    if len(evolution_quotidien) <= 90:
        evolution_graph = evolution_quotidien
    else:
        # Garder tous les points des 60 derniers jours + 1 par semaine avant
        recent = evolution_quotidien[-60:]
        older  = evolution_quotidien[:-60][::7]
        evolution_graph = older + recent

    granularite = "quotidien" if source_historique and "flux annuel" in source_historique else "mensuel"

    print(f"  ✓ Carburants : SP95 {prix_sp95}€/L (var. sem. {var_sem_sp95:+.1f}%, an {var_an_sp95:+.1f}%)")
    print(f"               Gazole {prix_gazole}€/L (var. sem. {var_sem_gazole:+.1f}%, an {var_an_gazole:+.1f}%)")
    print(f"  📅 Dernière donnée : {derniere_donnee} | {len(evolution_graph)} points graphique")

    return {
        "gazole": {
            "prix":        prix_gazole,
            "variation_an": var_an_gazole,
            "variation_sem": var_sem_gazole,
        },
        "sp95": {
            "prix":        prix_sp95,
            "variation_an": var_an_sp95,
            "variation_sem": var_sem_sp95,
        },
        "sp98": {
            "prix":        prix_sp98,
            "variation_an": var_an_sp95,
        ...
        "e10": {
            "prix":        prix_e10,
            "variation_an": calc_var('e10', prix_e10, 365),
            "variation_sem": calc_var('e10', prix_e10, 7),
        },
        "evolution":       evolution_graph,
        "derniere_donnee": derniere_donnee,
        "source":          source_historique or "Indéterminée",
