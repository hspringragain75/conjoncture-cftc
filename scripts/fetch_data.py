...
def build_conventions_collectives_enrichies(smic_net, smic_brut):
    ...
    # Construire un index IDCC → mtime depuis l'index kali-data (déjà chargé en étape 1)
    # Structure réelle d'une entrée kali-data index.json :
    # { "id": "KALICONT...", "mtime": 1556652289, "num": "1486",
    #   "texte_de_base": "KALITEXT...", "effectif": 857061, ... }
    # On indexe par num (IDCC sans zfill) → mtime.
    # Plus besoin de KALITEXT_MAP — l'IDCC suffit comme clé.
    idcc_mtime = {}
    try:
        ...
        for entry in kali_idx:
            ...
            if num and mtime:
                # Stocker avec et sans zfill pour matcher les deux formats
                idcc_mtime[num.zfill(4)] = int(mtime)
                idcc_mtime[num.lstrip("0") or "0"] = int(mtime)
        # Debug : afficher la structure de la première entrée
        if kali_idx:
            sample = kali_idx[0]
            print(f"  🔍 Structure index kali-data (1ère entrée) : clés={list(sample.keys())}")
            print(f"     num={sample.get('num')!r} mtime={sample.get('mtime')!r} id={str(sample.get('id',''))[:30]!r}")
        print(f"  📅 kali-data mtime chargé : {len(idcc_mtime)//2} CCN avec date")
    except Exception as e:
        print(f"  ⚠️  kali-data mtime indisponible ({e}) — détection avenants désactivée")

    # Comparer mtime kali-data vs derniere_revalorisation codée en dur
    ...
    if idcc_mtime:
        ...
        for branch in BRANCHES_BASE:
            ...
            if (annee_kali, mois_kali) > (annee_codee, mois_codee):
                ...
                nb_ok += 1
        print(f"  📊 Avenants : {nb_alertes} alerte(s) · {nb_ok} à jour · {nb_inconnu} non trouvés")
        if nb_alertes:
        ...
        print("  ℹ️  Détection avenants désactivée (kali-data mtime indisponible)")
