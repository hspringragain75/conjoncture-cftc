...
def build_emplois_vacants_data():
    ...
    # Cascade de noms de datasets (DARES renomme parfois ses datasets)
    _vacants_ids = [
    ...
    _occupes_ids = [
        ...
        "dares_emploisoccupes",
    ]
    vacants_raw = next((r for ds in _vacants_ids for r in [fetch_dares_dataset(ds, limit=100)] if r), None)
    occupes_raw = next((r for ds in _occupes_ids for r in [fetch_dares_dataset(ds, limit=100)] if r), None)
...
def fetch_insee_series(series_id, start_period="2015"):
    ...
    3 tentatives : timeout 10s → 20s → 30s, pause 3s puis 6s entre chaque.
    Le délai fixe évite le "connection reset by peer" causé par les appels en rafale.
    """
    ...
    # Délai fixe avant chaque appel pour éviter le rate-limit INSEE
    _time.sleep(1)
...
def build_marches_financiers_data():
    ...
    # Source 1 : BCE data-api (nouvelle URL, remplace sdw-wsrest.ecb.europa.eu mort)
    try:
    ...
    # ^TNX cote en dixièmes (ex: 42.5 = 4.25%). OAT FR ≈ TNX - 1.2%
    if not oat_data:
        try:
    ...
    # → FRED PNICKUSDM (LME Nickel $/tonne mensuel, World Bank/IMF, stable)
    nickel_data = []
    ...
    if not nickel_data:
        _, nickel_source = fetch_lme_metal("NI=F", (15500, 800, 40), "Nickel (fallback statique)")
        nickel_data = _
