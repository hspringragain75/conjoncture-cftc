...
def build_carburants_data():
    ...
    default_carburants = {
        "gazole": {"prix": 1.65, "variation_an": -3.5},
        "sp95": {"prix": 1.72, "variation_an": -5.0},
        "sp98": {"prix": 1.82, "variation_an": -4.5},
        "evolution": [
            ...
            {"mois": "Juil 2025", "gazole": 1.58, "sp95": 1.70},
            {"mois": "Oct 2025", "gazole": 1.58, "sp95": 1.72},
            {"mois": "Jan 2026", "gazole": 1.62, "sp95": 1.71},
            {"mois": "Mar 2026", "gazole": 1.65, "sp95": 1.72},
        ]
    }
...
def build_marches_financiers_data():
    ...
    return {
        ...
        "analyse": {
            "description": "Donnees quotidiennes sur 1 an avec moyennes mobiles 50j/200j, variations multi-horizons et correlation inflation. 8 indicateurs suivis.",
            "periode": f"{oat_data[0]['date'] if oat_data else 'N/A'} - {oat_data[-1]['date'] if oat_data else 'N/A'}"
        },
        "sources": f"FRED ({oat_source}, Brent, EUR/USD, Or, Gaz), Yahoo Finance (CAC 40, matieres premieres), Banque de France (taux immo), INSEE (IPC)"
    }
...
def build_revue_presse():
    ...
    # Sources vérifiées comme fonctionnelles (RSS accessible sans bot-blocking)
    # Sources institutionnelles FR abandonnées (SPA JS, bot-blocking, 404) :
    # INSEE (500), DARES (SPA), BdF (404), MinTravail (SPA), FranceTravail (SPA),
    # UNEDIC (404), CComptes (timeout), OFCE (404), LesEchos (403), OCDE (403)
    SOURCES = [
        # ── PRESSE ÉCONOMIQUE (RSS ouverts) ──────────────────────────────────
        {
            "nom": "Le Monde Éco",
        ...
        {
            "nom": "L'Humanité Social",
            "url": "https://www.humanite.fr/rss.xml",
            "couleur": "#b91c1c",
            "emoji": "✊",
            "tag": "Social & travail",
        },
        # ── THINK TANKS & RECHERCHE (WordPress → /feed/) ─────────────────────
        {
            "nom": "Terra Nova",
        ...
        {
            "nom": "La Croix Social",
            "url": "https://www.la-croix.com/rss/economie.rss",
            "couleur": "#065f46",
            "emoji": "⚖️",
            "tag": "Social & travail",
        },
        # ── EUROPE (RSS encore actif) ─────────────────────────────────────────
        {
            "nom": "Euractiv FR",
        ...
        # ── ALTERNATIVES AUX SOURCES INSTITUTIONNELLES ───────────────────────
        # L'INSEE publie ses communiqués de presse via service-public.fr qui a un RSS actif
        {
            "nom": "Service-Public.fr",
            "url": "https://www.service-public.fr/rss/particuliers.xml",
            "couleur": "#1e40af",
            "emoji": "🏛️",
            "tag": "Droit & réglementation",
        },
        # INSEE données : flux RSS SDMX (mises à jour séries)
        {
            "nom": "INSEE Données",
            "url": "https://www.insee.fr/rss/donnees",
            "couleur": "#1e40af",
            "emoji": "📊",
            "tag": "Statistiques nationales",
        },
        # BNP Paribas Economic Research a un RSS ouvert avec contenu macro France
        {
            "nom": "BNP Research",
            "url": "https://economic-research.bnpparibas.com/RSS/fr-FR",
            "couleur": "#16a34a",
            "emoji": "🏦",
            "tag": "Conjoncture & finances",
        },
    ]
    ...
    def fetch_rss(source):
        """Récupère et parse le flux RSS d'une source — parser tolérant multi-format."""
        headers = {
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=15) as resp:
                raw = resp.read()
                # Détecter l'encodage : header HTTP > déclaration XML > fallback utf-8
                encoding = resp.headers.get_content_charset()
                if not encoding:
                    # Chercher <?xml encoding="..." ?> dans les premiers octets
                    m = re.search(rb'encoding=["\']([^"\']+)["\']', raw[:200])
                    encoding = m.group(1).decode('ascii') if m else "utf-8"
                content = raw.decode(encoding, errors="replace")

        except Exception as e:
            print(f"  ⚠️  {source['nom']} ({source['url'][:50]}) : {str(e)[:80]}")
            return []

        # --- Parser XML tolérant ---
        # Certains flux ont des namespaces complexes — on les supprime pour simplifier
        content_clean = re.sub(r' xmlns[^=]*="[^"]*"', '', content)
        content_clean = re.sub(r'<([a-z]+):[a-z]', lambda m: '<' + m.group(0)[1:].replace(':', '_'), content_clean)

        try:
            root = ET.fromstring(content_clean.encode('utf-8', errors='replace'))
        except ET.ParseError:
            # Dernier recours : extraction regex directe sans XML
            print(f"  ⚠️  {source['nom']} : XML invalide, extraction regex")
            return _extract_regex(content, source, nettoyer_html, parse_date, score_pertinence, MOTS_CLES_NAO)

        articles_raw = []

        # Détecter le format : Atom (feed/entry) ou RSS (channel/item)
        tag = root.tag.lower()
        is_atom = "feed" in tag
        ...
        if is_atom:
            ...
            for entry in entries[:20]:
                def geta(t):
                    el = entry.find(t) or entry.find(f"atom_{t}")
                    if el is None:
                        # Cherche avec namespace partiel
                        for child in entry:
                            if child.tag.split('}')[-1] == t:
                                return child.text or ""
                    return (el.text or "") if el is not None else ""

                titre = nettoyer_html(geta("title"))
                resume = nettoyer_html(geta("summary") or geta("content"))
                # Lien : attribut href sur <link> ou texte
                lien_el = entry.find("link")
                if lien_el is not None:
                    lien = lien_el.get("href") or lien_el.text or ""
                else:
                    lien = ""
                date_str = geta("published") or geta("updated")
                articles_raw.append((titre, resume, lien, date_str))
        else:
            # Format RSS 2.0
            ...
            for item in items[:20]:
                ...
                titre    = nettoyer_html(getr("title"))
                resume   = nettoyer_html(getr("description") or getr("summary"))
                lien     = getr("link") or getr("guid")
                # pubDate peut être dans dc:date ou pubDate
                date_str = getr("pubDate") or getr("date") or getr("dc_date")
                articles_raw.append((titre, resume, lien, date_str))

        # Construire les objets articles finaux
        result = []
        for titre, resume, lien, date_str in articles_raw:
    ...
    def _extract_regex(content, source, nettoyer_html, parse_date, score_pertinence, MOTS_CLES_NAO):
        """Extraction RSS/Atom par regex — fallback si XML invalide."""
        result = []
        # Essayer items RSS
        pattern_item = re.compile(r'<item[^>]*>(.*?)</item>', re.DOTALL | re.IGNORECASE)
        pattern_entry = re.compile(r'<entry[^>]*>(.*?)</entry>', re.DOTALL | re.IGNORECASE)

        def extract_tag(block, tag):
            m = re.search(rf'<{tag}[^>]*>(.*?)</{tag}>', block, re.DOTALL | re.IGNORECASE)
            return m.group(1).strip() if m else ""

        blocks = pattern_item.findall(content) or pattern_entry.findall(content)
        for block in blocks[:20]:
            titre    = nettoyer_html(extract_tag(block, "title"))
            resume   = nettoyer_html(extract_tag(block, "description") or extract_tag(block, "summary"))
            lien     = nettoyer_html(extract_tag(block, "link") or extract_tag(block, "guid"))
            date_str = extract_tag(block, "pubDate") or extract_tag(block, "published") or extract_tag(block, "updated")
            if not titre or len(titre) < 5:
                continue
            date_obj = parse_date(date_str)
            result.append({
