...
def build_emplois_vacants_data():
    ...
    def fetch_dares_aggregated(dataset_id, value_field, taux_field=None):
        """
        Récupère les données DARES agrégées par trimestre via group_by.
        Retourne directement le total national par trimestre.
        """
        import urllib.parse
        select = f"date, sum({value_field}) as total"
        if taux_field:
            select += f", avg({taux_field}) as taux_moy"
        ...
        url = f"{DARES_BASE_URL}/catalog/datasets/{dataset_id}/records?{params}"
        try:
            req = urllib.request.Request(url, headers={
                "Accept": "application/json", "User-Agent": "CFTC-Dashboard/2.0"
            })
