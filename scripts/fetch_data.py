...
def build_emplois_vacants_data():
    ...
    def fetch_dares_aggregated(dataset_id, value_field, taux_field=None):
        """
        Récupère les données DARES agrégées par trimestre via group_by + sum().
        URL construite manuellement pour éviter les problèmes d'encodage urlencode.
        """
        base = f"{DARES_BASE_URL}/catalog/datasets/{dataset_id}/records"
        # Construire select
        sel = f"date,sum({value_field})%20as%20total"
        if taux_field:
            sel += f",avg({taux_field})%20as%20taux_moy"
        # URL avec encodage manuel correct pour l'API OpenDataSoft
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=30) as response:
                ...
                results = data.get("results", [])
                if results:
                    print(f"  ✅ DARES {dataset_id}: {len(results)} trimestres agrégés")
                return results if results else None
        except urllib.error.HTTPError as e:
            print(f"  ⚠️ HTTP {e.code} DARES aggregated {dataset_id}")
            return None
        except Exception as e:
            print(f"  ⚠️ Erreur DARES aggregated {dataset_id}: {e}")
            return None

    vacants_agg = fetch_dares_aggregated(
        "dares_emploivacants_brut_emploisvacants",
        value_field="nombre_d_emplois_vacants",
        taux_field="taux_d_emplois_vacants_en"
    )
    occupes_agg = fetch_dares_aggregated(
        "dares_emploivacants_brut_emploisoccupes",
        value_field="nombre_d_emplois_occupes"
    )
    vacants_raw = vacants_agg
    occupes_raw = occupes_agg
    ...
    # Parser les résultats group_by (1 ligne par trimestre, totaux exacts)
    def parse_aggregated(records, val_field="total", taux_field="taux_moy"):
        result = []
        for record in records:
            ...
            if "-T" in date:
                ...
                trimestre = date
            try:    total = float(record[val_field]) if record.get(val_field) is not None else None
            except: total = None
            try:    taux  = round(float(record[taux_field]), 2) if record.get(taux_field) is not None else None
            except: taux  = None
            result.append({"trimestre": trimestre, "total": total, "taux": taux})
        def _tri(x):
            try:
                t, y = x["trimestre"].split()
                return (int(y), int(t[1]))
            except: return (0, 0)
        return sorted(result, key=_tri)

    if not vacants_raw:
        print("  ⚠️ Utilisation des données par défaut")
        return default_data

    vacants_parsed = parse_aggregated(vacants_raw, "total", "taux_moy")
    occupes_parsed = parse_aggregated(occupes_raw, "total", None) if occupes_raw else []

    if not vacants_parsed:
        print("  ⚠️ Utilisation des données par défaut")
        return default_data

    emplois_vacants = [{"trimestre": p["trimestre"], "valeur": round(p["total"]) if p["total"] else 0, "secteur": "Ensemble"} for p in vacants_parsed]
    oc_dict = {p["trimestre"]: round(p["total"]) for p in occupes_parsed if p["total"]}
    emplois_occupes = [{"trimestre": p["trimestre"], "valeur": oc_dict.get(p["trimestre"], 20250000), "secteur": "Ensemble"} for p in vacants_parsed]
    taux_vacance    = [{"trimestre": p["trimestre"], "taux": p["taux"]} for p in vacants_parsed if p["taux"] is not None]

    print(f"  ✓ {len(emplois_vacants)} trimestres d'emplois vacants récupérés (totaux agrégés)")
    if emplois_vacants:
        last = emplois_vacants[-1]
        print(f"  ✓ Dernier: {last['trimestre']} → {last['valeur']:,} postes vacants")
    ...
    if emplois_vacants:
        return {
            ...
            "source": "DARES - Enquête ACEMO (postes vacants agrégés)",
            "derniere_maj": emplois_vacants[-1]["trimestre"] if emplois_vacants else "N/A",
        }
