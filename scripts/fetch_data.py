...
def build_conventions_collectives_enrichies(smic_net, smic_brut):
    ...
    def get_dernier_avenant_salaires(kalicont_id, token):
        ...
        Utilise /search avec fond=KALI et filtre sur l'idConteneur,
        ce qui est l'endpoint PISTE documenté fonctionnel pour les CCN.
        ...
        payload = json.dumps({
            "recherche": {
                "champs": [{"typeChamp": "TITRE", "criteres": [{"typeRecherche": "TOUS_LES_MOTS_DANS_UN_CHAMP", "valeur": "salaire minima revalorisation"}], "operateur": "OU"}],
                "filtres": [
                    {"facette": "NATURE", "valeur": "TEXTES_SALAIRES"},
                    {"facette": "ID_CONTENEUR", "valeur": kalicont_id},
                ],
        ...
        try:
            ...
            body = _he.read().decode("utf-8", errors="replace")[:300]
            raise RuntimeError(f"HTTP {_he.code} — body: {body}")

        # Extraire les dates des résultats
        dates = []
        for item in result.get("results", []):
            for t in item.get("titles", []):
                d = (t.get("datePubliTexte") or t.get("dateSignatureTexte")
                     or t.get("dateDebut") or t.get("date") or "")
                if d and len(d) >= 7:
                    dates.append(d[:7])
            # Aussi au niveau racine
            d = (item.get("datePubliTexte") or item.get("dateSignatureTexte")
                 or item.get("dateDebut") or item.get("date") or "")
            if d and len(d) >= 7:
                dates.append(d[:7])
