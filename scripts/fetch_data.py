...
def fetch_insee_series(series_id, start_period="2015"):
    ...
    Pause de 2s entre chaque tentative pour laisser l'API récupérer.
    """
    ...
    for attempt, timeout in enumerate(timeouts, 1):
        try:
            ...
            print(f"  ⚠️ HTTP {e.code} pour série {series_id} (tentative {attempt}/{len(timeouts)})")
            if e.code in (400, 404, 410):
                break  # Pas la peine de réessayer si la série n'existe pas
        except urllib.error.URLError as e:
            reason = str(e.reason) if hasattr(e, "reason") else str(e)
            print(f"  ⚠️ Erreur réseau série {series_id} (tentative {attempt}/{len(timeouts)}): {reason}")
        except Exception as e:
            print(f"  ⚠️ Erreur inattendue série {series_id} (tentative {attempt}/{len(timeouts)}): {e}")

        if attempt < len(timeouts):
            _time.sleep(2)  # Pause avant retry

    print(f"  ⚠️ Série {series_id} indisponible après {len(timeouts)} tentatives — données par défaut")
    return None
...
def build_marches_financiers_data():
    ...
    # Source 1 : stooq.com — taux OAT 10 ans FR (10YFRB.B), quotidien, sans quota
    try:
        print("    -> OAT 10 ans : stooq.com (10YFRB.B)...")
        stooq_url = f"https://stooq.com/q/d/l/?s=10yfrb.b&d1={one_year_ago.replace('-','')}&d2={dt_local.now().strftime('%Y%m%d')}&i=d"
        resp = safe_urlopen(stooq_url, timeout=15)
        ...
        for line in lines[1:]:
            ...
            if len(parts) >= 5 and parts[4].strip() not in ("null", "N/D", ""):
                try:
                    v = float(parts[4].strip())   # colonne "Close"
                    oat_data.append({"date": parts[0].strip(), "valeur": round(v, 3)})
                except (ValueError, TypeError):
                    pass
        if oat_data:
            oat_source = "stooq.com (quotidien)"
            print(f"    OAT stooq: {len(oat_data)} points, dernier={oat_data[-1]['valeur']}% ({oat_data[-1]['date']})")
    except Exception as e:
        print(f"    stooq indisponible: {e}")

    # Source 2 : FRED — série IRL10Y (remplace IRLTLT01FRD156N désormais 404)
    if not oat_data:
        try:
            print("    -> OAT 10 ans : FRED (IRL10Y)...")
            fred_url = f"https://fred.stlouisfed.org/graph/fredgraph.csv?id=IRL10Y&cosd={one_year_ago}"
    ...
    # Nickel : NI=F (CME) → NI.F (stooq) → statique (LME ~15500 $/t)
    nickel_data, nickel_source = fetch_lme_metal("NI%3DF", (15500, 1500, 35), "Nickel")
    if not nickel_data or nickel_source == "Valeur statique (fallback)":
        # Fallback stooq : NI.F (contrat LME nickel)
        try:
            one_year_ago_ni = (dt_local.now() - timedelta(days=400)).strftime("%Y%m%d")
            stooq_ni = f"https://stooq.com/q/d/l/?s=ni.f&d1={one_year_ago_ni}&d2={dt_local.now().strftime('%Y%m%d')}&i=d"
            resp_ni = safe_urlopen(stooq_ni, timeout=15)
            csv_ni = resp_ni.read().decode("utf-8")
            lines_ni = csv_ni.strip().split("\n")
            ni_tmp = []
            for line in lines_ni[1:]:
                parts = line.split(",")
                if len(parts) >= 5 and parts[4].strip() not in ("null", "N/D", ""):
                    try:
                        v = float(parts[4].strip())
                        ni_tmp.append({"date": parts[0].strip(), "valeur": round(v, 2)})
                    except (ValueError, TypeError):
                        pass
            if ni_tmp:
                nickel_data = ni_tmp
                nickel_source = "stooq.com/NI.F (quotidien)"
                print(f"    Nickel stooq: {len(nickel_data)} points, dernier={nickel_data[-1]['valeur']} ({nickel_data[-1]['date']})")
        except Exception as e_ni:
            print(f"    Nickel stooq indisponible: {e_ni}")
