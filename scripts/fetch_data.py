...
def build_emplois_vacants_data():
    ...
    emplois_vacants = [{"trimestre": p["trimestre"],
                        "valeur": round(p["somme_ev"]),
                        "secteur": "Ensemble"} for p in parsed]
    emplois_occupes = [{"trimestre": p["trimestre"],
                        "valeur": round(p["somme_oc"]),
                        "secteur": "Ensemble"} for p in parsed]
    taux_vacance    = [{"trimestre": p["trimestre"],
                        "taux":   round(p["somme_taux"] / p["nb"], 2)} for p in parsed]

    print(f"  ✓ {len(taux_vacance)} trimestres d'emplois vacants (taux moyen)")
    if taux_vacance:
        print(f"  ✓ Dernier: {taux_vacance[-1]['trimestre']} → taux={taux_vacance[-1]['taux']}%")
