    ...
    if idcc_kalitext:
        ...
        for branch in BRANCHES_BASE:
            ...
            if not ktext:
                nb_err += 1
                if nb_err <= 5:
                    print(f"  ⚠️  IDCC {idcc} absent de kali-data index")
                continue
            ...
            try:
                ...
                nb_err += 1
                print(f"  ⚠️  GitHub API erreur IDCC {idcc}: {type(_gh_err).__name__}: {str(_gh_err)[:120]}")
                if nb_err >= 3:
                    print("  ⚠️  Trop d'erreurs GitHub API — abandon")
                    break
