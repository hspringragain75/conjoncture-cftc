...
def build_conventions_collectives_enrichies(smic_net, smic_brut):
    ...
    KALITEXT_MAP = {
        "3248": "KALITEXT000044022682",  # Métallurgie
        "1518": "KALITEXT000005679933",  # Aide à domicile (particuliers)
        "1979": "KALITEXT000005679929",  # HCR
        "1486": "KALITEXT000005679895",  # SYNTEC
        "2402": "KALITEXT000005679957",  # Travail temporaire
        "1090": "KALITEXT000005679883",  # GMS
        "3043": "KALITEXT000027887802",  # Propreté
        "1459": "KALITEXT000005679909",  # Nettoyage industriel
        "1996": "KALITEXT000005679988",  # Transports routiers
        "2941": "KALITEXT000022946969",  # BAD
        "3239": "KALITEXT000005679924",  # Banque (AFB)
        "2120": "KALITEXT000005679975",  # Commerce non alimentaire
        "1597": "KALITEXT000005679984",  # Bâtiment ouvriers
        "1605": "KALITEXT000005679985",  # Bâtiment ETAM
        "2216": "KALITEXT000005679997",  # Commerce détail alimentaire spécialisé
        "0803": "KALITEXT000005679876",  # Restauration rapide
        "2098": "KALITEXT000005679969",  # FHP
        "1501": "KALITEXT000005679986",  # Travaux publics ouvriers
        "1404": "KALITEXT000005679903",  # Animation
        "1351": "KALITEXT000005679899",  # Sécurité privée
        "2569": "KALITEXT000005679971",  # Services automobile
        "0736": "KALITEXT000005679867",  # FEHAP
        "2596": "KALITEXT000005679978",  # Immobilier
        "2150": "KALITEXT000005679970",  # Industrie chimique
        "0787": "KALITEXT000005679872",  # Boulangerie artisanale
        "3131": "KALITEXT000005679932",  # Télécommunications
        "1285": "KALITEXT000005679896",  # Habillement textile
        "0650": "KALITEXT000005679864",  # Pharmacie d'officine
        "0095": "KALITEXT000005679857",  # Commerce gros alimentaire
        "2609": "KALITEXT000005679979",  # Coiffure
        "1483": "KALITEXT000005679907",  # Mutualité
        "2511": "KALITEXT000005679968",  # Sport
        "2264": "KALITEXT000005679996",  # Distribution directe
        "1424": "KALITEXT000005679904",  # Cabinets médicaux
        "0292": "KALITEXT000005679856",  # Plasturgie
        "0044": "KALITEXT000005679855",  # Industries alimentaires
        "2691": "KALITEXT000005679982",  # Paysagistes
        "0675": "KALITEXT000005679863",  # Papiers-cartons
        "3016": "KALITEXT000005679921",  # Organismes de formation
        "2148": "KALITEXT000005679969",  # Agences d'assurances
        "0016": "KALITEXT000005679851",  # Transports aériens
        "0158": "KALITEXT000005679858",  # Imprimerie
        "0979": "KALITEXT000005679881",  # Hôtellerie de plein air
        "3230": "KALITEXT000027572690",  # ELISFA
        "1258": "KALITEXT000005679894",  # Textiles synthétiques
        "1043": "KALITEXT000005679886",  # Chaussure succursaliste
        "2785": "KALITEXT000005679983",  # Avocats salariés
        "0086": "KALITEXT000005679860",  # Navigation fluviale
    }
    ...
    # Construire un index KALITEXT → mtime depuis l'index kali-data déjà chargé
    # (kali_index est déjà en mémoire depuis l'étape 1)
    kalitext_mtime = {}
    try:
        # kali_index est la liste chargée dans effectifs_api mais on doit recharger
        # car on n'a gardé que les effectifs. On refait un appel léger sur le même URL.
        ...
        for entry in kali_idx:
            ktext = entry.get("id") or entry.get("texte_de_base") or ""
            mtime = entry.get("mtime")  # timestamp Unix (secondes)
            if ktext and mtime:
                kalitext_mtime[ktext] = int(mtime)
        print(f"  📅 kali-data mtime chargé : {len(kalitext_mtime)} entrées")
    except Exception as e:
        print(f"  ⚠️  kali-data mtime indisponible ({e}) — détection avenants désactivée")

    # Comparer mtime kali-data vs derniere_revalorisation codée
    alertes_avenants = {}
    if kalitext_mtime:
        nb_alertes = 0
        nb_ok = 0
        nb_inconnu = 0
        for idcc, ktext_id in KALITEXT_MAP.items():
            ...
            if annee_codee == 0:
                nb_inconnu += 1
                continue
            mtime = kalitext_mtime.get(ktext_id)
            if mtime is None:
                nb_inconnu += 1
                continue
            # Convertir mtime en (annee, mois)
            from datetime import datetime as _dt, timezone as _tz
            dt_kali = _dt.fromtimestamp(mtime, tz=_tz.utc)
            annee_kali, mois_kali = dt_kali.year, dt_kali.month
            if (annee_kali, mois_kali) > (annee_codee, mois_codee):
                mois_nom = [k for k, v in MOIS_FR.items() if v == mois_kali]
                label = f"{mois_nom[0].capitalize()} {annee_kali}" if mois_nom else f"{annee_kali}-{mois_kali:02d}"
                alertes_avenants[idcc] = {
                    "avenant_detecte": label,
                    "grille_codee":    date_codee,
                    "message":         f"⚠️ CCN modifiée ({label}) — grille à vérifier sur Légifrance",
                }
                nb_alertes += 1
            else:
                nb_ok += 1
        print(f"  📊 Avenants : {nb_alertes} alerte(s) · {nb_ok} à jour · {nb_inconnu} non trouvés dans kali-data")
        if nb_alertes:
            print(f"  ⚠️  Branches dont la CCN a été modifiée après la grille codée :")
            for idcc, alerte in alertes_avenants.items():
                nom = next((b["nom"] for b in BRANCHES_BASE if b["idcc"] == idcc), idcc)
                print(f"      IDCC {idcc} {nom} — kali-data {alerte['avenant_detecte']} > grille {alerte['grille_codee']}")
    else:
        print("  ℹ️  Détection avenants désactivée (kali-data index indisponible)")
