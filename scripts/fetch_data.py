...
def build_marches_financiers_data():
    ...
    if not brent_data:
        ...
        try:
            ...
            oil_result = json.loads(resp.read().decode("utf-8"))
            if "data" in oil_result and "price" in oil_result["data"]:
                prix = round(oil_result["data"]["price"], 2)
                brent_data = [{"date": dt_local.now().strftime("%Y-%m-%d"), "valeur": prix}]
                brent_source = "OilPriceAPI (dernier prix)"
                print(f"    Brent OilPriceAPI: {prix} USD")
        except Exception as e:
    ...
    inflation_mensuelle = [
        ...
        {"date": "2025-07", "valeur": 0.9}, {"date": "2025-08", "valeur": 1.0},
        {"date": "2025-09", "valeur": 1.1}, {"date": "2025-10", "valeur": 1.0},
        {"date": "2025-11", "valeur": 1.1}, {"date": "2025-12", "valeur": 1.2},
        {"date": "2026-01", "valeur": 1.0}, {"date": "2026-02", "valeur": 0.8},
    ]
    ...
    print(f"    Correlations inflation -> OAT:{corr_oat} Brent:{corr_brent} CAC:{corr_cac}")
    ...
    return {
        ...
        "analyse": {
            "description": "Donnees quotidiennes sur 1 an avec moyennes mobiles 50j/200j, variations multi-horizons et correlation inflation.",
            "periode": f"{oat_data[0]['date'] if oat_data else 'N/A'} - {oat_data[-1]['date'] if oat_data else 'N/A'}"
        },
        "sources": f"FRED ({oat_source}), Yahoo Finance (Brent, CAC 40), INSEE (IPC)"
    }
