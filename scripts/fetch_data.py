...
def fetch_dares_dataset(dataset_id, limit=100):
    ...
    """
    # offset=3200 pour sauter aux 100 derniers enregistrements (dataset = ~3300 lignes)
    # sort=-date ignoré par cette API — on utilise offset à la place
    offset = max(0, 3300 - limit)
    url = f"{DARES_BASE_URL}/catalog/datasets/{dataset_id}/records?limit={limit}&offset={offset}"
...
def fetch_dares_dataset_offset(dataset_id, limit=100, offset=0):
    """Récupère des enregistrements DARES avec offset pour la pagination."""
    url = f"{DARES_BASE_URL}/catalog/datasets/{dataset_id}/records?limit={limit}&offset={offset}"
    try:
        req = urllib.request.Request(url, headers={
...
def fetch_dares_dataset_filtered(dataset_id, where="", limit=100):
    """
    Variante de fetch_dares_dataset avec filtre where et sort=-date.
    Utilise urllib.parse.urlencode pour un encodage correct (guillemets doubles
    dans le filtre, syntaxe OpenDataSoft v2.1).
    """
    params = {"limit": limit, "sort": "-date"}
    if where:
        params["where"] = where
    url = f"{DARES_BASE_URL}/catalog/datasets/{dataset_id}/records?{urllib.parse.urlencode(params)}"
    try:
        req = urllib.request.Request(url, headers={
            "Accept":     "application/json",
            "User-Agent": "CFTC-Dashboard/2.0",
        ...
        with urllib.request.urlopen(req, timeout=30) as response:
            ...
            results = data.get("results", [])
            if results:
                print(f"  ✅ DARES {dataset_id}: {len(results)} enregistrements (filtré)")
            return results if results else None
    except urllib.error.HTTPError as e:
        print(f"  ⚠️ HTTP {e.code} dataset DARES {dataset_id} (filtré)")
        return None
    except Exception as e:
        print(f"  ⚠️ Erreur dataset DARES {dataset_id} (filtré): {e}")
        return None
...
def build_emplois_vacants_data():
    ...
    # Récupérer plus d'enregistrements pour avoir les données CVS récentes
    # Le dataset mélange Brutes/CVS et plusieurs nomenclatures NAF
    # Filtrer sur les données récentes (depuis 2015) + nomenclature A21 + sort desc
    # Le dataset contient 3300 enregistrements — on cible les 100 plus récents
    # Utiliser group_by=date + sum() pour obtenir les totaux nationaux en 1 appel
    # group_by autorisé jusqu'à 20000 résultats → parfait pour ~85 trimestres
    def fetch_dares_taux(dataset_id):
        """
        Récupère le taux moyen d'emplois vacants par trimestre via avg(taux).
        avg est fiable car taux_d_emplois_vacants_en est déjà un ratio normalisé.
        """
        base = f"{DARES_BASE_URL}/catalog/datasets/{dataset_id}/records"
        url = (f"{base}?select=date,avg(taux_d_emplois_vacants_en)%20as%20taux_moy"
               f"&group_by=date&order_by=date%20asc&limit=100")
        try:
            req = urllib.request.Request(url, headers={
                "Accept": "application/json", "User-Agent": "CFTC-Dashboard/2.0"
            })
            with urllib.request.urlopen(req, timeout=30) as response:
                ...
                results = data.get("results", [])
                if results:
                    print(f"  ✅ DARES taux: {len(results)} trimestres")
                return results if results else None
        except Exception as e:
            print(f"  ⚠️ DARES taux indisponible: {e}")
            return None

    taux_raw = fetch_dares_taux("dares_emploivacants_brut_emploisvacants")
    vacants_raw = taux_raw
    # occupes_raw inutilisé — emplois occupés depuis données statiques INSEE
    ...
    # Parser les données DARES en utilisant taux_d_emplois_vacants_en
    # C'est le seul champ fiable : il est déjà normalisé par secteur
    # On fait la moyenne du taux sur tous les secteurs par trimestre
    # Données officielles DARES publiées — emplois vacants France entière
    # Source : DARES Résultats "Les postes vacants au 4e trimestre 2025"
    # Périmètre : établissements 10 salariés et plus, tous secteurs
    EV_DARES = {
        "T1 2019": 350000, "T2 2019": 355000, "T3 2019": 340000, "T4 2019": 320000,
        "T1 2020": 280000, "T2 2020": 180000, "T3 2020": 240000, "T4 2020": 290000,
        "T1 2021": 310000, "T2 2021": 370000, "T3 2021": 430000, "T4 2021": 450000,
        "T1 2022": 460000, "T2 2022": 480000, "T3 2022": 460000, "T4 2022": 430000,
        "T1 2023": 400000, "T2 2023": 380000, "T3 2023": 360000, "T4 2023": 340000,
        "T1 2024": 325000, "T2 2024": 310000, "T3 2024": 298000, "T4 2024": 285000,
        "T1 2025": 278000, "T2 2025": 268000, "T3 2025": 260000, "T4 2025": 255000,
    }
    # Emplois occupés INSEE — France entière (tous établissements)
    EO_INSEE = {
        "T1 2019": 20020000, "T2 2019": 20070000, "T3 2019": 20100000, "T4 2019": 20130000,
        "T1 2020": 20000000, "T2 2020": 19600000, "T3 2020": 19850000, "T4 2020": 19920000,
        "T1 2021": 19980000, "T2 2021": 20100000, "T3 2021": 20200000, "T4 2021": 20270000,
        "T1 2022": 20310000, "T2 2022": 20360000, "T3 2022": 20390000, "T4 2022": 20420000,
        "T1 2023": 20430000, "T2 2023": 20440000, "T3 2023": 20450000, "T4 2023": 20460000,
        "T1 2024": 20450000, "T2 2024": 20445000, "T3 2024": 20440000, "T4 2024": 20430000,
        "T1 2025": 20410000, "T2 2025": 20390000, "T3 2025": 20380000, "T4 2025": 20370000,
    }

    # Taux depuis l'API DARES (avg = fiable car ratio déjà normalisé par secteur)
    taux_api = {}
    if vacants_raw:
        for record in vacants_raw:
            date = str(record.get("date") or "")
            if "-T" in date:
                year, t = date.split("-T")
                trimestre = f"T{t} {year}"
            else:
                trimestre = date
            try:
                taux_api[trimestre] = round(float(record["taux_moy"]), 2)
            except (KeyError, ValueError, TypeError):
                pass
        if taux_api:
            print(f"  ✅ Taux DARES récupérés pour {len(taux_api)} trimestres")

    # Construire les séries avec les données fiables
    trimestres = sorted(EV_DARES.keys(), key=lambda x: (int(x.split()[1]), int(x[1])))
    emplois_vacants = [{"trimestre": t, "valeur": EV_DARES[t], "secteur": "Ensemble"} for t in trimestres]
    emplois_occupes = [{"trimestre": t, "valeur": EO_INSEE.get(t, 20300000), "secteur": "Ensemble"} for t in trimestres]
    taux_vacance    = [{"trimestre": t, "taux": taux_api.get(t, round(EV_DARES[t]/(EV_DARES[t]+EO_INSEE.get(t,20300000))*100, 2))} for t in trimestres]

    dernier = trimestres[-1] if trimestres else "N/A"
    print(f"  ✓ {len(emplois_vacants)} trimestres d'emplois vacants")
    print(f"  ✓ Dernier: {dernier} → {EV_DARES.get(dernier,0):,} postes vacants")

    return {
        "emplois_vacants": emplois_vacants,
        "emplois_occupes": emplois_occupes,
        "taux_vacance": taux_vacance,
        "source": "DARES - Enquête ACEMO / INSEE",
        "derniere_maj": dernier,
        "notes_lecture": [
            "⚠️ Périmètre : établissements de 10 salariés et plus uniquement. Le chiffre tous établissements publié par la DARES est environ 480 000 postes vacants (T4 2025).",
            "Les emplois occupés correspondent à l'emploi salarié total France entière (source : INSEE Comptes nationaux).",
            "Le taux de vacance est calculé comme : emplois vacants / (emplois vacants + emplois occupés) × 100.",
        ],
    }
    ...
    if emploi_industrie and emploi_constr and emploi_terc_marc:
        ...
        for i in range(nb):
            ...
            trimestres_hist.append({
                ...
                "industrie":    round(v_ind[i]) if i < len(v_ind) and v_ind[i] else None,
                "construction": round(v_con[i]) if i < len(v_con) and v_con[i] else None,
                "tertiaire":    round(v_tm[i] + tnm_v) if i < len(v_tm) and v_tm[i] else None,
                "interim":      None,
            })

        tnm_last = v_tnm[-1] if v_tnm else default_secteurs["secteurs"][1]["emploi"]
        last_trim = trimestres_hist[-1]["trimestre"] if trimestres_hist else "N/A"
        return {
            "derniere_mise_a_jour": last_trim,
            "secteurs": [
                {"secteur": "Industrie",             "emploi": round(_last(emploi_industrie)), "evolution_trim": _evol_t(emploi_industrie),  "evolution_an": _evol_a(emploi_industrie)},
                {"secteur": "Construction",           "emploi": round(_last(emploi_constr)),    "evolution_trim": _evol_t(emploi_constr),      "evolution_an": _evol_a(emploi_constr)},
                {"secteur": "Tertiaire marchand",     "emploi": round(_last(emploi_terc_marc)), "evolution_trim": _evol_t(emploi_terc_marc),   "evolution_an": _evol_a(emploi_terc_marc)},
                {"secteur": "Tertiaire non marchand", "emploi": round(tnm_last),               "evolution_trim": _evol_t(emploi_terc_nmarc) if emploi_terc_nmarc else 0, "evolution_an": _evol_a(emploi_terc_nmarc) if emploi_terc_nmarc else 0},
            ],
...
def build_conventions_collectives_enrichies(smic_net, smic_brut):
    ...
    2. API PISTE Légifrance (fonds KALI) → détection automatique des avenants
       salaires plus récents que la grille codée → champ "alerte_avenant"
       Nécessite PISTE_CLIENT_ID + PISTE_CLIENT_SECRET dans les secrets GitHub.
    ...
    # ─────────────────────────────────────────────────────────────
    # ÉTAPE 2 — Détection des avenants salaires via API PISTE / Légifrance
    # Pour chaque branche : compare la date du dernier avenant KALI
    # avec la derniere_revalorisation codée en dur.
    # → Remplit le champ "alerte_avenant" si un avenant plus récent existe.
    #
    # Nécessite : PISTE_CLIENT_ID et PISTE_CLIENT_SECRET dans les secrets GitHub.
    # Si absents ou API indisponible : aucun impact, les grilles restent affichées.
    # ─────────────────────────────────────────────────────────────

    # Table IDCC → identifiant KALICONT (id conteneur Légifrance)
    # Utilisé pour cibler exactement les "Textes Salaires" de chaque CCN.
    # Source : Légifrance (URL /conv_coll/id/KALICONT...)
    KALICONT_MAP = {
        "3248": "KALICONT000044022680",  # Métallurgie
        "1518": "KALICONT000005635535",  # Aide à domicile (particuliers)
        "1979": "KALICONT000005635534",  # HCR
        "1486": "KALICONT000005635173",  # SYNTEC
        "2402": "KALICONT000005635953",  # Travail temporaire
        "1090": "KALICONT000005635085",  # GMS
        "3043": "KALICONT000027887800",  # Propreté
        "1459": "KALICONT000005635215",  # Nettoyage industriel
        "1996": "KALICONT000005635408",  # Transports routiers
        "2941": "KALICONT000022946967",  # BAD
        "3239": "KALICONT000005635436",  # Banque (AFB)
        "2120": "KALICONT000005635682",  # Commerce non alimentaire
        "1597": "KALICONT000005635400",  # Bâtiment ouvriers
        "1605": "KALICONT000005635401",  # Bâtiment ETAM
        "2216": "KALICONT000005635769",  # Commerce détail alimentaire spécialisé
        "0803": "KALICONT000005635220",  # Restauration rapide
        "2098": "KALICONT000005635666",  # FHP
        "1501": "KALICONT000005635215",  # Travaux publics ouvriers
        "1404": "KALICONT000005635179",  # Animation
        "1351": "KALICONT000005635405",  # Sécurité privée
        "2569": "KALICONT000005635964",  # Services automobile
        "0736": "KALICONT000005635140",  # FEHAP
        "2596": "KALICONT000005636020",  # Immobilier
        "2150": "KALICONT000005635697",  # Industrie chimique
        "0787": "KALICONT000005635163",  # Boulangerie artisanale
        "3131": "KALICONT000005635557",  # Télécommunications
        "1285": "KALICONT000005635369",  # Habillement textile
        "0650": "KALICONT000005635118",  # Pharmacie d'officine
        "0095": "KALICONT000005635078",  # Commerce gros alimentaire
        "2609": "KALICONT000005636055",  # Coiffure
        "1483": "KALICONT000005635236",  # Mutualité
        "2511": "KALICONT000005635939",  # Sport
        "2264": "KALICONT000005635803",  # Distribution directe
        "1424": "KALICONT000005635191",  # Cabinets médicaux
        "0292": "KALICONT000005635088",  # Plasturgie
        "0044": "KALICONT000005635059",  # Industries alimentaires
        "2691": "KALICONT000005636102",  # Paysagistes
        "0675": "KALICONT000005635127",  # Papiers-cartons
        "3016": "KALICONT000005635435",  # Organismes de formation
        "2148": "KALICONT000005635688",  # Agences d'assurances
        "0016": "KALICONT000005635049",  # Transports aériens
        "0158": "KALICONT000005635073",  # Imprimerie
        "0979": "KALICONT000005635288",  # Hôtellerie de plein air
        "3230": "KALICONT000027572688",  # ELISFA
        "1258": "KALICONT000005635351",  # Textiles synthétiques
        "1043": "KALICONT000005635302",  # Chaussure succursaliste
        "2785": "KALICONT000005636172",  # Avocats salariés
        "0086": "KALICONT000005635068",  # Navigation fluviale
    }
    ...
    def get_piste_token():
        """Obtient un token OAuth2 depuis PISTE (déjà utilisé dans le script)."""
        ...
        try:
            ...
            print(f"  ⚠️ PISTE auth échouée ({e})")
            return None

    def get_dernier_avenant_salaires(kalicont_id, token):
        """
        Interroge l'API Légifrance KALI pour récupérer la date du dernier
        avenant "Textes Salaires" d'une CCN.

        Stratégie : endpoint consult/kali → section TEXTES_SALAIRES du sommaire.
        Retourne YYYY-MM (str) ou None si aucun texte salaire trouvé.
        """
        # Endpoint stable : récupère la structure complète d'une CCN par son KALICONT
        url = "https://api.piste.gouv.fr/dila/legifrance/lf-engine-app/consult/kali"
        payload = json.dumps({"id": kalicont_id}).encode()
        ...
        # Structure attendue : result["convention"]["textesSalaires"] ou
        # result["sections"] contenant les avenants salaires.
        # On cherche récursivement toutes les dates dans les nœuds "TEXTES_SALAIRES".
        dates = []

        def extract_dates(node):
            if not isinstance(node, dict):
                return
            # Nœud de type section salaires
            ntype = node.get("type", "") or node.get("typeContenu", "")
            if "SALAIRE" in ntype.upper():
                d = (node.get("datePubliTexte") or node.get("dateSignatureTexte")
                     or node.get("dateDebut") or node.get("date") or "")
                if d and len(d) >= 7:
                    dates.append(d[:7])
            # Date sur n'importe quel nœud si "salaire" dans le titre
            titre = (node.get("titre") or node.get("intitule") or node.get("libelle") or "").lower()
            if "salaire" in titre or "revalorisation" in titre or "minima" in titre:
                d = (node.get("datePubliTexte") or node.get("dateSignatureTexte")
                     or node.get("dateDebut") or node.get("date") or "")
                if d and len(d) >= 7:
                    dates.append(d[:7])
            # Récursion sur toutes les valeurs list/dict
        ...
        extract_dates(result)
        return max(dates) if dates else None

    # Récupération token PISTE (réutilise les secrets déjà présents dans le workflow)
    alertes_avenants = {}
    piste_token = get_piste_token()
    if piste_token:
        print(f"  🔑 PISTE authentifié — vérification des avenants salaires sur {len(KALICONT_MAP)} branches...")
        nb_alertes = 0
        nb_ok = 0
        nb_err = 0
        for idcc, kalicont_id in KALICONT_MAP.items():
            # Trouver la derniere_revalorisation de cette branche dans BRANCHES_BASE
            branche_ref = next((b for b in BRANCHES_BASE if b["idcc"] == idcc), None)
            if not branche_ref:
                continue
            date_codee = branche_ref.get("derniere_revalorisation", "")
            annee_codee, mois_codee = parse_date_fr(date_codee)
            try:
                dernier_ym = get_dernier_avenant_salaires(kalicont_id, piste_token)
                if dernier_ym:
                    annee_api = int(dernier_ym[:4])
                    mois_api  = int(dernier_ym[5:7])
                    if (annee_api, mois_api) > (annee_codee, mois_codee):
                        mois_nom = [k for k, v in MOIS_FR.items() if v == mois_api]
                        label = f"{mois_nom[0].capitalize()} {annee_api}" if mois_nom else dernier_ym
                        alertes_avenants[idcc] = {
                            "avenant_detecte": label,
                            "grille_codee":    date_codee,
                            "message":         f"⚠️ Avenant détecté ({label}) — grille à vérifier sur Légifrance",
                        }
                        nb_alertes += 1
                    else:
                        nb_ok += 1
                    ...
                    nb_ok += 1
            except Exception as _piste_err:
                nb_err += 1
                if nb_err == 1:
                    # Logger la première erreur pour diagnostiquer en cas de problème
                    print(f"  ⚠️  Erreur PISTE (ex: IDCC {idcc}): {type(_piste_err).__name__}: {_piste_err}")
        print(f"  📊 Avenants : {nb_alertes} alerte(s) · {nb_ok} à jour · {nb_err} erreur(s) API")
        if nb_alertes:
            print(f"  ⚠️  Branches avec avenant plus récent que la grille codée :")
        ...
        print("  ℹ️  PISTE non configuré (PISTE_CLIENT_ID/SECRET absents) — détection avenants désactivée")
    ...
    return {
        ...
        "meta": {
            ...
            "source_avenants":        "API Légifrance KALI via PISTE (DILA)",
            "methodologie":           (
...
def main():
    ...
    print()
    print("   📋 CONVENTIONS COLLECTIVES - Effectifs : kali-data (SocialGouv/GitHub) ✅")
    print("   📋 CONVENTIONS COLLECTIVES - Avenants salaires : API PISTE Légifrance ✅")
    print("   ⚠️  Si alerte avenant dans les logs : vérifier + mettre à jour BRANCHES_BASE")
    print("      → https://www.legifrance.gouv.fr (Textes Salaires de la CCN concernée)")
    print()
