...
def build_emplois_vacants_data():
    ...
    vacants_raw = fetch_dares_dataset_filtered(
        "dares_emploivacants_brut_emploisvacants",
        where='date >= "2015-T1" AND nomenclature = "A21"',
        limit=100
    )
    occupes_raw = fetch_dares_dataset_filtered(
    ...
    def parse_dares_records(records, value_field=None):
        ...
        # Nomenclature préférée : A21 (la plus courante dans le dataset)
        ...
        for record in records:
            # Filtrer sur la nomenclature préférée
