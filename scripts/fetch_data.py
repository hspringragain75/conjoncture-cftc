...
def build_conventions_collectives_enrichies(smic_net, smic_brut):
    ...
    try:
        ...
        for entry in kali_idx:
            ...
            ktext = entry.get("texte_de_base") or ""
            if num and ktext:
                idcc_kalitext[num] = ktext
        # Debug : montrer les formats num dans l'index vs idcc dans BRANCHES_BASE
        sample_nums = [(str(e.get("num","")).strip(), str(e.get("num","")).strip().zfill(4)) for e in kali_idx[:5]]
        print(f"  🔍 Exemples num dans index : {sample_nums}")
        sample_idcc = [b["idcc"] for b in BRANCHES_BASE[:5]]
        print(f"  🔍 Exemples idcc BRANCHES_BASE : {sample_idcc}")
        print(f"  🔍 Matches : {sum(1 for b in BRANCHES_BASE if str(b['idcc']).zfill(4) in idcc_kalitext)}/48")
        print(f"  📅 kali-data index : {len(idcc_kalitext)} CCN avec KALITEXT")
    except Exception as e:
    ...
    if idcc_kalitext:
        ...
        for branch in BRANCHES_BASE:
            ...
            try:
                ...
                nb_err += 1
                if nb_err <= 3:
                    print(f"  ⚠️  GitHub API erreur IDCC {idcc} ({ktext}): {type(_gh_err).__name__}: {_gh_err}")
