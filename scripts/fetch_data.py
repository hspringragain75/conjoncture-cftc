...
def build_emplois_vacants_data():
    ...
    def parse_dares_records(records, value_field=None):
        ...
        for record in records:
            # Filtrer sur CVS (données corrigées des variations saisonnières)
            type_donnees = (record.get("type_de_donnees") or "").strip()
            if type_donnees and type_donnees.upper() not in ("CVS", ""):
                continue
