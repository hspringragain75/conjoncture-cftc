...
def build_marches_financiers_data():
    """
    Recupere les donnees des marches financiers :
    - OAT 10 ans (TEC-10) via AFT, fallback FRED, fallback BCE/SDW
    - Prix du Brent via OilPriceAPI
    - CAC 40 via Yahoo Finance (v8 JSON API)
    """
    import ssl
    import re
    from datetime import datetime as dt_local, timedelta

    print("  📊 Marches financiers...")
    ...
    try:
        print("    -> OAT 10 ans : tentative AFT (TEC-10)...")
        aft_url = "https://www.aft.gouv.fr/en/today-tec-10-index"
        aft_headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36",
            "Accept-Language": "en-US,en;q=0.9,fr;q=0.7",
        }
        resp = safe_urlopen(aft_url, headers=aft_headers, timeout=20)
        html = resp.read().decode("utf-8", errors="replace")
        pairs = re.findall(r'(\d{2}/\d{2}/\d{4})\s*</td>\s*<td[^>]*>\s*([\d.,]+)\s*%?', html)
        if not pairs:
            pairs = re.findall(r'(\d{2}/\d{2}/\d{4})[^0-9]+([\d]+[.,]\d+)', html)
        if pairs:
            for date_str, val_str in pairs:
                try:
                    v = float(val_str.replace(",", ".").replace("%", ""))
                    d = dt_local.strptime(date_str, "%d/%m/%Y")
                    oat_historique.append({"date": d.strftime("%Y-%m-%d"), "valeur": round(v, 2)})
                except (ValueError, TypeError):
                    continue
            if oat_historique:
                oat_historique.sort(key=lambda x: x["date"])
                dernier = oat_historique[-1]
                oat_valeur = dernier["valeur"]
                oat_date = dernier["date"]
                oat_source = "AFT (TEC-10 quotidien)"
                print(f"    OAT 10 ans AFT : {oat_valeur}% ({oat_date})")
        if oat_valeur is None:
            raise RuntimeError("AFT: aucune paire date/valeur extraite")
    except Exception as e:
        print(f"    AFT indisponible : {e}")

    if oat_valeur is None:
        try:
            print("    -> OAT 10 ans : fallback FRED...")
            fred_url = "https://fred.stlouisfed.org/graph/fredgraph.csv?id=IRLTLT01FRM156N"
                ...
                for line in reversed(lines[1:]):
                    ...
                    if len(parts) >= 2 and parts[1].strip() != ".":
                        try:
                            ...
                            oat_valeur = round(v, 2)
                            oat_date = parts[0].strip()
                            oat_source = "FRED/OECD (mensuel)"
                            for l in lines[-12:]:
                                p = l.split(",")
                                if len(p) >= 2:
                                    try:
                                        oat_historique.append({"date": p[0].strip(), "valeur": round(float(p[1].strip()), 2)})
                                    except (ValueError, TypeError):
                                        pass
                            print(f"    OAT 10 ans FRED : {oat_valeur}% ({oat_date})")
                            break
                        except ValueError:
                            continue
        except Exception as e:
            print(f"    FRED indisponible : {e}")

    if oat_valeur is None:
        try:
            print("    -> OAT 10 ans : fallback BCE/SDW...")
                    ...
                    for line in reversed(lines[1:]):
                        ...
                        if len(parts) > max(time_idx, val_idx):
                            try:
                                ...
                                oat_valeur = round(v, 2)
                                oat_date = parts[time_idx].strip().strip('"')
                                oat_source = "BCE/SDW (mensuel)"
                                for l in lines[-12:]:
                                    p = l.split(",")
                                    if len(p) > max(time_idx, val_idx):
                                        try:
                                            oat_historique.append({"date": p[time_idx].strip().strip('"'), "valeur": round(float(p[val_idx].strip().strip('"')), 2)})
                                        except (ValueError, TypeError):
                                            pass
                                print(f"    OAT 10 ans BCE : {oat_valeur}% ({oat_date})")
                                break
                            except ValueError:
                                continue
        except Exception as e:
    ...
    if oat_valeur is None:
        ...
        oat_historique = [
            {"date": "2025-09", "valeur": 2.95}, {"date": "2025-10", "valeur": 3.02},
            {"date": "2025-11", "valeur": 3.08}, {"date": "2025-12", "valeur": 3.10},
            {"date": "2026-01", "valeur": 3.12}, {"date": "2026-02", "valeur": 3.15},
        ]
        print(f"    OAT 10 ans : fallback statique {oat_valeur}%")
    ...
    try:
        print("    -> Petrole Brent : tentative OilPriceAPI...")
        oil_api_key = "24cb91ff68cac55c508fff64ca15351f2ccc800919d88156d133339a818f1de1"
        oil_url = "https://api.oilpriceapi.com/v1/prices/latest"
        oil_headers = {"Authorization": f"Token {oil_api_key}", "Content-Type": "application/json"}
        resp = safe_urlopen(oil_url, headers=oil_headers, timeout=15)
        oil_data = json.loads(resp.read().decode("utf-8"))
        if "data" in oil_data and "price" in oil_data["data"]:
            brent_prix = round(oil_data["data"]["price"], 2)
            brent_date = oil_data["data"].get("created_at", dt_local.now().strftime("%Y-%m-%d"))
            if isinstance(brent_date, str) and "T" in brent_date:
                brent_date = brent_date.split("T")[0]
            brent_source = "OilPriceAPI"
            print(f"    Brent : {brent_prix} USD ({brent_date})")
        else:
            raise RuntimeError(f"Structure inattendue : {list(oil_data.keys())}")
    except Exception as e:
        print(f"    OilPriceAPI indisponible : {e}")

    if brent_prix is None:
        brent_prix = 73.50
        brent_date = "2026-02-28"
        brent_source = "Valeur statique (fallback)"
        print(f"    Brent : fallback statique {brent_prix} USD")

    brent_historique = [
        {"date": "2024-06", "valeur": 82.6}, {"date": "2024-07", "valeur": 84.8},
        {"date": "2024-08", "valeur": 79.0}, {"date": "2024-09", "valeur": 73.9},
        {"date": "2024-10", "valeur": 74.4}, {"date": "2024-11", "valeur": 73.1},
        {"date": "2024-12", "valeur": 73.8}, {"date": "2025-01", "valeur": 77.5},
        {"date": "2025-02", "valeur": 75.5}, {"date": "2025-03", "valeur": 72.8},
        {"date": "2025-04", "valeur": 66.9}, {"date": "2025-05", "valeur": 64.8},
        {"date": "2025-06", "valeur": 65.2}, {"date": "2025-07", "valeur": 67.5},
        {"date": "2025-08", "valeur": 69.3}, {"date": "2025-09", "valeur": 70.1},
        {"date": "2025-10", "valeur": 71.2}, {"date": "2025-11", "valeur": 72.0},
        {"date": "2025-12", "valeur": 72.8}, {"date": "2026-01", "valeur": 73.1},
        {"date": "2026-02", "valeur": brent_prix},
    ]

    cac40_valeur = None
    cac40_date = None
    cac40_variation_jour = None
    cac40_source = "N/A"
    cac40_historique = []

    try:
        print("    -> CAC 40 : tentative Yahoo Finance...")
        yf_url = "https://query1.finance.yahoo.com/v8/finance/chart/%5EFCHI?range=6mo&interval=1d"
        yf_headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"}
        resp = safe_urlopen(yf_url, headers=yf_headers, timeout=15)
        yf_data = json.loads(resp.read().decode("utf-8"))
        result = yf_data["chart"]["result"][0]
        meta = result["meta"]
        timestamps = result["timestamp"]
        closes = result["indicators"]["quote"][0]["close"]
        cac40_valeur = round(meta.get("regularMarketPrice", closes[-1]), 2)
        cac40_date = dt_local.fromtimestamp(timestamps[-1]).strftime("%Y-%m-%d")
        if len(closes) >= 2 and closes[-2] and closes[-1]:
            cac40_variation_jour = round((closes[-1] / closes[-2] - 1) * 100, 2)
        monthly_data = {}
        for ts, c in zip(timestamps, closes):
            if c is None:
                continue
            d = dt_local.fromtimestamp(ts)
            monthly_data[d.strftime("%Y-%m")] = round(c, 0)
        cac40_historique = [{"date": k, "valeur": v} for k, v in sorted(monthly_data.items())]
        cac40_source = "Yahoo Finance"
        print(f"    CAC 40 : {cac40_valeur} pts ({cac40_date})")
    except Exception as e:
        print(f"    Yahoo Finance indisponible : {e}")

    if cac40_valeur is None:
        cac40_valeur = 8120.43
        cac40_date = "2026-02-28"
        cac40_variation_jour = 0.35
        cac40_source = "Valeur statique (fallback)"
        cac40_historique = [
            {"date": "2024-06", "valeur": 7530}, {"date": "2024-07", "valeur": 7573},
            {"date": "2024-08", "valeur": 7431}, {"date": "2024-09", "valeur": 7635},
            {"date": "2024-10", "valeur": 7382}, {"date": "2024-11", "valeur": 7235},
            {"date": "2024-12", "valeur": 7380}, {"date": "2025-01", "valeur": 7950},
            {"date": "2025-02", "valeur": 8120}, {"date": "2025-03", "valeur": 7990},
            {"date": "2025-04", "valeur": 7350}, {"date": "2025-05", "valeur": 7850},
            {"date": "2025-06", "valeur": 7790}, {"date": "2025-07", "valeur": 7650},
            {"date": "2025-08", "valeur": 7560}, {"date": "2025-09", "valeur": 7730},
            {"date": "2025-10", "valeur": 7850}, {"date": "2025-11", "valeur": 7920},
            {"date": "2025-12", "valeur": 8020}, {"date": "2026-01", "valeur": 8050},
            {"date": "2026-02", "valeur": cac40_valeur},
        ]
        print(f"    CAC 40 : fallback statique {cac40_valeur}")

    return {
        "oat_10ans": {
            "valeur": oat_valeur, "date": oat_date, "unite": "%",
            "source": oat_source, "historique": oat_historique[-24:],
            "evolution_annuelle": [
        ...
        "petrole_brent": {
            "valeur": brent_prix, "date": brent_date, "unite": "USD/baril",
            "source": brent_source, "historique": brent_historique,
        ...
        "cac40": {
            "valeur": cac40_valeur, "date": cac40_date,
            "variation_jour_pct": cac40_variation_jour or 0,
            "unite": "points", "source": cac40_source,
            "historique": cac40_historique,
        ...
        "sources": "AFT (OAT TEC-10), OilPriceAPI (Brent), Yahoo Finance (CAC 40)"
    }
...
def main():
    ...
    data = {
        ...
        # ============================================================
        "sources_par_onglet": {
            "conjoncture": "INSEE - Comptes nationaux, Banque de France, AFT (OAT), OilPriceAPI (Brent), Yahoo Finance (CAC 40)",
