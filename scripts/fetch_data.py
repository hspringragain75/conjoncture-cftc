...
def score_grille_rows(rows):
    if not rows:
        return 0.0
    score = 0.0
    has_mensuel = any(r.get("minimum_mensuel") for r in rows)
    has_coeff   = any(r.get("coefficient") for r in rows)
    nb_niveaux  = len({r.get("niveau") for r in rows if r.get("niveau")})

    # Montant mensuel présent = signal fort (seul critère vraiment fiable)
    if has_mensuel:
        score += 0.55  # dépasse déjà le seuil seul
    if len(rows) >= 3:
        score += 0.20
    if len(rows) >= 2:
        score += 0.10
...
def extract_grille_from_kali_agreement(agreement_json):
    ...
    # Extraire toutes les grilles candidates avec leurs montants max
    all_candidates = []
    for block in candidates:
        ...
        montants = [r["minimum_mensuel"] for r in rows if r.get("minimum_mensuel")]
        max_montant = max(montants) if montants else 0
        all_candidates.append({
            "rows": rows,
            "confidence": confidence,
            "max_montant": max_montant,
            "path": block.get("path"),
            "excerpt": (block.get("text") or "")[:280],
        })

    if not all_candidates:
        return [], {"confidence": 0.0, "source_path": None, "source_excerpt": None}

    # Stratégie de sélection : parmi les blocs avec confidence >= 0.40,
    # prendre celui avec le montant mensuel maximal (= le plus récent probablement).
    # Si aucun ne dépasse 0.40, prendre le meilleur par confidence.
    SMIC_PLANCHER = 1400.0  # montant en-dessous duquel on suspecte une grille obsolète

    eligibles = [c for c in all_candidates if c["confidence"] >= 0.40]
    if not eligibles:
        eligibles = all_candidates  # fallback : prendre quand même le meilleur

    # Parmi les éligibles, filtrer ceux dont le max_montant est plausible (> plancher)
    plausibles = [c for c in eligibles if c["max_montant"] >= SMIC_PLANCHER]

    # Si des blocs plausibles existent, prendre celui avec le montant max
    # Sinon fallback sur confidence max (peut être un vrai mini bas, ex: apprentis)
    if plausibles:
        best = max(plausibles, key=lambda c: c["max_montant"])
    else:
        best = max(eligibles, key=lambda c: c["confidence"])
...
def compute_branche_statut(grille, smic_net, confidence=0.0):
    ...
    if confidence < 0.40:
        return "a_verifier"
    montants = [row.get("minimum_mensuel") or 0 for row in grille if row.get("minimum_mensuel")]
    if not montants:
        return "a_verifier"
    max_montant = max(montants)
    # Si le montant max est < 85% du SMIC actuel, la grille est clairement obsolète
    # (aucune CCN légale n'a un plafond aussi bas aujourd'hui) → a_verifier
    if max_montant < smic_net * 0.85:
        return "a_verifier"
    if any(m < smic_net for m in montants):
        return "non_conforme"
    return "conforme"
...
def compute_branche_quality(grille, confidence):
    ...
    if confidence < 0.40:
        return "fragile"
    if confidence < 0.70:
        return "partielle"   # 1-2 niveaux extraits, montant présent
    if confidence < 0.85:
        return "moyenne"
    return "bonne"
...
def build_conventions_collectives_enrichies(smic_net, smic_brut):
    """
    100% automatique — zéro saisie manuelle.

    Catalogue : toutes les CCN disponibles dans kali-data (SocialGouv).
    Grilles : extraction automatique depuis les JSON individuels kali-data.
    Statut : conforme / non_conforme / a_verifier selon extraction + SMIC.
    """
    print("📋 Construction des conventions collectives (100% automatique)...")

    # ── Étape 1 : charger l'index kali-data → catalogue des CCN ──────────────
    try:
        kali_index = fetch_kali_index()
        print(f"  ✅ Index kali-data chargé : {len(kali_index)} conventions")
    except Exception as e:
        print(f"  ❌ Index kali-data indisponible : {e}")
        return {
            "smic_reference": {"mensuel": smic_net, "mensuel_brut": smic_brut,
                               "annuel": round(smic_net * 12), "date": "Janvier 2026"},
            "statistiques_branches": {},
            "branches": [],
            "meta": {"erreur": str(e), "date_generation": datetime.now().isoformat()},
        }

    # ── Étape 2 : enrichir chaque CCN depuis son JSON individuel ─────────────
    branches = []
    nb_ok = 0
    nb_verify = 0
    nb_non_conforme = 0

    for idcc, kali_entry in kali_index.items():
        # Construire la base depuis l'index (sans saisie manuelle)
        base = {
            "idcc":  idcc,
            "id":    f"branch_{idcc}",
            "nom":   kali_entry.get("shortTitle") or kali_entry.get("title") or f"CCN {idcc}",
            "titre_complet": kali_entry.get("title") or "",
            "effectif":      kali_entry.get("effectif"),
            "date_publi":    (kali_entry.get("date_publi") or "")[:10],
            "source": (kali_entry.get("url") or
                       f"https://www.legifrance.gouv.fr/search/result?query=idcc+{idcc}"),
            "meta": {},
        }

        enriched = enrich_branch_from_kali(base, kali_entry, smic_net)
        ...
        if statut == "conforme":
            ...
            nb_non_conforme += 1
            m = enriched.get("meta", {})
            grille = enriched.get("grille", [])
            mins = [r.get("minimum_mensuel") for r in grille if r.get("minimum_mensuel")]
            src = (m.get("source_excerpt") or "")[:120].replace("\n", " ")
            print(f"    ❌ IDCC {idcc} {enriched['nom'][:28]:<28} "
                  f"conf={m.get('confidence',0):.2f} montants={mins} "
                  f"| extrait: {src!r}")
        else:
            nb_verify += 1

    # tri par effectif décroissant (None en dernier)
    branches.sort(key=lambda b: int(b.get("effectif") or 0), reverse=True)

    stats = summarize_conventions(branches)

    print(f"  📊 {stats['total_branches']} CCN · ✅ {stats['branches_conformes']} conformes "
          f"· ❌ {stats['branches_non_conformes']} sous SMIC · 🟠 {stats['branches_a_verifier']} à vérifier")

    nb_avec_grille = sum(1 for b in branches if b.get("grille"))
    nb_avec_effectif = sum(1 for b in branches if b.get("effectif"))
    print(f"  📋 Grilles extraites : {nb_avec_grille}/{stats['total_branches']} "
          f"· {nb_avec_effectif} avec effectif")
    ...
    return {
        ...
        "statistiques_branches": {
            **stats,
            "source_statistiques": "kali-data (SocialGouv / Fabrique numérique des ministères sociaux)",
        },
        "meta": {
            "source_effectifs": "DSN via kali-data",
            "source_grilles":   "SocialGouv kali-data (extraction automatique)",
            "date_generation":  datetime.now().isoformat(),
            "mode":             "100pct_automatique",
            "note":             "Extraction heuristique depuis textes juridiques KALI. "
                                "Statut a_verifier si grille non extraite ou confidence < 55%.",
        },
        "branches": branches,
    }
...
def main():
    ...
    print("   ⚠️  Si alerte avenant dans les logs : vérifier sur Légifrance (lien dans les données)")
