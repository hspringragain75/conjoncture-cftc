...
def build_emplois_vacants_data():
    ...
    # DEBUG temporaire
    dates = sorted(set(r.get("date","") for r in vacants_raw if r.get("date")))
    nomens = sorted(set(r.get("nomenclature","") for r in vacants_raw if r.get("nomenclature")))
    types = sorted(set(r.get("type_de_donnees","") for r in vacants_raw if r.get("type_de_donnees")))
    print(f"  🔍 Dates: {dates[:5]}...{dates[-3:] if len(dates)>5 else ''}")
    print(f"  🔍 Nomenclatures: {nomens}")
    print(f"  🔍 Types: {types}")
    ...
    def parse_dares_records(records, value_field=None):
        ...
        result = sorted(by_trimestre.values(), key=lambda x: x["trimestre"])
        if result:
