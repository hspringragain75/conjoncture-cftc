...
def build_conventions_collectives_enrichies(smic_net, smic_brut):
    ...
    def get_dernier_avenant_salaires(kalicont_id, token):
        """
        Interroge l'API Légifrance KALI pour récupérer la liste des textes salaires
        d'une CCN et retourne la date du plus récent (format YYYY-MM-DD ou None).
        """
        url = "https://api.piste.gouv.fr/dila/legifrance/lf-engine-app/list/sommaire"
        payload = json.dumps({
            "fond": "KALI",
            "idConteneur": kalicont_id,
            "typeSommaire": "TEXTES_SALAIRES",
        }).encode()
        ...
        # La réponse contient une liste d'articles/textes avec des dates
        items = result.get("listeElementsContentieux", []) or result.get("listeTableMatieres", [])
        if not items:
            # Tentative sur la structure plate
            items = result.get("sommaire", [])
        dates = []
        for item in items:
            d = item.get("datePubliTexte") or item.get("dateDebut") or item.get("date") or ""
            if d and len(d) >= 7:
