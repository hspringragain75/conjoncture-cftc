...
def compute_uncertainty_decomposition(
    ...
    """
    Décomposition par ablation : on annule successivement chaque source
    d'incertitude pour mesurer sa contribution à l'IC total.

    Run A (référence complète)   : vol réelle + signaux marchés + incertitude BDF
    Run B (sans signaux marchés) : vol réelle + vol_bdf seulement
    Run C (sans incertitude BDF) : vol réelle + signaux marchés, cible fixe
    Run D (sans vol historique)  : vol minimale + signaux marchés + incertitude BDF

    Contribution de chaque source = IC_complet - IC_sans_cette_source
    Normalisation : somme des contributions = 100%
    """
    month, _ = get_month_year()
    VOL_MIN = 0.05  # volatilité quasi-nulle pour annuler cette source
    ...
    # Perturbations BDF multiples pour stabiliser l'estimation
    bdf_perturbations_inf = [random.gauss(0, bdf_uncertainty * 0.5) for _ in range(5)]
    bdf_perturbations_cho = [random.gauss(0, bdf_uncertainty * 0.35) for _ in range(5)]
    ...
    def run_ic(v_inf, v_cho, t_inf, t_cho, n=N_SIMULATIONS):
        ri, rc = monte_carlo_bivariate(
            ...
            n_sims=n, start_month=month,
            seasonal_inf=SEASONAL_INFLATION
        )
        return (round(ri['p90'][-1] - ri['p10'][-1], 2),
                round(rc['p90'][-1] - rc['p10'][-1], 2))

    # Run A : complet (moyenne sur plusieurs perturbations BDF)
    ic_a_inf_list, ic_a_cho_list = [], []
    for p_inf, p_cho in zip(bdf_perturbations_inf[:3], bdf_perturbations_cho[:3]):
        a_inf, a_cho = run_ic(vol_inf, vol_cho,
                               t_inf_full + p_inf, t_cho_full + p_cho)
        ic_a_inf_list.append(a_inf)
        ic_a_cho_list.append(a_cho)
    ic_a_inf = round(mean(ic_a_inf_list), 2)
    ic_a_cho = round(mean(ic_a_cho_list), 2)

    # Run B : sans signaux marchés (cible BDF de base + incertitude BDF)
    ic_b_inf_list, ic_b_cho_list = [], []
    for p_inf, p_cho in zip(bdf_perturbations_inf[:3], bdf_perturbations_cho[:3]):
        b_inf, b_cho = run_ic(vol_inf, vol_cho,
                               target_inf + p_inf, target_cho + p_cho)
        ic_b_inf_list.append(b_inf)
        ic_b_cho_list.append(b_cho)
    ic_b_inf = round(mean(ic_b_inf_list), 2)
    ic_b_cho = round(mean(ic_b_cho_list), 2)

    # Run C : sans incertitude BDF (cible fixe + signaux marchés)
    ic_c_inf, ic_c_cho = run_ic(vol_inf, vol_cho, t_inf_full, t_cho_full)

    # Run D : sans volatilité historique (vol minimale)
    ic_d_inf_list, ic_d_cho_list = [], []
    for p_inf, p_cho in zip(bdf_perturbations_inf[:3], bdf_perturbations_cho[:3]):
        d_inf, d_cho = run_ic(VOL_MIN, VOL_MIN,
                               t_inf_full + p_inf, t_cho_full + p_cho)
        ic_d_inf_list.append(d_inf)
        ic_d_cho_list.append(d_cho)
    ic_d_inf = round(mean(ic_d_inf_list), 2)
    ic_d_cho = round(mean(ic_d_cho_list), 2)

    # Contributions par ablation (clampées à 0 minimum)
    def contributions(ic_full, ic_sans_mkt, ic_sans_bdf, ic_sans_vol):
        contrib_mkt = max(0.0, round(ic_full - ic_sans_mkt, 2))
        contrib_bdf = max(0.0, round(ic_full - ic_sans_bdf, 2))
        contrib_vol = max(0.0, round(ic_full - ic_sans_vol, 2))
        total = contrib_vol + contrib_mkt + contrib_bdf
        if total <= 0:
            # Fallback : tout attribuer à la volatilité
            return ic_full, 0.0, 0.0, ic_full
        # Normaliser
        scale = ic_full / total
        return (round(contrib_vol * scale, 2),
                round(contrib_mkt * scale, 2),
                round(contrib_bdf * scale, 2),
                ic_full)

    cv_inf, cm_inf, cb_inf, total_inf = contributions(
        ic_a_inf, ic_b_inf, ic_c_inf, ic_d_inf)
    cv_cho, cm_cho, cb_cho, total_cho = contributions(
        ic_a_cho, ic_b_cho, ic_c_cho, ic_d_cho)
    ...
    return {
        'inflation': {
            'ic_total_pts': total_inf,
            'sources': {
                'volatilite_historique': {
                    'ic_pts': cv_inf,
                    'part_pct': pct(cv_inf, total_inf),
                    'label': "Variabilité passée de l'inflation"
                },
                'signaux_marches': {
                    'ic_pts': cm_inf,
                    'part_pct': pct(cm_inf, total_inf),
                    'label': 'Brent, gaz, EUR/USD, SMB...'
                },
                'incertitude_bdf': {
                    'ic_pts': cb_inf,
                    'part_pct': pct(cb_inf, total_inf),
                    'label': 'Incertitude sur la cible Banque de France'
                }
            }
        },
        'chomage': {
            'ic_total_pts': total_cho,
            'sources': {
                'volatilite_historique': {
                    'ic_pts': cv_cho,
                    'part_pct': pct(cv_cho, total_cho),
                    'label': 'Variabilité passée du chômage'
                },
                'signaux_marches': {
                    'ic_pts': cm_cho,
                    'part_pct': pct(cm_cho, total_cho),
                    'label': 'Défaillances, taux immo, CAC 40...'
                },
                'incertitude_bdf': {
                    'ic_pts': cb_cho,
                    'part_pct': pct(cb_cho, total_cho),
                    'label': 'Incertitude sur la cible Banque de France'
                }
            }
        },
        ...
        'note': (
            f'IC = intervalle p90-p10 à 12 mois. '
            f'3 × {N_SIMULATIONS} simulations bivariées (corrélation Phillips ρ={PHILLIPS_CORRELATION}).'
        )
    }
