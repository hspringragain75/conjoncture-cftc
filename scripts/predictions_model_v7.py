...
def compute_uncertainty_decomposition(
    ...
    def run(c_inf, t_inf, v_inf, c_cho, t_cho, v_cho, n=N_SIMULATIONS):
        ri, rc = monte_carlo_bivariate(
            current_inf=c_inf, target_inf=t_inf, vol_inf=v_inf,
            current_cho=c_cho, target_cho=t_cho, vol_cho=v_cho,
    ...
    ic_vol_inf, ic_vol_cho = run(current_inf, target_inf, vol_inf,
                                  current_cho, target_cho, vol_cho)

    # Run 2 : + signaux marchés
    t_inf_adj = target_inf + market_signals.get('inflation_adjustment', 0)
    t_cho_adj = target_cho + market_signals.get('chomage_adjustment', 0)
    ic_mkt_inf, ic_mkt_cho = run(current_inf, t_inf_adj, vol_inf,
                                   current_cho, t_cho_adj, vol_cho)

    # Run 3 : + incertitude BDF
    t_inf_bdf = t_inf_adj + random.gauss(0, bdf_uncertainty * 0.3)
    t_cho_bdf = t_cho_adj + random.gauss(0, bdf_uncertainty * 0.2)
    ic_full_inf, ic_full_cho = run(current_inf, t_inf_bdf, vol_inf,
                                    current_cho, t_cho_bdf, vol_cho)
    ...
    return {
        'inflation': {
            ...
            'sources': {
                'volatilite_historique': {
                    'ic_pts': ic_vol_inf,
                    'part_pct': pct(ic_vol_inf, total_inf),
                    'label': "Variabilité passée de l'inflation"
                },
                'signaux_marches': {
                    'ic_pts': round(max(0, ic_mkt_inf - ic_vol_inf), 2),
                    'part_pct': pct(max(0, ic_mkt_inf - ic_vol_inf), total_inf),
                    'label': 'Brent, gaz, EUR/USD, SMB...'
                },
                'incertitude_bdf': {
                    'ic_pts': round(max(0, ic_full_inf - ic_mkt_inf), 2),
                    'part_pct': pct(max(0, ic_full_inf - ic_mkt_inf), total_inf),
        ...
        'chomage': {
            ...
            'sources': {
                'volatilite_historique': {
                    'ic_pts': ic_vol_cho,
                    'part_pct': pct(ic_vol_cho, total_cho),
                    'label': 'Variabilité passée du chômage'
                },
                'signaux_marches': {
                    'ic_pts': round(max(0, ic_mkt_cho - ic_vol_cho), 2),
                    'part_pct': pct(max(0, ic_mkt_cho - ic_vol_cho), total_cho),
                    'label': 'Défaillances, taux immo, CAC 40...'
                },
                'incertitude_bdf': {
                    'ic_pts': round(max(0, ic_full_cho - ic_mkt_cho), 2),
                    'part_pct': pct(max(0, ic_full_cho - ic_mkt_cho), total_cho),
                    'label': 'Incertitude sur la cible Banque de France'
                }
            }
        },
